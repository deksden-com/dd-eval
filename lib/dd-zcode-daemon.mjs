...
class DaemonRuntime {
  ...
  async prepareStop(cancel) {
    ...
    for (const session of this.sessions.values()) {
      ...
      let observed = await inspectSessionWithBridge(this.bridge, options);
      if (this.treeRunning(observed)) {
        if (!cancel) unsettled.push(session.provider_session_id);
        else {
          const cancelled = await cancelSessionWithBridge(this.bridge, options);
          // Cancellation acceptance is not settlement.  Re-read the same
          // controlled tree before letting the daemon close its endpoint.
          observed = await inspectSessionWithBridge(this.bridge, options);
