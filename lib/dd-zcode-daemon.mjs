...
export async function callDaemon(stateDir, operation, params = {}, timeoutMs = 30_000) {
  ...
  const request = { schema_id: REQUEST_SCHEMA, id: process.env.DD_EVAL_OPERATION_ID ?? randomUUID(), operation, params };
  return await new Promise((resolve, reject) => {
    ...
    const timer = Number.isFinite(timeoutMs) && timeoutMs > 0
      ? observedTimeout(() => { client.destroy(); reject(new DaemonError("daemon_timeout", `${operation} timed out`, true)); }, timeoutMs)
      : null;
    ...
    client.on("end", () => finish(new DaemonError("daemon_connection_closed", `${operation} connection closed before a response`, true)));
    client.on("close", () => finish(new DaemonError("daemon_connection_closed", `${operation} connection closed before a response`, true)));
  });
}

function sameConfig(state, config) {
  return ["cwd", "journal", "bin", "zcodePath", "ddFlowBin", "ddFlowHome", "projectRoot", "resourceHome", "livenessTimeoutMs"].every((key) => (state.config?.[key] ?? null) === (config[key] ?? null));
}
...
export async function stopDaemon(options) {
  ...
  return await confirmDaemonStopped(() => callDaemon(options.stateDir, "daemon.stop", { cancelTree: options.cancelTree === true }, timeoutMs), socket, timeoutMs);
}
...
class DaemonRuntime {
  ...
  persist(patch = {}) {
    ...
    this.persisting = this.persisting.then(() => writeState(this.paths.state, snapshot));
    ...
    if (resourceProcess?.id && resourceProcess.lease_token) {
      this.persisting = this.persisting.then(async () => {
        try { await heartbeatDaemonProcess(this.state.config, resourceProcess); }
  ...
  options(params) {
    ...
    const generation = this.activeGeneration ?? this.controlGeneration ?? 0;
    return { ...this.state.config, ...params, adapterSessionId: adapter, daemonId: this.state.daemon_id, allowBackground: true, liveSession: this.liveAdapters.has(adapter), assertDispatch: () => { if (generation !== (this.controlGeneration ?? 0)) throw new DaemonError("operation_cancelled", "ZCode prompt was cancelled before native dispatch"); } };
  }
  ...
  treeRunning(result) {
    return result?.read?.projection?.status === "running" || (result?.subagents?.running ?? result?.evidence?.subagents?.running ?? result?.after?.running ?? []).length > 0;
  }
  ...
  async prepareStop(cancel) {
    ...
    for (const session of this.sessions.values()) {
      ...
      catch (error) { if (!cancel) throw error; }
      if (!observed || this.treeRunning(observed) || (cancel && this.activeProductive)) {
  ...
  async shutdown() {
    ...
    const resourceProcess = this.state.resource_process;
    if (resourceProcess?.id && resourceProcess.lease_token) {
      try { await finishDaemonProcess(this.state.config, resourceProcess, "stopped", "daemon_stopped"); }
    ...
    connection.on("data", (chunk) => {
      ...
      void (async () => {
        ...
        try {
          ...
          const result = await durableDaemonDispatch(paths.dir, request, () => runtime.dispatch(request.operation, request.params ?? {}));
