...
class DaemonRuntime {
  ...
  async dispatch(operation, params) {
    ...
    if (operation === "session.prompt") return await this.productive(operation, async () => { await this.requireSettled(params); return await promptSessionWithBridge(this.bridge, this.options(params)); });
