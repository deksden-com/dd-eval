...
export async function startDaemon(options) {
  ...
  await log.close();
  try {
    if (resourceProcess && child.pid) await runtimeProcess(config, "confirm", { id: resourceProcess.id, "lease-token": resourceProcess.lease_token, pid: child.pid, "process-group-id": child.pid, "lease-ms": 300_000 });
