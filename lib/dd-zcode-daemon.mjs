...
function sameConfig(state, config) {
  return ["cwd", "journal", "bin", "zcodePath", "ddFlowBin", "ddFlowHome", "projectRoot", "livenessTimeoutMs"].every((key) => (state.config?.[key] ?? null) === (config[key] ?? null));
}
...
export async function startDaemon(options) {
  ...
  const config = {
    ...
    livenessTimeoutMs: options.livenessTimeoutMs ?? 180_000,
  };
