...
export async function callDaemon(stateDir, operation, params = {}, timeoutMs = 30_000) {
  ...
  return await new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => { client.destroy(); reject(new DaemonError("daemon_timeout", `${operation} timed out`, true)); }, timeoutMs);
    const finish = (error, value) => {
      if (settled) return;
      settled = true; clearTimeout(timer); client.destroy(); error ? reject(error) : resolve(value);
    };
...
function sameConfig(state, config) {
  return ["cwd", "journal", "bin", "zcodePath", "ddFlowBin", "ddFlowHome", "projectRoot"].every((key) => (state.config?.[key] ?? null) === (config[key] ?? null));
}
...
export async function startDaemon(options) {
  ...
  const config = {
    ...
    ddFlowBin: options.ddFlowBin ? absolute(options.ddFlowBin, "--dd-flow-bin") : null,
...
export async function stopDaemon(options) {
  return await callDaemon(options.stateDir, "daemon.stop", { cancelTree: options.cancelTree === true }, options.timeoutMs ?? 30_000);
}
...
class DaemonRuntime {
  ...
  options(params) {
    ...
    return { ...this.state.config, ...params, daemonId: this.state.daemon_id, allowBackground: true, liveSession: this.liveAdapters.has(adapter) };
  }

  track(result, topology = result?.evidence?.subagents) {
    if (result?.provider_session_id) this.sessions.set(result.provider_session_id, {
  ...
  async dispatch(operation, params) {
    ...
    if (operation === "session.inspect") {
      ...
      this.track(result, result.subagents);
...
export async function serveDaemon(stateDir) {
  ...
  process.on("SIGTERM", () => void runtime.persist({ shutdown_state: "running" }).finally(() => process.exit(1)));
  process.on("SIGINT", () => void runtime.persist({ shutdown_state: "running" }).finally(() => process.exit(1)));
