...
export async function startDaemon(options) {
  ...
  if (["clean", "unclean"].includes(previous?.shutdown_state)) await authorizeRetainedDaemonResume(paths.dir, previous, options.sessionId, config);
  if (previous && !["failed", "starting", "clean"].includes(previous.shutdown_state)) throw new DroidError("invalid_harness_crash", "Previous Droid ownership must be reconciled before another daemon starts");
  ...
  await writeJsonAtomic(paths.state, { schema_id: "dd-droid/daemon-state@1", config, daemon_id: config.daemonId, shutdown_state: "starting", sessions: previous?.sessions ?? [], root_session_id: previous?.root_session_id ?? null, descendants: previous?.descendants ?? [], last_result: previous?.last_result ?? null, resource_process: resource });
...
export async function serveDaemon(stateDir) {
  ...
  const dispatch = async request => {
    ...
    if (operation === "session.cancel") {
      ...
      catch (error) { inspected = { harness: "droid-cli", provider_session_id: params.sessionId, descendants: [...runtime.topology.values()], inspection_error: errorRecord(error) }; }
      return { ...inspected, cancelled: true, settled: cleanup.settled, cleanup };
    }
    if (operation === "daemon.stop") {
      ...
      stopping = true;
      try {
        const cleanup = await runtime.closeTree(params.cancelTree === true);
        if (runtime.rootId) try { await runtime.inspect(runtime.rootId, { ignoreActive: true }); } catch (error) { cleanup.inspection_error = errorRecord(error); }
        // Native evidence failures remain visible, but cannot strand copied
        // credentials or the daemon after the owned process tree is gone.
        await removeDroidAuth(state.config); const clean = !cleanup.forced && !cleanup.topology_error && !cleanup.inspection_error;
        await save({ shutdown_state: clean ? "clean" : "unclean", active_tree: false, cleanup }); await finishDaemonProcess(state.config, state.resource_process);
        return { stopped: true, clean, settled: true, cleanup, _shutdown: true };
      }
    ...
    busy = operation;
    try {
      await save({ active_operation: busy });
  ...
  const server = net.createServer(socket => { socket.setEncoding("utf8"); let buffer = "", consumed = false; socket.on("error", () => {}); socket.on("data", chunk => { if (consumed) return; buffer += chunk; const at = buffer.indexOf("\n"); if (at < 0) return; consumed = true;
    void (async () => { let request; try { request = JSON.parse(buffer.slice(0, at)); if (request.schema_id !== REQUEST || typeof request.id !== "string" || typeof request.operation !== "string") throw new DroidError("daemon_protocol", "Malformed Droid daemon request"); let result;
      ...
      } else result = await durableDaemonDispatch(paths.dir, request, () => dispatch(request)); const { _shutdown, ...receipt } = result;
      // Close the listening endpoint before replying. `server.close()` keeps
      // this already accepted connection alive, so the caller receives its
      // durable stop receipt while the Unix pathname is atomically retired.
      // Unlinking it from the request handler races that final response and
      // can leave a stop caller observing the socket for its whole timeout.
      if (_shutdown) server.close();
      socket.end(`${JSON.stringify({ schema_id: RESPONSE, id: request.id, ok: true, result: receipt })}\n`);
      } catch (error) { socket.end(`${JSON.stringify({ schema_id: RESPONSE, id: request?.id, ok: false, error: errorRecord(error) })}\n`); } })();
  }); });
