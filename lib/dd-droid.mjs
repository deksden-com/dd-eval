...
export class DroidRuntime {
  ...
  async resume(id) {
    this.requireIdentity(id); if (this.closed && (await this.captureProcesses()).length) throw new DroidError("tree_not_settled", "Previous provider still owns processes; cancel the tree before resume"); if (!this.closed && this.loaded) return await this.inspect(id);
  ...
  async prompt(id, prompt, operationId) {
    ...
    const timer = setInterval(() => { if (clock.sample(this.activity)) reject(new DroidError("subject_liveness_timeout", "Droid emitted no native progress within its inactivity window")); }, 1_000);
    try {
  ...
  async refreshTopology() {
    ...
    for (const task of invocations) {
      ...
      const native = await readDroidSession(this.paths.factory, task.childSessionId);
      if (!native || !native.settings) { this.topologyIncomplete = true; continue; }
      if (native.header.callingSessionId !== this.rootId || native.header.callingToolUseId !== task.parentToolUseId) throw new DroidError("droid_child_identity_invalid", "Task invocation disagrees with physical child transcript");
      this.profile(native.settings);
      const prior = this.topology.get(task.childSessionId);
      if (prior && prior.tool_use_id !== task.parentToolUseId) throw new DroidError("droid_child_identity_invalid", "Native child was reused for another Work invocation");
      this.topology.set(task.childSessionId, { provider_session_id: task.childSessionId, parent_provider_session_id: this.rootId, tool_use_id: task.parentToolUseId, task_id: task.taskInvocationId, status: terminal(task.status) ? status(task.status) : "running", source: "droid.task_invocations", transcript_path: native.transcript_path });
    }
  ...
  async closeTree(cancel = false) {
    ...
    await this.refreshTopology(); await this.save({ active_tree: false, cleanup: { settled: true, forced, observed_processes: this.ownedProcesses.length } });
    return { settled: true, forced, observed_processes: this.ownedProcesses.length };
  }
}
