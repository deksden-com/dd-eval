...
export { processSnapshot } from "./process-snapshot.mjs";
import { captureDispatchGuard, cancelPendingDispatch } from "./dispatch-fence.mjs";
import { createHash, randomUUID } from "node:crypto";
import { appendFile, chmod, copyFile, mkdir, readFile, readdir, realpath, stat, unlink, writeFile } from "node:fs/promises";
...
import { errorRecord, reportedError } from "./operation-errors.mjs";
import { checkObservedProfile, observeModel } from "./model-observations.mjs";
import { DroidMetadataReader, readDroidRoutingLog } from "./droid-observation.mjs";
...
export async function prepareDroidHome(config) {
  ...
  const hooks = Object.fromEntries(["SessionStart", "PreToolUse", "PostToolUse", "Stop", "SessionEnd", "SubagentStop"].map(event => [event, [{ matcher: "*", hooks: [{ type: "command", command, timeout: 30 }] }]]));
  await writeJsonAtomic(path.join(paths.factory, "hooks.json"), hooks);
  const worker = `---\nname: dd-flow-worker\ndescription: Execute one explicit dd-flow Work packet or a technical capacity marker\nmodel: ${config.model}\nreasoningEffort: ${config.reasoning}\nmcpServers: []\n---\nFor a Work assignment, your first technical action is the exact standalone work start command supplied by the coordinator. If that command is absent, report an incomplete packet and stop: do not run stage pause, stage finish, or another lifecycle command. Follow only the returned Work packet, invoke its exact work finish or fail command, then stop. A Task child never executes or finishes its coordinator's Stage Work. Do not create children or take another Work. For a technical capacity marker, only return the requested marker; do not read the project or call dd-flow.\n`;
...
export async function readDroidSession(factory, id) {
  const file = await metadataReader(factory).locate(id);
  if (!file) return null;
...
export class DroidRuntime {
  ...
  async start() {
    ...
    this.child.stdout.setEncoding("utf8").on("data", chunk => { this.buffer += chunk; let at; while ((at = this.buffer.indexOf("\n")) >= 0) { const line = this.buffer.slice(0, at); this.buffer = this.buffer.slice(at + 1); this.draining = this.draining.then(async () => { if (!line.trim()) return; await this.event(JSON.parse(line)); }).catch(async error => { await this.handleObservationError(error); if (!this.fatalError) this.rejectPending(error); }); } });
    ...
    clearInterval(this.modelObserver);
    this.modelObserver = setInterval(() => {
      if (!this.loaded || this.closing || this.modelObservation) return;
      this.modelObservation = this.observeModels().catch(error => this.handleObservationError(error)).finally(() => { this.modelObservation = null; });
    }, 2_000);
    this.modelObserver.unref();
  }
  ...
  async event(event) {
    ...
    if (n.type === "settings_updated" && this.loaded) await this.observeSettings(sessionId ?? this.rootId, n.settings, { kind: "native.settings_updated", event_sha256: fingerprint });
  ...
  profile(settings) {
    const model = settings?.modelId ?? settings?.model ?? null;
    const nativeModel = this.models?.find(item => (item.id ?? item.modelId) === model);
    const result = { provider: settings?.provider ?? nativeModel?.provider ?? null, model, reasoning: settings?.reasoningEffort ?? null, mode: settings?.autonomyMode ?? (settings?.autonomyLevel ? `auto-${settings.autonomyLevel}` : null) };
    checkObservedProfile(this.config, result, { required: this.config.mode ? ["mode"] : [] });
    return result;
  }
  async observeSettings(id, settings, source, extra = {}) {
    const observed = this.profile(settings);
    const event = await observeModel({ journal: this.config.journal, harness: "droid-cli", sessionId: id, parentSessionId: id === this.rootId ? null : this.rootId,
      requested: this.config, observed, source, ...extra });
    if (id === this.rootId) this.observedProfile = observed;
    this.modelEventIds ??= new Map();
    if (event && this.modelEventIds.get(id) !== event.id) {
      this.modelEventIds.set(id, event.id);
      await this.save({ model_observation: { last_event_id: event.id, observed_at: event.observed_at } });
    }
    return observed;
  }
  validateMetadata(id, native) {
    if (!native || path.resolve(native.header.cwd) !== this.config.cwd || native.header.id !== id || (id !== this.rootId && native.header.callingSessionId !== this.rootId)) throw new DroidError("session_identity_mismatch", "Native model observation is outside the owned Session tree");
    if (id !== this.rootId && !this.topology.has(id)) this.topology.set(id, { provider_session_id: id, parent_provider_session_id: this.rootId, tool_use_id: native.header.callingToolUseId, status: "unknown", source: "native.metadata" });
  }
  async observeModels() {
    if (!this.rootId || !this.loaded) return;
    const catalog = path.join(this.paths.factory, "task-invocations.json");
    const info = await stat(catalog).catch(error => { if (error.code === "ENOENT") return null; throw error; });
    if (info && info.mtimeMs !== this.modelCatalogMtime) {
      const value = await readJson(catalog);
      for (const task of value?.invocations ?? []) if (task.parentSessionId === this.rootId && task.childSessionId) {
        const native = await this.metadata.read(task.childSessionId);
        if (native) this.validateMetadata(task.childSessionId, native);
      }
      this.modelCatalogMtime = info.mtimeMs;
    }
    const log = path.join(this.paths.factory, "logs", "droid-log-single.log");
    const routing = await readDroidRoutingLog(log, this.state.model_log_cursor);
    for (const transition of routing.events) {
      const native = await this.metadata.read(transition.session_id);
      if (!native || (transition.session_id !== this.rootId && native.header.callingSessionId !== this.rootId)) continue;
      this.validateMetadata(transition.session_id, native);
      await this.observeSettings(transition.session_id, { ...native.settings, modelId: transition.model, model: transition.model },
        { path: log, channel: "native_router", event_sha256: transition.event_sha256 }, { reason: transition.reason, nativeTimestamp: transition.native_timestamp, previousModel: transition.previous_model });
    }
    await this.save({ model_log_cursor: routing.cursor });
    for (const id of [this.rootId, ...this.topology.keys()]) {
      const native = await this.metadata.read(id);
      if (!native) {
        await observeModel({ journal: this.config.journal, harness: "droid-cli", sessionId: id, requested: this.config, source: "native.settings", evidence: "unavailable", reason: "settings_missing_or_partial" });
        continue;
      }
      this.validateMetadata(id, native);
      await this.observeSettings(id, native.settings, native.source);
    }
  }
  async handleObservationError(error) {
    if (["profile_integrity_violation", "session_identity_mismatch", "droid_child_identity_invalid", "droid_turn_identity_mismatch", "droid_hook_identity_invalid", "model_observation_storage_failed"].includes(error.code)) {
      if (!this.fatalError) {
        this.fatalError = error;
        try { await this.save({ fatal_error: errorRecord(error) }); await this.journal("integrity_failure", { error: errorRecord(error) }); }
        finally {
          this.rejectPending(error);
          // Never await cleanup from the native event drain: closeTree waits for it.
          setTimeout(() => { void this.closeTree(true).catch(cleanup => this.save({ cleanup_error: errorRecord(cleanup) }).catch(() => {})); }, 0);
        }
      }
      return;
    }
    try { await observeModel({ journal: this.config.journal, harness: "droid-cli", sessionId: this.rootId, requested: this.config, source: "native.observer", evidence: "unavailable", reason: error.code ?? "observation_unavailable" }); }
    catch (failure) { await this.handleObservationError(Object.assign(new Error(`Cannot persist model observation: ${failure.message}`), { code: "model_observation_storage_failed" })); }
  }
  ...
  async resume(id) {
    ...
    this.loaded = true; this.observedProfile = await this.observeSettings(id, result.settings, "native.load_session"); this.workingState = result.workingState ?? (result.isAgentLoopInProgress ? "unknown" : "idle"); this.cancelled = false; await this.save(); return await this.inspect(id);
  }
  ...
  async recoverOperation(operationId) {
    ...
    if (current?.operationId === operationId && current.turnId === binding.turn_id) {
      ...
      await this.saveBinding(current, { native_outcome: outcome });
  ...
  async refreshTopology() {
    ...
    for (const [id, tasks] of groups) {
      ...
      await this.observeSettings(id, native.settings, { path: native.transcript_path, sha256: native.source_sha256 });
  ...
  async observeHook(payload, eventId) {
    try { return await this.observeHookChecked(payload, eventId); }
    catch (error) { await this.handleObservationError(error); throw error; }
  }
  async observeHookChecked(payload, eventId) {
    ...
    do { native = ["PreToolUse", "PostToolUse"].includes(event) ? await this.metadata.read(id) : await readDroidSession(this.paths.factory, id); if (native) break; await sleep(25); } while (Date.now() < deadline);
    ...
    if (this.fatalError) return { continue: false, stopReason: this.fatalError.message };
    let profile;
    try { profile = await this.observeSettings(id, native.settings, { path: native.transcript_path, sha256: native.source_sha256 }); }
    catch (error) { await this.handleObservationError(error); return { continue: false, stopReason: error.message }; }
    if (!this.config.ddFlowBin || payload.tool_name !== "Execute") return {};
    await this.snapshotUsage(id);
  ...
  async closeTree(cancel = false) {
    ...
    this.cleanupPromise = this.closeOwnedTree(cancel);
    try { return await this.cleanupPromise; } finally { this.cleanupPromise = null; }
  }
  async closeOwnedTree(cancel) {
    await this.captureProcesses();
    if (cancel) cancelPendingDispatch(this);
    ...
    clearInterval(this.processObserver); clearInterval(this.modelObserver); await this.processObservation;
