...
export function executionState(events, runId, execution) {
  ...
  for (const event of events) {
    ...
    if (event.type === "dev.dd.eval.operation.suspended" && launchOperation && id === operationId && !terminal()) {
      result = { execution: execution.id, state: "awaiting_provider", code: data.error?.code, control: data.error?.details?.controller ?? null };
    }
...
export function executionCleanupRequired(events, runId, execution, operationId) {
  const state = executionState(events, runId, execution);
  return state.operation_id === operationId && state.cancellation?.settled !== true;
}
