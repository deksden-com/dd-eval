...
export async function commandJson(bin, args, { cwd, env = {}, input = null, onProgress = null } = {}) {
  return await new Promise((resolve, reject) => {
    ...
    const child = spawn(executable, [...args, "--json"], { cwd, env: { ...process.env, ...env }, stdio: [input === null ? "ignore" : "pipe", "pipe", "pipe"] });
    ...
    child.on("close", (status) => {
      if (status !== 0) return reject(failure(stderr.trim() || stdout.trim() || `${executable} exited ${status}`, "flow_reconciliation_failed"));
...
export async function commandText(bin, args, { cwd, env = {} } = {}) {
  return await new Promise((resolve, reject) => {
    const executable = bin === "dd-flow" && typeof env.DD_FLOW_BIN === "string" ? env.DD_FLOW_BIN : bin;
    const child = spawn(executable, args, { cwd, env: { ...process.env, ...env }, stdio: ["ignore", "pipe", "pipe"] }); let stdout = ""; let stderr = "";
