...
export async function callDaemon(stateDir, operation, params = {}, timeoutMs = 30_000) {
  ...
  return await new Promise((resolve, reject) => {
    ...
    const arm = () => { timer = setTimeout(async () => {
      ...
      try {
        const status = await callDaemon(stateDir, "daemon.status", {}, Math.min(timeoutMs, 5_000));
      ...
      finish(new DaemonError("daemon_timeout", `${operation} timed out without productive provider activity`, true));
    }, timeoutMs); };
...
export async function startDaemon(options) {
  ...
  const previous = await readState(paths.dir), config = { cwd: await realpath(absolute(options.cwd, "--cwd")), bin: options.bin ?? process.env.DD_AGY_BIN ?? "agy", provider: String(options.provider ?? "google"), model: String(options.model ?? AGY_BASELINE.model), reasoning: String(options.reasoning ?? "high"), mode: String(options.mode ?? "accept-edits"), projectRoot: await realpath(absolute(options.projectRoot, "--project-root")), ddFlowBin: absolute(options.ddFlowBin, "--dd-flow-bin"), ddFlowHome: absolute(options.ddFlowHome, "--dd-flow-home"), resourceHome: process.env.DD_FLOW_RESOURCE_HOME ? absolute(process.env.DD_FLOW_RESOURCE_HOME, "DD_FLOW_RESOURCE_HOME") : null, env: { DD_FLOW_HOME: absolute(options.ddFlowHome, "--dd-flow-home") }, entryPath: absolute(options.entryPath, "dd-agy entry"), daemonId: randomUUID(), geminiDir: paths.gemini, appDataDir: "runtime", temporary: paths.temporary };
  ...
  const registered = await runtimeProcess(config, "register", { kind: "agy-daemon", owner: `agy:${path.basename(paths.dir)}`, operation: `agy-daemon:${path.basename(paths.dir)}`, stdout: paths.log, stderr: paths.log, "lease-ms": 300_000 });
  ...
  try { if (resourceProcess && child.pid) await runtimeProcess(config, "confirm", { id: resourceProcess.id, "lease-token": resourceProcess.lease_token, pid: child.pid, "process-group-id": child.pid, "lease-ms": 300_000 }); else if (resourceProcess) await runtimeProcess(config, "finish", { id: resourceProcess.id, "lease-token": resourceProcess.lease_token, state: "failed", reason: "daemon_spawn_missing_pid" }); }
...
class Runtime {
  ...
  persist(patch = {}) { Object.assign(this.state, patch, { pid: process.pid, updated_at: new Date().toISOString(), last_activity_at: this.lastActivityAt, sessions: this.init ? [{ provider_session_id: this.init.conversation_id, root_provider_session_id: this.init.conversation_id, cwd: this.config.cwd }] : [] }); const snapshot = structuredClone(this.state); this.persisting = this.persisting.then(() => writeJson(this.paths.state, snapshot)); const resourceProcess = this.state.resource_process; if (resourceProcess?.id && resourceProcess.lease_token) this.persisting = this.persisting.then(async () => { try { await runtimeProcess(this.config, "heartbeat", { id: resourceProcess.id, "lease-token": resourceProcess.lease_token, "lease-ms": 300_000 }); } catch {} }); return this.persisting; }
  ...
  async finishResource(state = "stopped", reason = "daemon_stopped") { const resourceProcess = this.state.resource_process; if (!resourceProcess?.id || !resourceProcess.lease_token) return; try { await runtimeProcess(this.config, "finish", { id: resourceProcess.id, "lease-token": resourceProcess.lease_token, state, reason }); } catch {} }
}
