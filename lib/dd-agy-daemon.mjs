...
export async function startDaemon(options) {
  ...
  try { process.kill(-child.pid, "SIGTERM"); } catch { /* The child may already be gone. */ }
  if (resourceProcess) await runtimeProcess(config, "finish", { id: resourceProcess.id, "lease-token": resourceProcess.lease_token, state: "failed", reason: "daemon_start_timeout" }).catch(() => {});
