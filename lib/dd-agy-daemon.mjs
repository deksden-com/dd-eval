...
class Runtime {
  ...
  async start(sessionId = null) {
    ...
    this.child.on("close", (code, signal) => { const interrupted = Boolean(this.active); this.exited = true; const beforeInit = !this.init;
      const error = new DaemonError(beforeInit ? "agy_init_missing" : "agy_terminal_result_missing", `Antigravity exited ${beforeInit ? "before init" : "before result"} (${code ?? signal})`);
      ...
      void this.persist(patch).finally(() => { if (beforeInit) this.server?.close(); });
    });
...
export async function serveDaemon(stateDir) {
  ...
  try { await runtime.start(); }
    ...
    await runtime.persist({ shutdown_state: "failed", startup_error: errorPayload(error), active_tree: false });
