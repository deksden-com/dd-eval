...
import { AGY_BASELINE, AgyError, assertProfile, executable, observedProfile, runAgy, usageSnapshot } from "./dd-agy.mjs";
...
function locations(stateDir) { const dir = absolute(stateDir, "--state-dir"), local = path.join(dir, "daemon.sock"); return { dir, socket: Buffer.byteLength(local) < 100 ? local : `/tmp/dd-agy-${createHash("sha256").update(dir).digest("hex").slice(0, 22)}.sock`, state: path.join(dir, "daemon.json"), journal: path.join(dir, "events.jsonl"), log: path.join(dir, "daemon.log"), gemini: path.join(dir, "gemini"), runtime: path.join(dir, "gemini", "runtime"), config: path.join(dir, "gemini", "config"), temporary: path.join(dir, "tmp") }; }
...
export async function callDaemon(stateDir, operation, params = {}, timeoutMs = 30_000) {
  const paths = locations(stateDir), request = { schema_id: REQUEST_SCHEMA, id: randomUUID(), operation, params };
...
function sameConfig(state, config) { return ["cwd","bin","provider","model","reasoning","mode","projectRoot","ddFlowBin","ddFlowHome"].every(key => (state.config?.[key] ?? null) === (config[key] ?? null)); }
export async function startDaemon(options) {
  ...
  const previous = await readState(paths.dir), config = { cwd: await realpath(absolute(options.cwd, "--cwd")), bin: options.bin ?? process.env.DD_AGY_BIN ?? "agy", provider: String(options.provider ?? "google"), model: String(options.model ?? AGY_BASELINE.model), reasoning: String(options.reasoning ?? "high"), mode: String(options.mode ?? "accept-edits"), projectRoot: await realpath(absolute(options.projectRoot, "--project-root")), ddFlowBin: absolute(options.ddFlowBin, "--dd-flow-bin"), ddFlowHome: absolute(options.ddFlowHome, "--dd-flow-home"), entryPath: absolute(options.entryPath, "dd-agy entry"), daemonId: randomUUID(), geminiDir: paths.gemini, appDataDir: "runtime", temporary: paths.temporary };
  ...
  if (version !== AGY_BASELINE.version) throw new DaemonError("agy_version_unsupported", `unsupported Antigravity CLI version ${version}; expected ${AGY_BASELINE.version}`);
  await writeJson(paths.state, { schema_id: STATE_SCHEMA, daemon_id: config.daemonId, shutdown_state: "starting", active_tree: false, versions: { agy: version }, config, sessions: [] });
  ...
  const deadline = Date.now() + 30_000; while (Date.now() < deadline) { await new Promise(resolve => setTimeout(resolve, 100)); try { const status = await callDaemon(paths.dir, "daemon.status", {}, 1000); if (status.provider_ready) return status; } catch (error) { if (!["daemon_not_running","daemon_timeout"].includes(error.code)) throw error; } }
...
class Runtime {
  constructor(paths, state) { this.paths = paths; this.state = state; this.config = state.config; this.child = null; this.buffer = ""; this.init = null; this.readyPromise = null; this.active = null; this.exited = false; this.toolCalls = new Map(); this.descendants = new Map(); this.lastResult = null; this.lastStop = null; this.transcriptPath = null; }
  async persist(patch = {}) { Object.assign(this.state, patch, { pid: process.pid, sessions: this.init ? [{ provider_session_id: this.init.conversation_id, root_provider_session_id: this.init.conversation_id, cwd: this.config.cwd }] : [] }); await writeJson(this.paths.state, this.state); }
  ...
  args(sessionId) { return ["--input-format", "stream-json", "--output-format", "stream-json", "--model", this.config.model, "--effort", this.config.reasoning, "--mode", this.config.mode, "--dangerously-skip-permissions", "--print-timeout", "30m", `--gemini_dir=${this.paths.gemini}`, "--app_data_dir=runtime", ...(sessionId ? ["--conversation", sessionId] : ["--new-project"])]; }
  async start(sessionId = null) {
    ...
    this.child.stdout.setEncoding("utf8").on("data", chunk => { this.buffer += chunk; void this.drain(readyResolve).catch(error => { readyReject(error); if (this.active) { this.active.reject(error); this.active = null; } }); });
    ...
    this.child.on("close", (code, signal) => { const interrupted = Boolean(this.active); this.exited = true; if (!this.init) readyReject(new DaemonError("agy_init_missing", `Antigravity exited before init (${code ?? signal})`)); if (this.active) { this.active.reject(new DaemonError("agy_terminal_result_missing", `Antigravity exited before result (${code ?? signal})`)); this.active = null; } void this.persist({ provider_exit: { code, signal }, active_tree: interrupted }); });
  ...
  async finishTurn(result) { this.lastResult = result; const current = this.active; this.active = null; await forwardUsage(this.config, result.conversation_id ?? this.init.conversation_id, result, this.toolSnapshot(), [...this.descendants.values()]); await this.persist({ active_tree: result.status === "RUNNING" || this.lastStop?.fullyIdle === false }); if (current) current.resolve(this.receipt(result)); }
  receipt(result = this.lastResult) { return { harness: "antigravity-cli", runtime_family: "antigravity", provider_session_id: this.init?.conversation_id ?? null, adapter_session_id: this.init?.conversation_id ?? null, cwd: this.config.cwd, profile: this.profile, result, usage: usageSnapshot(result ?? {}, this.toolSnapshot()), descendants: [...this.descendants.values()], transcript_path: this.transcriptPath, settled: Boolean(result && result.status !== "RUNNING" && this.lastStop?.fullyIdle !== false) }; }
  async zeroUsage() { await forwardUsage(this.config, this.init.conversation_id, { usage: {} }, this.toolSnapshot(), []); }
  async prompt(text) { if (!text) throw new DaemonError("prompt_required", "prompt is required"); await this.start(); if (this.active) throw new DaemonError("operation_in_progress", "an Antigravity turn is already running"); this.lastStop = null; await this.persist({ active_tree: true }); return await new Promise((resolve, reject) => { this.active = { resolve, reject }; this.child.stdin.write(`${JSON.stringify({ event: "user", message: { content: text } })}\n`, error => { if (error) { this.active = null; reject(error); } }); }); }
  ...
  async cancel() { if (!this.child || this.exited) return this.receipt(); try { process.kill(-this.child.pid, "SIGINT"); } catch (error) { if (error.code !== "ESRCH") throw error; } const deadline = Date.now() + 5000; while (!this.exited && Date.now() < deadline) await new Promise(resolve => setTimeout(resolve, 100)); if (!this.exited) { try { process.kill(-this.child.pid, "SIGTERM"); } catch (error) { if (error.code !== "ESRCH") throw error; } } await this.persist({ active_tree: false }); return { ...this.receipt(), cancelled: true };
  }
  async close(cancelTree) { if (this.active && !cancelTree) throw new DaemonError("tree_not_settled", "Antigravity turn is still running"); if (cancelTree) await this.cancel(); else if (this.child && !this.exited) this.child.stdin.end(); const deadline = Date.now() + 5000; while (!this.exited && Date.now() < deadline) await new Promise(resolve => setTimeout(resolve, 100)); if (!this.exited) await this.cancel(); }
}
...
async function dispatch(runtime, request) {
  ...
  if (request.operation === "daemon.status") return { daemon_id: runtime.config.daemonId, shutdown_state: runtime.state.shutdown_state, active_tree: runtime.state.active_tree, provider_ready: Boolean(runtime.init && !runtime.exited), pid: process.pid, provider_pid: runtime.child?.pid ?? null, versions: runtime.state.versions, config: runtime.config, sessions: runtime.state.sessions ?? [], receipt: runtime.receipt() };
  if (request.operation === "session.create") { if (params.sessionId) throw new DaemonError("invalid_create", "create does not accept a Session ID"); await runtime.zeroUsage(); return await runtime.prompt(params.prompt); }
  ...
  if (request.operation === "daemon.stop") { await runtime.close(params.cancelTree === true); await runtime.persist({ shutdown_state: "clean", active_tree: false }); setImmediate(() => runtime.server?.close()); return { stopped: true, daemon_id: runtime.config.daemonId }; }
