...
export class Runtime {
  ...
  async start(sessionId = null) {
    ...
    this.child.on("close", (code, signal) => { void this.draining.then(() => { const interrupted = Boolean(this.active); this.exited = true; const beforeInit = !this.init;
      ...
      void providerRegistered.then(async () => {
        try {
        ...
        } catch (cleanup) { await this.persist({ shutdown_state: "cleanup_failed", active_tree: true, cleanup_error: errorPayload(cleanup) }); }
