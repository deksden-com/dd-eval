...
class Runtime {
  ...
  receipt(result = this.lastResult) {
    ...
    return { harness: "antigravity-cli", runtime_family: "antigravity", provider_session_id: this.init?.conversation_id ?? null, adapter_session_id: this.init?.conversation_id ?? null, cwd: this.config.cwd, profile: this.profile, result, ...(assistantText === null ? {} : { assistant_text: assistantText }), usage: usageSnapshot(result ?? {}, this.toolSnapshot()), descendants: [...this.descendants.values()], transcript_path: this.transcriptPath, settled: Boolean(result && result.status !== "RUNNING" && this.lastStop?.fullyIdle !== false) };
  }
