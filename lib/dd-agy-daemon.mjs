...
export class Runtime {
  ...
  async finishTurn(result) {
    ...
    await this.persist({ active_tree: result.status === "RUNNING" || rootStop?.fullyIdle === false, last_result: result });
  ...
  receipt(result = this.lastResult) {
    ...
    const treeSettled = [...this.descendants.values()].every(child => ["completed", "failed", "cancelled"].includes(child.status));
