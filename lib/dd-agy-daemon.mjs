...
function sameConfig(state, config) { return ["cwd","bin","provider","model","reasoning","mode","projectRoot","ddFlowBin","ddFlowHome","resourceHome"].every(key => (state.config?.[key] ?? null) === (config[key] ?? null)); }
...
class Runtime {
  ...
  persist(patch = {}) { Object.assign(this.state, patch, { pid: process.pid, updated_at: new Date().toISOString(), last_activity_at: this.lastActivityAt, sessions: this.init ? [{ provider_session_id: this.init.conversation_id, root_provider_session_id: this.init.conversation_id, cwd: this.config.cwd }] : [] }); const snapshot = structuredClone(this.state); this.persisting = this.persisting.then(() => writeJson(this.paths.state, snapshot)); const resourceProcess = this.state.resource_process; if (resourceProcess?.id && resourceProcess.lease_token) this.persisting = this.persisting.then(async () => { try { await heartbeatDaemonProcess(this.config, resourceProcess); } catch {} }); return this.persisting; }
