...
export async function callDaemon(stateDir, operation, params = {}, timeoutMs = 30_000) {
  ...
  return await new Promise((resolve, reject) => {
    const client = net.createConnection(paths.socket); let buffer = "", settled = false, timer, lastActivity = Date.now();
    const finish = (error, result) => { if (settled) return; settled = true; if (timer) clearTimeout(timer); client.destroy(); error ? reject(error) : resolve(result); };
    const arm = () => { timer = observedTimeout(async () => {
    ...
    }, timeoutMs, { onGap: gap => { void appendFile(paths.journal, `${JSON.stringify({ kind: "observation_gap", payload: { ...gap, operation_id: request.id } })}\n`, { mode: 0o600 }).catch(() => {}); } }); };
