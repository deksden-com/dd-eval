...
export async function prepare(paths, config) {
  ...
  // Antigravity's process-level configuration is not inherited by native
  // subagents.  Put the same handlers in the isolated workspace discovery
  // root before the first Session is created, so direct children receive the
  // lifecycle contract too.  Workspace hooks are event-keyed (unlike the
  // named global-config wrapper above), so replace only a prior handler that
  // points at this daemon and preserve every unrelated project hook.
  ...
  for (const [event, handlers] of Object.entries(hooks["dd-flow"])) {
    const retained = Array.isArray(existing[event]) ? existing[event].filter(item => Array.isArray(item?.hooks)
      ? !item.hooks.some(isManagedHandler)
      : !isManagedHandler(item)) : [];
    workspace[event] = [...retained, ...handlers];
  }
