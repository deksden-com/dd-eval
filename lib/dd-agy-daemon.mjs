...
export class Runtime {
  ...
  async prompt(text, assertDispatch = captureDispatchGuard(this)) {
    ...
    if (this.lastResult && !this.receipt().settled) throw new DaemonError("tree_not_settled", "the prior Antigravity agent tree is not fully settled");
  ...
  childActivity(sessionId) {
    ...
    if (!child) return;
    this.descendants.set(sessionId, { ...child, previous_terminal_status: child.status === "unknown" ? child.previous_terminal_status : child.status, status: "unknown", tree_settled: false, settlement_evidence: null, activity_generation: this.turnGeneration });
  }
