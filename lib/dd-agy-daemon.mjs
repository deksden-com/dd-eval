...
export async function startDaemon(options) {
  ...
  const resumedObservations = config.resumeSessionId && previous?.sessions?.some(session => session.provider_session_id === config.resumeSessionId) ? previous.session_observations ?? [] : [];
  await writeJson(paths.state, { schema_id: STATE_SCHEMA, daemon_id: config.daemonId, shutdown_state: "starting", active_tree: false, versions: { agy: version }, config, sessions: [], session_observations: resumedObservations, resource_process: resourceProcess });
...
export class Runtime {
  ...
  async finishTurn(result) {
    if (result.conversation_id && result.conversation_id !== this.init?.conversation_id) {
      ...
      if (child) this.descendants.set(result.conversation_id, { ...child, status: result.status === "SUCCESS" ? "completed" : result.status === "ERROR" ? "failed" : "unknown" });
      return;
    }
    ...
    await this.persist({ active_tree: result.status === "RUNNING" || rootStop?.fullyIdle === false, last_result: { status: result.status ?? null, error: result.error ?? null } });
    ...
    if (result.status === "ERROR") {
      ...
      const code = /quota|insufficient.?credit|billing|usage limit/.test(normalized) ? "agy_provider_quota_exhausted"
        : /rate.?limit|too many requests|\b429\b/.test(normalized) ? "agy_provider_rate_limited"
          : /auth|unauthori[sz]ed|forbidden|account/.test(normalized) ? "agy_provider_account"
            : "agy_provider_failed";
      return current.reject(new DaemonError(code, message, ["agy_provider_quota_exhausted", "agy_provider_rate_limited"].includes(code), { provider_result: result, usage_ingest: this.usageIngest }));
    }
  ...
  async observeHook(event, payload) { const conversationId = payload.conversationId ?? this.init?.conversation_id; const child = await this.childForHook(conversationId); const prior = this.sessionObservations.get(conversationId) ?? {}; const execution = Number.isInteger(payload.executionNum) ? payload.executionNum : null; const floor = prior.execution_floor; const oldExecution = prior.stop?.executionNum; const stale = event === "Stop" && execution !== null && ((Number.isInteger(floor) && execution <= floor) || (Number.isInteger(oldExecution) && execution < oldExecution)); const fingerprint = createHash("sha256").update(JSON.stringify({ event, payload })).digest("hex"); if (!stale && prior.last_hook !== fingerprint) this.markActivity(); if (conversationId && !stale) this.sessionObservations.set(conversationId, { ...prior, last_hook: fingerprint, ...(payload.transcriptPath ? { transcript_path: payload.transcriptPath } : {}), ...(event === "Stop" ? { stop: { fullyIdle: payload.fullyIdle === true, terminationReason: payload.terminationReason ?? null, executionNum: payload.executionNum ?? null } } : {}) }); if (child && !stale && event === "Stop" && payload.fullyIdle === true) this.descendants.set(conversationId, { ...child, status: "completed" }); const parent = child?.parent_provider_session_id ?? null;
    ...
    const eventId = createHash("sha256").update(JSON.stringify({ daemon: this.config.daemonId, event, conversationId, execution: payload.executionNum ?? null, step: payload.stepIdx ?? null, tool: payload.toolCall?.name ?? null, args: payload.toolCall?.args ?? null })).digest("hex"); await this.persist(); await this.journal("hook", { stale, hook_event: event, event_id: eventId, conversation_id: conversationId, fully_idle: payload.fullyIdle ?? null }); return { event_id: eventId, daemon_id: this.config.daemonId, parent_conversation_id: parent, profile: observed, requested_profile: { provider: this.config.provider, model: this.config.model, reasoning: this.config.reasoning, mode: this.config.mode }, project_root: this.config.projectRoot, dd_flow_bin: this.config.ddFlowBin, dd_flow_home: this.config.ddFlowHome };
  }
