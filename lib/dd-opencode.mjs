...
export class OpenCodeClient {
  ...
  async prompt(id, { prompt, provider, model, variant, agent, onModelMessages, onObservationError, assertDispatch }) {
    const previous = await this.messages(id);
    if (!Array.isArray(previous)) throw new OpenCodeError("operation_observation_lost", "OpenCode history cannot establish the prompt boundary", true);
    const previousIds = new Set(previous.map(message => message.info?.id).filter(Boolean));
    let marker, observation = null;
    ...
    const observe = async (session, seen = new Set(), parent = null) => {
      ...
      await onModelMessages?.(session, messages, parent);
      return [session, messages, ...await Promise.all((children ?? []).map(child => observe(child.id, seen, session)))];
    };
    const refreshObservation = () => observation ??= (async () => {
      try { marker = createHash("sha256").update(JSON.stringify(await observe(id))).digest("hex"); }
      catch (error) { await onObservationError?.(error); /* Missing data is not progress. */ }
    })().finally(() => { observation = null; });
    // Begin observing before the long-running request.  Waiting for the first
    // interval can let a short liveness window expire before any native
    // content has had a chance to renew it.
    void refreshObservation();
    const timer = setInterval(() => { void refreshObservation(); }, Math.max(10, Math.min(5_000, this.timeoutMs / 4)));
    try {
      assertDispatch?.();
      const reply = await this.request("POST", `/session/${encodeURIComponent(id)}/message`, { model: { providerID: provider, modelID: model }, variant, agent, parts: [{ type: "text", text: prompt }] }, { progress: () => marker });
      if (!reply?.info?.id || reply.info.sessionID !== id || reply.info.role !== "assistant" || previousIds.has(reply.info.id) || !Array.isArray(reply.parts)) {
        throw new OpenCodeError("operation_observation_lost", "OpenCode did not return an identifiable current assistant response", true, { session_id: id, message_id: reply?.info?.id ?? null });
      }
      if (reply.info.error) {
        const native = reply.info.error;
        const message = native.data?.message ?? native.message ?? native.name ?? "OpenCode provider failed";
        const status = native.data?.statusCode;
        const code = status === 429 || /rate limit/i.test(message) ? "provider_rate_limited" : status === 402 || /payment required|quota|usage balance exhausted/i.test(message) ? "provider_quota_exhausted" : "opencode_provider_failed";
        throw new OpenCodeError(code, message, native.data?.isRetryable === true || code === "provider_rate_limited", { session_id: id, message_id: reply.info.id, provider_error: native });
      }
      return reply;
    }
    finally { clearInterval(timer); await observation; }
  }
