...
export class OpenCodeClient {
  ...
  async request(method, pathname, body, { directory = this.directory, timeoutMs = this.timeoutMs } = {}) {
    ...
    try {
      ...
      throw new OpenCodeError(error.name === "AbortError" ? "opencode_timeout" : "opencode_unavailable", error.message, true);
  ...
  prompt(id, { prompt, provider, model, variant, agent }) { return this.request("POST", `/session/${encodeURIComponent(id)}/message`, { model: { providerID: provider, modelID: model }, variant, agent, parts: [{ type: "text", text: prompt }] }); }
