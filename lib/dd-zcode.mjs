...
export class AcpBridge {
  constructor(options) {
    ...
    this.modelSessions = new Map(); this.modelProfiles = new Map();
  }

  configure(options) { this.options = { ...this.options, ...options }; }
  registerModelSession(adapterId, sessionId, requested, parentSessionId = null, armed = true) {
    this.modelSessions.set(adapterId, { sessionId, requested, parentSessionId, armed });
  }
  async recordModel(adapterId, observed, source) {
    const identity = this.modelSessions.get(adapterId); if (!identity) return;
    this.modelProfiles.set(identity.sessionId, observed);
    await observeModel({ journal: this.options.journal, harness: this.options.clientInfo?.name === "dd-grok" ? "grok-acp" : "zcode-acp",
      sessionId: identity.sessionId, parentSessionId: identity.parentSessionId, requested: identity.requested, observed, source });
  }
  async observeModelUpdate(message) {
    const update = message.params?.update;
    if (update?.sessionUpdate !== "config_option_update" || !Array.isArray(update.configOptions)) return;
    const value = category => update.configOptions.find(option => option.type === "select" && (option.category === category || option.id === category))?.currentValue ?? null;
    const observed = { model: value("model"), reasoning: value("thought_level"), mode: value("mode") };
    const identity = this.modelSessions.get(message.params.sessionId); if (!identity?.armed) return;
    checkObservedProfile(identity.requested, observed);
    await this.recordModel(message.params.sessionId, observed, "acp.config_option_update");
  }
  ...
  receive(line) {
    ...
    if (typeof message.params?.sessionId === "string" && update?.sessionUpdate === "retry_state" && update.type === "failed") {
      for (const pending of this.pending.values()) {
        if (pending.method === "session/prompt" && pending.sessionId === message.params.sessionId) pending.providerError = update;
      }
    }
    ...
    if (message.id !== undefined) {
      ...
      if (message.error) {
        ...
        const provider = pending.providerError;
    ...
    if (message.method) this.notifications = this.notifications
      .then(async () => { await this.observeModelUpdate(message); await this.options.onNotification?.(message); })
      .catch((error) => { this.notificationError ??= error; if (error.code === "profile_integrity_violation") this.rejectAll(error); });
  }
  ...
  observeToolCall(message) {
    ...
    const id = JSON.stringify([message.params?.sessionId ?? null, update.toolCallId]);
  ...
  toolSummary(sessionId) {
    const by_tool = {};
    let total = 0; let failures = 0;
    for (const [id, name] of this.toolCalls) {
      if (sessionId !== undefined && JSON.parse(id)[0] !== sessionId) continue;
  ...
  request(method, params = {}, timeoutMs = 30_000, { idleSessionId = null, idleTimeoutMs = null } = {}) {
    if (this.notificationError?.code === "profile_integrity_violation" && ["session/new", "session/prompt", "session/fork"].includes(method)) return Promise.reject(this.notificationError);
    ...
    return new Promise((resolve, reject) => {
      ...
      this.pending.set(id, { resolve, reject, timer, idleTimer, method, sessionId: params.sessionId ?? null });
...
export function assertProfile(requested, observed) { return checkObservedProfile(requested, observed); }
...
async function inspect(bridge, sessionId) {
  ...
  return { read, subagents, usage, observed_profile: observed };
}
...
async function cancelTree(bridge, sessionId, before) {
  ...
  for (const child of before.running ?? []) {
    ...
    if (taskId) {
      try { cancellations.push(await bridge.request("session/cancelBackgroundTask", { sessionId, taskId })); }
...
async function controlledIdentity(bridge, options) {
  ...
  bridge.registerModelSession?.(adapterSessionId, providerSessionId, profile(options));
  return { providerSessionId, adapterSessionId };
}

async function applyProfile(bridge, sessionId, requested, assertDispatch = () => {}) {
  assertDispatch();
  ...
  const observed = observedProfile(read);
  await bridge.recordModel?.(sessionId, observed, "zcode/session/read.applied");
  const receipt = assertProfile(requested, observed);
  const identity = bridge.modelSessions?.get(sessionId); if (identity) identity.armed = true;
  return receipt;
}
...
function flowForwarder(options, rootProviderSessionId, bridge) {
  ...
  return async (notification) => {
    ...
    const childId = notification.params?.update?._meta?.zcodeRuntime?.childSessionId;
    const observed = bridge?.modelProfiles?.get(childId ?? rootProviderSessionId) ?? {};
    if (childId && !observed.model) await observeModel({ journal: options.journal, harness: "zcode-acp", sessionId: childId, parentSessionId: rootProviderSessionId, requested: profile(options), observed, source: "zcode.child_tool_event", evidence: "unavailable" });
    const payload = zcodeLifecycleEnvelope(notification, { ...options, observedProfile: observed }, rootProviderSessionId);
...
export function zcodeLifecycleEnvelope(notification, options, rootProviderSessionId) {
  return { ...notification, _meta: { ddZcode: {
    ...
    requestedProfile: profile(options),
    observedProfile: observedFields(options.observedProfile),
...
export async function createSessionWithBridge(bridge, options, initialized) {
  ...
  bridge.registerModelSession?.(created.sessionId, providerSessionId, requested, null, false);
  ...
  await forwardUsage(options, providerSessionId, usage, bridge.toolSummary(created.sessionId));
  await bridge.flush();
  ...
  const evidence = { read: null, subagents: { running: [], completed: [] }, usage, observed_profile: profile_receipt.observed, tool_calls: bridge.toolSummary(created.sessionId) };
...
export async function promptSessionWithBridge(bridge, options) {
  ...
  let forward;
  bridge.configure({ ...options, onNotification: async (event) => { await forward?.(event); await options.onNotification?.(event); } });
  const identity = await controlledIdentity(bridge, { ...options, cwd }); forward = flowForwarder(options, identity.providerSessionId, bridge);
  ...
  await forwardUsage(options, identity.providerSessionId, await bridge.request("zcode/session/usage", { sessionId: identity.adapterSessionId }), bridge.toolSummary(identity.adapterSessionId));
  const timeoutMs = options.timeoutMs ?? (options.livenessTimeoutMs ? null : 1_800_000);
  const textCursor = bridge.assistantTextCursor(identity.adapterSessionId);
  options.assertDispatch?.();
  ...
  if (stopReason && stopReason !== "end_turn") {
    ...
    error.details = { stop_reason: stopReason, provider_session_id: identity.providerSessionId, adapter_session_id: identity.adapterSessionId, evidence: publicEvidence(evidence) };
    throw error;
  }
