...
export class AcpBridge {
  ...
  receive(line) {
    ...
    if (message.id !== undefined) {
      ...
      if (message.error) {
        const details = message.error.data?.details ?? null;
        const messageText = [message.error.message, details].filter(Boolean).join(": ") || JSON.stringify(message.error);
        const error = new Error(messageText);
        error.code = /rate limit/i.test(messageText) ? "provider_rate_limited" : "acp_request_failed";
        error.retryable = /rate limit/i.test(messageText);
        error.details = { acp_code: message.error.code ?? null, ...(details ? { provider_details: details } : {}) };
...
export async function promptSessionWithBridge(bridge, options) {
  ...
  const turn = await bridge.request("session/prompt", { sessionId: identity.adapterSessionId, prompt: [{ type: "text", text: prompt }] }, timeoutMs, { idleSessionId: identity.adapterSessionId, idleTimeoutMs: options.livenessTimeoutMs ?? null });
  await bridge.flush();
...
export async function promptProbeBatchWithBridge(bridge, options) {
  ...
  // The probe deadline covers the whole batch, not merely the last agent
  // Turn.  Reserve a small, deterministic tail for reading the completed
  // Sessions and serializing the daemon response.  Without it a client and
  // the last Turn can expire in the same millisecond, turning real capacity
  // into a false zero-capacity result.
  const timeout = options.timeoutMs ?? 180_000;
  const observationReserve = Math.min(15_000, Math.max(5_000, Math.floor(timeout / 12)));
  const turnTimeout = Math.max(1_000, timeout - observationReserve);
  const turns = await Promise.allSettled(probes.map(async (probe) => ({
    ...probe,
    turn: await bridge.request("session/prompt", { sessionId: probe.adapter_session_id, prompt: [{ type: "text", text: probe.prompt }] }, turnTimeout, { idleSessionId: probe.adapter_session_id, idleTimeoutMs: options.livenessTimeoutMs ?? null }),
  })));
  await bridge.flush();
  const results = await Promise.all(turns.map(async (settled, index) => {
    if (settled.status === "rejected") {
      return { provider_session_id: probes[index].provider_session_id, adapter_session_id: probes[index].adapter_session_id, status: "failed", error: settled.reason instanceof Error ? settled.reason.message : String(settled.reason) };
    }
    const probe = settled.value;
    try {
      const read = await bridge.request("zcode/session/read", { sessionId: probe.adapter_session_id }, observationReserve);
      return {
      ...
      return { provider_session_id: probe.provider_session_id, adapter_session_id: probe.adapter_session_id, turn: probe.turn, status: "failed", error: error instanceof Error ? error.message : String(error) };
    }
  }));
