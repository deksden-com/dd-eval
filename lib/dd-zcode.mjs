...
export const ZCODE_BASELINE = Object.freeze({ zcode: "0.16.5", zcode_acp: "0.13.0", dd_harness: "dd-zcode-harness@1" });
...
export async function createSessionWithBridge(bridge, options, initialized) {
  ...
  await forwardUsage(options, providerSessionId, await bridge.request("zcode/session/usage", { sessionId: created.sessionId }), bridge.toolSummary());
  ...
  evidence.tool_calls = bridge.toolSummary();
  await forwardUsage(options, providerSessionId, evidence.usage, bridge.toolSummary());
...
export async function promptSessionWithBridge(bridge, options) {
  ...
  await forwardUsage(options, identity.providerSessionId, await bridge.request("zcode/session/usage", { sessionId: identity.adapterSessionId }), bridge.toolSummary());
  ...
  evidence.tool_calls = bridge.toolSummary();
  await forwardUsage(options, identity.providerSessionId, evidence.usage, bridge.toolSummary());
