...
async function cancelTree(bridge, sessionId, before) {
  ...
  for (let attempt = 0; attempt < 50; attempt += 1) {
    ...
    if (!(after.running ?? []).length) return { cancellations, before, after };
  }
  fail(`ZCode session tree did not settle after cancellation: ${JSON.stringify(cancellations)}`);
}
...
async function controlledIdentity(bridge, options) {
  ...
  await resume(bridge, adapterSessionId, options.cwd);
  const resolved = await bridge.request("zcode/session/resolve", { sessionId: adapterSessionId });
  if (resolved.providerSessionId !== providerSessionId) fail(`ZCode Session identity mismatch: expected ${providerSessionId}, observed ${resolved.providerSessionId}`);
...
export async function createSession(options) {
  ...
  let forward;
  return await withBridge({ ...options, onNotification: async (event) => { await forward?.(event); await options.onNotification?.(event); } }, async (bridge, initialized) => {
    ...
    return { harness: "zcode-acp", runtime_family: "zcode", adapter_session_id: created.sessionId, provider_session_id: providerSessionId, cwd, initialized, turn, profile: profile_receipt, evidence };
  });
}
...
export async function promptSession(options) {
  ...
  return await withBridge({ ...options, onNotification: async (event) => { await forward?.(event); await options.onNotification?.(event); } }, async (bridge) => {
    ...
    if ((evidence.subagents.running ?? []).length) {
    ...
    return { harness: "zcode-acp", provider_session_id: identity.providerSessionId, adapter_session_id: identity.adapterSessionId, turn, profile: profile_receipt, evidence };
  });
}

export async function inspectSession(options) {
  requireJournal(options);
  return await withBridge(options, async (bridge) => { const identity = await controlledIdentity(bridge, options); return { harness: "zcode-acp", provider_session_id: identity.providerSessionId, adapter_session_id: identity.adapterSessionId, ...await inspect(bridge, identity.adapterSessionId) }; });
}
...
export async function cancelSession(options) {
  ...
  return await withBridge(options, async (bridge) => {
    ...
    return { harness: "zcode-acp", provider_session_id: identity.providerSessionId, adapter_session_id: identity.adapterSessionId, ...await cancelTree(bridge, identity.adapterSessionId, before) };
  });
}
...
export async function forkSession(options) {
  ...
  return await withBridge(options, async (bridge) => {
    ...
    return { harness: "zcode-acp", parent_provider_session_id: identity.providerSessionId, parent_adapter_session_id: identity.adapterSessionId, provider_session_id: result.forkedSessionId, adapter_session_id: result.forkedSessionId, target: options.target };
  });
}
