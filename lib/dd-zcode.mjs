...
class AcpBridge {
  ...
  async start() {
    ...
    this.child = spawn(target.command, target.args, { stdio: ["pipe", "pipe", "pipe"], env: process.env });
    this.child.stderr.setEncoding("utf8").on("data", (chunk) => this.journal.write("bridge_stderr", { text: String(chunk) }));
    this.child.on("error", (error) => this.rejectAll(error));
    this.child.on("exit", (code) => this.rejectAll(new Error(`zcode-acp exited ${code}`)));
...
export function assertProfile(requested, observed) {
  ...
  if (mismatches.length) fail(`observed ZCode profile mismatch: ${JSON.stringify(mismatches)}`);
...
function flowForwarder(options, rootProviderSessionId) {
  ...
  return async (notification) => {
    ...
    const payload = { ...notification, _meta: { ddZcode: { rootProviderSessionId } } };
...
export async function createSession(options) {
  ...
  return await withBridge(options, async (bridge, initialized) => {
    ...
    const profile_receipt = await applyProfile(bridge, providerSessionId, requested);
    const evidence = await inspect(bridge, providerSessionId);
    return { harness: "zcode-acp", runtime_family: "zcode", adapter_session_id: created.sessionId, provider_session_id: providerSessionId, cwd, initialized, profile: profile_receipt, evidence };
  });
}

export async function promptSession(options) {
  requireJournal(options);
  const cwd = absolute(options.cwd, "--cwd"); const sessionId = text(options.sessionId, "--session-id"); const prompt = text(options.prompt, "prompt"); const requested = requiredProfile(options);
  let forward;
  return await withBridge({ ...options, onNotification: async (event) => { await forward?.(event); await options.onNotification?.(event); } }, async (bridge) => {
    await resume(bridge, sessionId, cwd); forward = flowForwarder(options, sessionId);
    const profile_receipt = await applyProfile(bridge, sessionId, requested);
    const turn = await bridge.request("session/prompt", { sessionId, prompt: [{ type: "text", text: prompt }] }, options.timeoutMs ?? 1_800_000);
    await bridge.flush();
    const evidence = await inspect(bridge, sessionId);
    return { harness: "zcode-acp", provider_session_id: sessionId, turn, profile: profile_receipt, evidence };
  });
}
...
export async function inspectSession(options) {
  ...
  return await withBridge(options, async (bridge) => { await resume(bridge, sessionId, options.cwd); return { harness: "zcode-acp", provider_session_id: sessionId, ...await inspect(bridge, sessionId) }; });
}

export async function cancelSession(options) {
  requireJournal(options);
  const sessionId = text(options.sessionId, "--session-id");
  return await withBridge(options, async (bridge) => {
    await resume(bridge, sessionId, options.cwd);
    const before = await bridge.request("zcode/session/subagents", { sessionId });
    const cancellations = [];
    ...
    const after = await bridge.request("zcode/session/subagents", { sessionId });
    if ((after.running ?? []).length) fail("ZCode session tree did not settle after cancellation");
    return { harness: "zcode-acp", provider_session_id: sessionId, cancellations, before, after };
  });
}
...
export async function forkSession(options) {
  ...
  return await withBridge(options, async (bridge) => {
    await resume(bridge, sessionId, options.cwd);
    const topology = await bridge.request("zcode/session/subagents", { sessionId });
    if ((topology.running ?? []).length) fail("cannot fork while background children are running");
    const result = await bridge.request("session/fork", { sessionId, target: options.target });
    if (!result.forkedSessionId) fail("ZCode fork returned no provider Session ID");
    return { harness: "zcode-acp", parent_provider_session_id: sessionId, provider_session_id: result.forkedSessionId, target: options.target };
  });
}
