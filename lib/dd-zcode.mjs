...
export async function promptProbeBatchWithBridge(bridge, options) {
  ...
  const turns = await Promise.allSettled(probes.map(async (probe) => ({
    ...probe,
    turn: await bridge.request("session/prompt", { sessionId: probe.adapter_session_id, prompt: [{ type: "text", text: probe.prompt }] }, timeout, { idleSessionId: probe.adapter_session_id, idleTimeoutMs: options.livenessTimeoutMs ?? null }),
  })));
  await bridge.flush();
  const results = [];
  for (const [index, settled] of turns.entries()) {
    if (settled.status === "rejected") {
      results.push({ provider_session_id: probes[index].provider_session_id, adapter_session_id: probes[index].adapter_session_id, status: "failed", error: settled.reason instanceof Error ? settled.reason.message : String(settled.reason) });
      continue;
    }
