  ...
  request(method, params = {}, timeoutMs = 30_000, { idleSessionId = null, idleTimeoutMs = null } = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkIdle = async () => {
        ...
        // native progress.  That is an unknown outcome, never proof that the
        // model stalled.  Let durable reconciliation inspect the same Turn.
        if (observationGapMs > Math.max(60_000, validIdleTimeout)) {
          const error = new Error(`${method} observation was interrupted for ${observationGapMs}ms`);
          error.code = "operation_observation_lost";
          error.details = { session_id: idleSessionId, observation_gap_ms: observationGapMs, observed_at: new Date(observedAt).toISOString() };
          finish(error);
          return;
        }
        const lastActivity = this.sessionActivity.get(idleSessionId) ?? Date.now();
        if (observedAt - lastActivity < validIdleTimeout) return;
