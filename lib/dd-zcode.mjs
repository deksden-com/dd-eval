...
export async function createSessionWithBridge(bridge, options, initialized) {
  ...
  await forwardUsage(options, providerSessionId, await bridge.request("zcode/session/usage", { sessionId: created.sessionId }), bridge.toolSummarySince(toolCursor));
  const turn = await bridge.request("session/prompt", { sessionId: created.sessionId, prompt: [{ type: "text", text: prompt }] }, options.timeoutMs ?? 1_800_000, { idleSessionId: created.sessionId, idleTimeoutMs: options.livenessTimeoutMs ?? 180_000 });
  await bridge.flush();
  const evidence = await inspect(bridge, created.sessionId);
  evidence.tool_calls = bridge.toolSummarySince(toolCursor);
  await forwardUsage(options, providerSessionId, evidence.usage, evidence.tool_calls);
  return receipt(options, { harness: "zcode-acp", runtime_family: "zcode", adapter_session_id: created.sessionId, provider_session_id: providerSessionId, cwd, initialized, turn, profile: profile_receipt, evidence: publicEvidence(evidence) });
}
