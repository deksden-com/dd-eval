...
export async function createSessionWithBridge(bridge, options, initialized) {
  ...
  await forwardUsage(options, providerSessionId, usage, bridge.toolSummary());
  await bridge.flush();
  ...
  const evidence = { read: null, subagents: { running: [], completed: [] }, usage, observed_profile: profile_receipt.observed, tool_calls: bridge.toolSummary() };
