...
export async function promptSessionWithBridge(bridge, options) {
  ...
  const turn = await bridge.request("session/prompt", { sessionId: identity.adapterSessionId, prompt: [{ type: "text", text: prompt }] }, options.timeoutMs ?? 1_800_000, { idleSessionId: identity.adapterSessionId, idleTimeoutMs: options.livenessTimeoutMs ?? null });
  await bridge.flush();
