  ...
  const dispatch = async (operation, params) => {
    ...
    if (operation === "daemon.stop") {
      ...
      for (const sessionId of sessions) {
        let observed = await inspectSessionWithBridge(bridge, { ...state.config, sessionId });
        const isIdle = observed.status?.type === "idle" || observed.status === "idle";
        if (!isIdle && params.cancelTree) {
          await cancelSessionWithBridge(bridge, { ...state.config, sessionId });
          observed = await inspectSessionWithBridge(bridge, { ...state.config, sessionId });
        }
        if (!(observed.status?.type === "idle" || observed.status === "idle")) unsettled.push(sessionId);
      }
      if (active || unsettled.length) throw Object.assign(new Error("daemon still owns running Sessions"), { code: "tree_not_settled", details: { sessions: unsettled, active_operation: active } });
