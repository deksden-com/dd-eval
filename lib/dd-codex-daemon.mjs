...
export async function serveDaemon(stateDir) {
  ...
  const dispatch = async (operation, params) => {
    ...
    if (operation === "session.prompt") { sessions.add(params.sessionId); active = operation; await save({ active_operation: active }); try { return await promptSessionWithBridge(bridge, { ...state.config, ...params }); } finally { active = null; await save({ active_operation: null }); } }
    if (operation === "session.start") { sessions.add(params.sessionId); active = operation; await save({ active_operation: active }); try { return await startSessionWithBridge(bridge, { ...state.config, ...params }); } finally { active = null; await save({ active_operation: null }); } }
    if (operation === "session.inspect") return await inspectSessionWithBridge(bridge, { ...state.config, ...params });
    if (operation === "session.cancel") { if (!sessions.has(params.sessionId)) throw Object.assign(new Error("cancellation Session is not owned by this daemon"), { code: "session_identity_mismatch" }); return await cancelSessionWithBridge(bridge, { ...state.config, ...params }); }
