...
const errorPayload = errorRecord;
const terminalTurn = turn => ["completed", "interrupted", "failed", "cancelled"].includes(turn?.status);
export function settledCodexSession(status, latestTurn, active) {
  if (status === "idle") return true;
  if (status === "notLoaded") return !latestTurn || terminalTurn(latestTurn);
  // `systemError` alone is not evidence that the provider stopped. A current
  // terminal Turn plus no daemon operation is the narrow settled case.
  return status === "systemError" && !active && terminalTurn(latestTurn);
}

export async function callDaemon(stateDir, operation, params = {}, timeoutMs = 30_000) {
  const target = locations(stateDir); const request = { schema_id: requestSchema, id: process.env.DD_EVAL_OPERATION_ID ?? randomUUID(), operation, params };
  return await new Promise((resolve, reject) => {
    const client = net.createConnection(target.socket); let buffer = ""; let settled = false; const timer = operation === "session.prompt" ? null : observedTimeout(() => finish(Object.assign(new Error(`${operation} timed out`), { code: "daemon_timeout", retryable: true })), timeoutMs);
...
export async function serveDaemon(stateDir) {
  ...
  const bridge = new CodexBridge({ ...state.config, env: { ...state.config.env, DD_FLOW_DAEMON_ID: state.daemon_id } }); await bridge.start(); const sessions = new Set(state.sessions ?? []); let active = null; let server; let persist = Promise.resolve();
  const save = (patch = {}) => { Object.assign(state, patch, { pid: process.pid, updated_at: new Date().toISOString(), sessions: [...sessions] }); const snapshot = structuredClone(state); persist = persist.then(() => writeState(paths.state, snapshot)).then(() => heartbeatDaemonProcess(snapshot.config, snapshot.resource_process).catch(() => {})); return persist; };
  const dispatch = async (operation, params) => {
    const assertDispatch = captureDispatchGuard(bridge);
    ...
    if (operation === "daemon.stop") {
      if (params.cancelTree) cancelPendingDispatch(bridge);
      if (active && !params.cancelTree) throw Object.assign(new Error("productive operation is running"), { code: "tree_not_settled" });
      await waitForSettlement({
        knownSessions: () => [...sessions],
        observe: async () => {
          const unsettled = [];
          // Set iteration also visits descendants discovered during this pass.
          for (const sessionId of sessions) {
            const observed = await inspectSessionWithBridge(bridge, { ...state.config, sessionId });
            for (const child of observed.descendants ?? []) sessions.add(child.provider_session_id);
            const status = observed.status?.type ?? observed.status;
            const stored = ["notLoaded", "systemError"].includes(status) ? await bridge.request("thread/turns/list", { threadId: sessionId, limit: 1, sortDirection: "desc", itemsView: "summary" }, 5_000) : null;
            if (!settledCodexSession(status, stored?.data?.[0] ?? null, active)) unsettled.push(sessionId);
          }
          return { sessions: unsettled.reverse(), active: Boolean(active), active_operation: active };
        },
        ...(params.cancelTree ? { cancel: async sessionId => { try { await cancelSessionWithBridge(bridge, { ...state.config, sessionId }); } catch (error) { if (error.code !== "turn_not_active") throw error; } } } : {})
      });
    ...
    if (operation === "session.create") { active = operation; try { await save({ active_operation: active }); const result = await createSessionWithBridge(bridge, { ...state.config, ...params }); sessions.add(result.provider_session_id); return result; } finally { active = null; await save({ active_operation: null }); } }
    if (operation === "session.prompt") { sessions.add(params.sessionId); active = operation; try { await save({ active_operation: active }); return await promptSessionWithBridge(bridge, { ...state.config, ...params, assertDispatch }); } finally { active = null; await save({ active_operation: null }); } }
    if (operation === "session.start") { sessions.add(params.sessionId); active = operation; try { await save({ active_operation: active }); return await startSessionWithBridge(bridge, { ...state.config, ...params, assertDispatch }); } finally { active = null; await save({ active_operation: null }); } }
    if (operation === "session.inspect") return await inspectSessionWithBridge(bridge, { ...state.config, ...params });
    if (operation === "session.cancel") { if (!sessions.has(params.sessionId)) throw Object.assign(new Error("cancellation Session is not owned by this daemon"), { code: "session_identity_mismatch" }); cancelPendingDispatch(bridge); return await cancelSessionWithBridge(bridge, { ...state.config, ...params }); }
    throw Object.assign(new Error(`unknown operation: ${operation}`), { code: "unknown_operation" });
  };
await removeSocket(paths.socket); server = net.createServer((connection) => { connection.setEncoding("utf8"); let buffer = ""; connection.on("data", (chunk) => { buffer += chunk; const at = buffer.indexOf("\n"); if (at < 0) return; const line = buffer.slice(0, at); buffer = buffer.slice(at + 1); void (async () => { let request; try { request = JSON.parse(line); if (request.schema_id !== requestSchema || !request.id || !request.operation) throw Object.assign(new Error("invalid daemon request"), { code: "daemon_protocol" }); const result = await durableDaemonDispatch(paths.dir, request, () => dispatch(request.operation, request.params ?? {})); connection.end(`${JSON.stringify({ schema_id: responseSchema, id: request.id, ok: true, result: { ...result, _shutdown: undefined } })}\n`); if (result._shutdown) { await bridge.close(); await save({ shutdown_state: "clean", stopped_at: new Date().toISOString() }); await finishDaemonProcess(state.config, state.resource_process); server.close(() => process.exit(0)); await removeSocket(paths.socket); } } catch (error) { if (connection.writableEnded) { await save({ shutdown_state: "cleanup_failed", cleanup_error: errorPayload(error) }); return; } connection.end(`${JSON.stringify({ schema_id: responseSchema, id: request?.id ?? null, ok: false, error: errorPayload(error) })}\n`); } })(); }); });
