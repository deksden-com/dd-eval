...
import { CodexBridge, cancelSessionWithBridge, createSessionWithBridge, inspectSessionWithBridge, promptSessionWithBridge } from "./dd-codex.mjs";
...
export async function startDaemon(options) {
  const paths = locations(options.stateDir); const config = { cwd: absolute(options.cwd, "--cwd"), journal: absolute(options.journal, "--journal"), bin: options.bin ?? null };
  ...
  await removeSocket(paths.socket); const state = { schema_id: stateSchema, daemon_id: randomUUID(), pid: null, cwd: config.cwd, journal: config.journal, config, socket: paths.socket, started_at: new Date().toISOString(), shutdown_state: "starting", sessions: [] }; await writeState(paths.state, state);
  ...
  const deadline = Date.now() + 15_000; while (Date.now() < deadline) { await new Promise((resolve) => setTimeout(resolve, 100)); try { return await callDaemon(paths.dir, "daemon.status", {}, 1_000); } catch (error) { if (!["daemon_not_running", "daemon_timeout"].includes(error.code)) throw error; } } throw Object.assign(new Error("daemon did not become ready"), { code: "daemon_start_failed" });
}
export async function stopDaemon(options) { return await callDaemon(options.stateDir, "daemon.stop", {}, options.timeoutMs ?? 30_000); }
...
export async function serveDaemon(stateDir) {
  ...
  const save = (patch = {}) => { Object.assign(state, patch, { pid: process.pid, updated_at: new Date().toISOString(), sessions: [...sessions] }); persist = persist.then(() => writeState(paths.state, state)); return persist; };
  const dispatch = async (operation, params) => {
    ...
    if (active && operation !== "session.inspect") throw Object.assign(new Error(`${active} is in progress`), { code: "operation_busy", retryable: true });
