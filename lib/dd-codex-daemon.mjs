...
export async function serveDaemon(stateDir) {
  ...
  const dispatch = async (operation, params) => {
    ...
    if (operation === "session.prompt") { sessions.add(params.sessionId); active = operation; await save({ active_operation: active }); try { return await promptSessionWithBridge(bridge, { ...state.config, ...params, assertDispatch }); } finally { active = null; await save({ active_operation: null }); } }
    if (operation === "session.start") { sessions.add(params.sessionId); active = operation; await save({ active_operation: active }); try { return await startSessionWithBridge(bridge, { ...state.config, ...params, assertDispatch }); } finally { active = null; await save({ active_operation: null }); } }
