      ...
      await waitForSettlement({
        ...
        observe: async () => {
          ...
          for (const sessionId of sessions) {
            ...
            const stored = status === "notLoaded" ? await bridge.request("thread/turns/list", { threadId: sessionId, limit: 1, sortDirection: "desc", itemsView: "summary" }, 5_000) : null;
            const terminalUnloaded = status === "notLoaded" && Array.isArray(stored?.data) && (stored.data.length === 0 || ["completed", "interrupted", "failed", "cancelled"].includes(stored.data[0]?.status));
            if (status !== "idle" && !terminalUnloaded) unsettled.push(sessionId);
          }
