...
export async function recordOperation({ eventsFile, source, runId, executionId, traceId, operationId, operation, subject = "runner", action }) {
  ...
  try {
    ...
    if (isObservationLoss(error)) await observationLost({ eventsFile, source, runId, executionId, traceId, subject, operationId, operation, error });
...
function isObservationLoss(error) { return ["rpc_timeout", "daemon_timeout", "turn_timeout"].includes(error?.code); }
function errorRecord(error) { return { code: typeof error?.code === "string" ? error.code : "operation_failed", message: error instanceof Error ? error.message : String(error) }; }
