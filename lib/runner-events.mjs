...
import { isObservationLoss, errorRecord } from "./operation-errors.mjs";
...
export async function appendEvent(file, input) {
  ...
  const current = prior.then(() => withLock(file, async () => {
    ...
    if (input.deduplicate && input.id) { const existing = events.find(event => event.id === input.id); if (existing) return existing; }
