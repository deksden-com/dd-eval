...
export function reduceEvents(events) {
  ...
  for (const event of events) {
    ...
    if (["dev.dd.eval.planned", "dev.dd.eval.completed", "dev.dd.eval.recovery_blocked"].includes(event.type) && typeof data.state === "string") state = data.state;
...
export function observationProjection(events) {
  ...
  for (const event of events) {
    ...
    executions.set(event.executionid, { ...prior, ...(explicit || inferred ? { state: explicit ?? inferred } : {}), last_sequence: event.data.sequence });
  }
  return {
...
async function publishObservation(eventsFile, events) {
  ...
  await writeJsonAtomic(path.join(path.dirname(eventsFile), "observation.json"), observationProjection(events));
}
