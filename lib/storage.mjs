...
export async function gcApply({ planFile }) {
  ...
  for (const item of plan.candidates) {
    ...
    if (target === runsRoot || path.dirname(target) !== runsRoot) throw new Error("GC plan must target one complete run");
    await withRunnerLock(`${target}.lifecycle`, async () => {
      if (!(await exists(target))) return;
      if ((await lstat(target)).isSymbolicLink()) throw new Error("GC target cannot be a symbolic link");
