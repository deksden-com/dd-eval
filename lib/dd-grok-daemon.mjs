...
function isolatedEnv(config) { return { ...process.env, HOME: config.grokHome, GROK_HOME: config.grokHome, DD_FLOW_HOME: config.ddFlowHome, ...(config.resourceHome ? { DD_FLOW_RESOURCE_HOME: config.resourceHome } : {}), ...(config.authPath ? { GROK_AUTH_PATH: config.authPath } : {}) }; }
...
function sameConfig(state, config) { return ["cwd", "journal", "bin", "authSource", "model", "reasoning", "mode", "ddFlowBin", "ddFlowHome", "projectRoot", "sessionArchive", "resourceHome"].every((key) => (state.config?.[key] ?? null) === (config[key] ?? null)); }
export async function startDaemon(options) {
  const paths = locations(options.stateDir); const archive = options.sessionArchive ? await readSessionArchiveManifest(options.sessionArchive) : null; const archiveCwd = archive ? absolute(archive.manifest.source_cwd, "Grok Build archive source cwd") : null; const cwd = options.cwd ? absolute(options.cwd, "--cwd") : archiveCwd; if (!cwd) throw new DaemonError("invalid_path", "--cwd must be an absolute path when no session archive is supplied"); if (archiveCwd && cwd !== archiveCwd) throw new DaemonError("session_archive_cwd_mismatch", "Grok Build session archives must start from their recorded source cwd", false, { archive_cwd: archiveCwd, requested_cwd: cwd }); const authSource = options.authPath ? absolute(options.authPath, "--auth-path") : path.join(os.homedir(), ".grok", "auth.json"); const config = { cwd, journal: absolute(options.journal, "--journal"), bin: options.bin ?? process.env.DD_GROK_BIN ?? "grok", authSource, authPath: path.join(paths.home, "auth.json"), model: String(options.model ?? ""), reasoning: String(options.reasoning ?? ""), mode: options.mode ?? "bypassPermissions", ddFlowBin: absolute(options.ddFlowBin, "--dd-flow-bin"), ddFlowHome: absolute(options.ddFlowHome, "--dd-flow-home"), projectRoot: absolute(options.projectRoot, "--project-root"), resourceHome: process.env.DD_FLOW_RESOURCE_HOME ? absolute(process.env.DD_FLOW_RESOURCE_HOME, "DD_FLOW_RESOURCE_HOME") : null, env: { DD_FLOW_HOME: absolute(options.ddFlowHome, "--dd-flow-home") }, sessionArchive: archive?.source ?? null, grokHome: paths.home };
...
class Runtime {
  ...
  track(result) { if (!result?.provider_session_id) return; const session = this.sessions.get(result.provider_session_id); this.sessions.set(result.provider_session_id, { provider_session_id: result.provider_session_id, adapter_session_id: result.adapter_session_id ?? result.provider_session_id, parent_provider_session_id: result.parent_provider_session_id ?? session?.parent_provider_session_id ?? null, root_provider_session_id: result.parent_provider_session_id ? (this.sessions.get(result.parent_provider_session_id)?.root_provider_session_id ?? result.parent_provider_session_id) : (session?.root_provider_session_id ?? result.provider_session_id), cwd: result.cwd ?? result.target?.newCwd ?? result.info?.cwd ?? session?.cwd ?? null }); }
  ...
  async refreshTree(cancel = false) { const running = []; for (const session of [...this.sessions.values()].filter((item) => !item.parent_provider_session_id)) { const observed = await inspectSessionWithBridge(this.bridge, this.options({ sessionId: session.provider_session_id })); this.loadedSessionId = session.provider_session_id; this.track(observed); if (this.running(observed).length) { if (cancel) await cancelSessionWithBridge(this.bridge, this.options({ sessionId: session.provider_session_id })); else running.push(session.provider_session_id); } } await this.persist({ active_tree: running.length > 0 || Boolean(this.active) }); return running; }
  ...
  async dispatch(operation, params) {
    ...
    if (operation === "session.archive") { await this.requireSettled(); return await this.productive(operation, async () => { let session = this.sessions.get(params.sessionId); if (!session?.cwd) { const result = await inspectSessionWithBridge(this.bridge, this.options(params)); this.loadedSessionId = params.sessionId; this.track(result); session = this.sessions.get(result.provider_session_id); } return await exportSessionArchive(this.paths, session.provider_session_id, absolute(session.cwd, "Grok Build session cwd"), params.archiveDir, this.state.versions); }); }
    if (operation === "daemon.stop") { if (this.active) throw new DaemonError("tree_not_settled", "productive operation is still running"); const running = await this.refreshTree(params.cancelTree === true); if (running.length) throw new DaemonError("tree_not_settled", "daemon still owns a running Session tree", false, { sessions: running }); for (const session of this.sessions.values()) { const observed = await inspectSessionWithBridge(this.bridge, this.options({ sessionId: session.provider_session_id })); this.track(observed); } await this.persist({ active_tree: false, shutdown_state: "stopping" }); return { daemon_id: this.state.daemon_id, stopped: true, clean: true, _shutdown: true }; }
    throw new DaemonError("unknown_operation", `unknown daemon operation: ${operation}`);
  }
  async shutdown() { await this.bridge.close(); await this.persist({ shutdown_state: "clean", stopped_at: new Date().toISOString() }); await finishDaemonProcess(this.state.config, this.state.resource_process).catch(() => {}); await new Promise((resolve) => this.server.close(resolve)); await removeSocket(this.paths.socket, this.paths.dir); }
}
