...
export class Runtime {
  ...
  recordDescendant(value, root, source, fallback) { const id = value?.provider_session_id ?? value?.session_id ?? value?.sessionId ?? value?.childSessionId ?? value?.subagentId; const parent = value?.parent_provider_session_id ?? value?.parentSessionId ?? root; if (typeof id !== "string" || !id || typeof parent !== "string" || !parent) return; this.bridge.registerModelSession?.(id, id, this.state.config, parent); const prior = this.descendants.get(id); this.descendants.set(id, { provider_session_id: id, parent_provider_session_id: parent, status: value?.status ?? value?.state ?? (fallback === "unknown" ? prior?.status ?? fallback : fallback), source }); }
  ...
  rootRunning(result) {
    const status = String(result?.info?.status ?? result?.info?.state ?? result?.info?.data?.status ?? "unknown").toLowerCase();
    // Grok 1.0.17 omits status from session/info. Only a durable native
    // creation/end_turn receipt can then establish an idle root. Dispatch
    // clears that receipt before sending any subsequent productive request.
    const receipt = this.sessions.get(result?.provider_session_id)?.native_root_receipt;
    if (status === "unknown" && ["created", "end_turn", "cancelled"].includes(receipt?.kind)) return false;
    return !["completed", "failed", "cancelled", "stopped", "idle"].includes(status);
  }
  async refreshTree(cancel = false) {
    ...
    for (const session of [...this.sessions.values()].filter(item => !item.parent_provider_session_id)) {
      ...
      if (cancel && (rootActive || this.rootRunning(observed) || this.running(observed).length)) {
      ...
      if (this.active || this.rootRunning(observed) || this.running(observed).length) running.push(id);
    }
  ...
  async dispatch(operation, params) {
    ...
    if (operation === "session.cancel") { if (!this.sessions.has(params.sessionId)) throw new DaemonError("session_identity_mismatch", "cancellation Session is not owned by this daemon"); const result = await cancelSessionWithBridge(this.bridge, this.options({ ...params, liveSession: true })); await this.persist({ active_tree: Boolean(this.active) || !result.settled }); return result; }
    ...
    if (operation === "daemon.stop") {
      await waitForSettlement({
        knownSessions: () => [...this.sessions.values()].filter(s => !s.parent_provider_session_id).map(s => s.provider_session_id),
  ...
  async shutdown() { await this.bridge.close(); await finishDaemonProcess(this.state.config, this.state.resource_process); await unlink(path.join(this.paths.home, "auth.json")).catch(error => { if (error.code !== "ENOENT") throw error; }); await this.persist({ shutdown_state: "clean", stopped_at: new Date().toISOString() }); await new Promise((resolve) => this.server.close(resolve)); await removeSocket(this.paths.socket, this.paths.dir); }
}
