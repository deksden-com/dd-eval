...
export async function startDaemon(options) {
  ...
  await removeSocket(paths.socket, paths.dir); await mkdir(paths.home, { recursive: true, mode: 0o700 }); const copiedAuth = await copyAuth(paths, authSource); await writeHook(paths, config, absolute(options.entryPath, "dd-grok entry")); const versions = await doctor({ bin: config.bin });
  ...
  await writeState(paths.state, state); const log = await open(paths.log, "a", 0o600); const child = spawn(process.execPath, [absolute(options.entryPath, "dd-grok entry"), "daemon", "serve", "--state-dir", paths.dir], { cwd: config.cwd, detached: true, stdio: ["ignore", log.fd, log.fd] }); child.unref(); await log.close();
...
class Runtime {
  constructor(paths, state, bridge, initialized) { this.paths = paths; this.state = state; this.bridge = bridge; this.initialized = initialized; this.sessions = new Map((state.sessions ?? []).map((item) => [item.provider_session_id, item])); this.active = null; this.server = null; this.persisting = Promise.resolve(); }
  ...
  options(params) { const sessionId = params.sessionId; const root = sessionId ? (this.sessions.get(sessionId)?.root_provider_session_id ?? sessionId) : undefined; return { ...this.state.config, ...params, initialized: this.initialized, daemonId: this.state.daemon_id, rootProviderSessionId: root, allowBackground: true, liveSession: Boolean(sessionId) }; }
  ...
  async dispatch(operation, params) {
    ...
    if (operation === "session.prompt") return await this.productive(operation, async () => await promptSessionWithBridge(this.bridge, this.options(params)));
    if (operation === "session.fork") return await this.productive(operation, async () => await forkSessionWithBridge(this.bridge, this.options(params)));
    if (operation === "session.inspect") { const result = await inspectSessionWithBridge(this.bridge, this.options(params)); this.track(result); await this.persist(); return result; }
    if (operation === "session.cancel") { const result = await cancelSessionWithBridge(this.bridge, this.options(params)); await this.persist({ active_tree: false }); return result; }
