...
class Runtime {
  ...
  recordDescendant(value, root, source, fallback) { const id = value?.provider_session_id ?? value?.session_id ?? value?.sessionId ?? value?.childSessionId ?? value?.subagentId; const parent = value?.parent_provider_session_id ?? value?.parentSessionId ?? root; if (typeof id !== "string" || !id || typeof parent !== "string" || !parent) return; const prior = this.descendants.get(id); this.descendants.set(id, { provider_session_id: id, parent_provider_session_id: parent, status: value?.status ?? value?.state ?? fallback, source, ...(prior?.status && fallback === "unknown" ? { status: prior.status } : {}) }); }
  observeSubagentEvent(message) { const root = message?.params?.sessionId; const update = message?.params?.update; const kind = String(update?.sessionUpdate ?? update?.type ?? message?.method ?? ""); if (!root || !/subagent/i.test(kind)) return; const values = [update?.subagent, update?.subagentInfo, update?.child, update?.result, update]; for (const value of values) this.recordDescendant(value, root, "x.ai/subagent/event", /fail|error/i.test(kind) ? "failed" : /cancel/i.test(kind) ? "cancelled" : /complete|finish|done/i.test(kind) ? "completed" : "running"); }
  ...
  async refreshTree(cancel = false) { const running = []; for (const session of [...this.sessions.values()].filter((item) => !item.parent_provider_session_id)) { let observed = await inspectSessionWithBridge(this.bridge, this.options({ sessionId: session.provider_session_id })); this.loadedSessionId = session.provider_session_id; this.track(observed); if (this.running(observed).length && cancel) { await cancelSessionWithBridge(this.bridge, this.options({ sessionId: session.provider_session_id })); observed = await inspectSessionWithBridge(this.bridge, this.options({ sessionId: session.provider_session_id })); this.track(observed); } if (this.running(observed).length) running.push(session.provider_session_id); } await this.persist({ active_tree: running.length > 0 || Boolean(this.active) }); return running; }
  ...
  async dispatch(operation, params) {
    ...
    if (operation === "session.inspect") { const result = await inspectSessionWithBridge(this.bridge, this.options(params)); this.loadedSessionId = params.sessionId; this.track(result); await this.persist(); return { ...result, descendants: this.topology(result.provider_session_id) }; }
    ...
    if (operation === "daemon.stop") { if (this.active && !params.cancelTree) throw new DaemonError("tree_not_settled", "productive operation is still running"); const running = await this.refreshTree(params.cancelTree === true); if (this.active || running.length) throw new DaemonError("tree_not_settled", "daemon still owns a running Session tree", false, { sessions: running, active_operation: this.active }); for (const session of this.sessions.values()) { const observed = await inspectSessionWithBridge(this.bridge, this.options({ sessionId: session.provider_session_id })); this.track(observed); } await this.persist({ active_tree: false, shutdown_state: "stopping" }); return { daemon_id: this.state.daemon_id, stopped: true, clean: true, _shutdown: true }; }
