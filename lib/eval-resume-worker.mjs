...
async function runWorker(file) {
  ...
  await withRunnerLock(`${root}.resume-observer`, async () => {
    ...
    if (["completed", "superseded", "recovery_blocked"].includes(saved.status)) return;
    ...
    const publish = async update => withRunnerLock(file, async () => {
      ...
      if (saved.status === "recovery_blocked") await appendEvent(path.join(root, "events.jsonl"), { source: "dd-eval://runner", runId: intent.run_id, type: "dev.dd.eval.recovery_blocked", data: { state: saved.status, request_id: intent.request_id, error: saved.error } });
