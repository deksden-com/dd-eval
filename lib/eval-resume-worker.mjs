    ...
    try {
      ...
      for (;;) {
        ...
        try {
          ...
          if (result.judge_status === "in_progress" || (result.state === "awaiting_provider" && result.executions?.some(item => item?.state === "failed"))) {
            if (Date.now() - lastSettlingUpdate >= 5_000) {
              await publish({ status: "settling", result, error: null });
              lastSettlingUpdate = Date.now();
            }
            await delay(settleDelay);
            settleDelay = Math.min(settleDelay * 2, 5_000);
            continue;
          }
          settleDelay = 250;
