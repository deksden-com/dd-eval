...
import { assertControlResumeSource, evalRun, runnerControlResume, runnerControlStatus, runnerResume, runnerCleanup, validateControlResumeInput, prepareRunnerContinuation } from "./runner.mjs";
import { appendEvent, hashJson, readEvents, sha256, writeJsonAtomic } from "./runner-events.mjs";
...
import { recoveryObservationBudget, withRecoveryObservation } from "./recovery-observation-budget.mjs";
...
async function preparedManifest(root) {
  const file = path.join(root, "manifest.json");
  let bytes, manifest;
  try { bytes = await readFile(file, "utf8"); manifest = JSON.parse(bytes); }
  catch (error) { fail(`Cannot read EVAL manifest: ${error instanceof Error ? error.message : String(error)}`, "control_input_invalid"); }
  if (!manifest || typeof manifest !== "object" || !/^EVAL-[A-Za-z0-9-]+$/.test(manifest.run_id ?? "")) fail("EVAL manifest has no valid run_id", "control_input_invalid");
...
// A worker can finish its local observation while the managed RUN remains
// awaiting provider reconciliation. That is not a terminal continuation.
const continuationTerminal = saved => ["superseded", "recovery_blocked", "failed"].includes(saved.status)
  || saved.status === "completed" && saved.result?.state !== "awaiting_provider";
...
export async function requestRunnerContinuation({ evalRoot, kind = "resume", requestId = randomUUID() }) {
  if (!["run", "resume", "cleanup"].includes(kind)) fail("Invalid runner attempt kind", "control_request_invalid");
  validateControlResumeInput({ requestId, waitMs: 0 });
  const root = path.resolve(evalRoot), { bytes, manifest } = await preparedManifest(root);
  const file = runnerFile(root, requestId);
  const intent = { eval_root: root, run_id: manifest.run_id, request_id: requestId, kind, manifest_sha256: sha256(bytes) };
  let retained;
  try { retained = await read(file); } catch (error) { if (error.code !== "ENOENT") throw error; }
  if (retained) {
    if (hashJson(retained.intent) !== hashJson(intent)) fail("Attempt request identifies another intent", "control_request_conflict");
    if (continuationTerminal(retained)) return { root, run_id: manifest.run_id, accepted: true, pending: retained.status !== "completed", continuation: { file, status: retained.status } };
  }
  if (kind !== "cleanup") {
    await prepareRunnerContinuation(manifest, await readEvents(path.join(root, "events.jsonl")));
  }
  await registerRunHome(root);
  await mkdir(path.dirname(file), { recursive: true });
  await withRunnerLock(file, async () => {
    let existing;
    try { existing = await read(file); } catch (error) { if (error.code !== "ENOENT") throw error; }
    if (existing) {
      if (hashJson(existing.intent) !== hashJson(intent)) fail("Attempt request identifies another intent", "control_request_conflict");
    } else await writeJsonAtomic(file, { schema_id: "dd-eval/resume-worker@1", intent, status: "requested" });
  });
  const current = await read(file);
  const terminal = continuationTerminal(current);
  const live = current.owner_pid ? workerLiveness(current, await processSnapshot()).state === "alive" : false;
  if (!terminal && !live) await spawnWorker(file, root, requestId);
  return { root, run_id: manifest.run_id, accepted: true, pending: current.status !== "completed", continuation: { file, status: current.status } };
}
...
async function acceptAndObserve({ evalRoot, requestId, fromRequestId, waitMs }, signal, progress) {
  ...
  const { bytes: manifestBytes, manifest } = await preparedManifest(root);
  ...
  let retained;
  try { retained = await read(file); } catch (error) { if (error.code !== "ENOENT") throw error; }
  if (retained && (retained.schema_id !== "dd-eval/resume-worker@1" || hashJson(retained.intent) !== hashJson(intent))) fail("Resume request already identifies another experiment continuation", "control_request_conflict");
  if (!retained) await prepareRunnerContinuation(manifest);
  ...
  for (;;) {
    ...
    if (!waitMs || scope?.pending === false || ["failed", "completed", "superseded", "recovery_blocked"].includes(current.status) || performance.now() >= deadline) {
...
async function runWorker(file) {
  ...
  const ordinary = ["run", "resume", "cleanup"].includes(intent?.kind);
  ...
  await withRunnerLock(`${root}.resume-observer`, async () => {
    ...
    const observation = recoveryObservationBudget(saved);
    const publishRootFailure = async () => {
      if (saved.status === "recovery_blocked" || saved.status === "failed" && (!ordinary || saved.process_id)) await appendEvent(path.join(root, "events.jsonl"), { source: "dd-eval://runner", runId: intent.run_id, type: saved.status === "failed" ? "dev.dd.eval.recovery_failed" : "dev.dd.eval.recovery_blocked", id: `resume-worker:${intent.request_id}:${saved.status}`, deduplicate: true, data: { state: saved.status, request_id: intent.request_id, error: saved.error } });
    };
    await publishRootFailure();
    if (continuationTerminal(saved)) return;
    ...
    const publish = async update => {
      if (update.status === "recovery_blocked") {
        const startedAt = new Date().toISOString();
        try {
          // Reserve the last second of the existing budget for read-only local
          // control receipts. This is diagnostics, never settlement proof.
          const timeout = Math.min(1000, observation.timeout());
          update.final_observation = { started_at: startedAt, ...(await withRecoveryObservation(observation, () => runnerControlStatus({ evalRoot: root, signal: AbortSignal.timeout(timeout) }))), finished_at: new Date().toISOString() };
        } catch (error) { update.final_observation = { started_at: startedAt, finished_at: new Date().toISOString(), unavailable: true, observation_complete: false, error: errorRecord(error) }; }
      ...
      saved = { ...current, ...update, recovery_observation: observation.state(), owner_pid: process.pid, owner_started_at: self.started, updated_at: new Date().toISOString() };
    ...
    try {
      ...
      if (intent.kind === "cleanup") {
        for (;;) {
          if (observation.exhausted(true, 1000)) {
            await publish({ status: "recovery_blocked", error: { code: "recovery_observation_budget_exhausted", message: "Cleanup remains unconfirmed; a new explicit cleanup request is required" } });
            return;
          }
          const result = await withRecoveryObservation(observation, () => runnerCleanup({ evalRoot: root, requestId: intent.request_id, expectedManifestSha256: intent.manifest_sha256 }));
          observation.exhausted();
          await publish({ result, status: result.cleanup_state === "settled" ? "completed" : "settling" });
          if (result.cleanup_state === "settled") return;
          await delay(observation.nextDelay(1000));
        }
      }
      while (!ordinary) {
        if (observation.exhausted(true, 1000)) {
          await publish({ status: "recovery_blocked", error: { code: "recovery_observation_budget_exhausted", message: "Scope release has no confirmed settlement within the active observation budget" } });
          return;
        }
        const scope = await withRecoveryObservation(observation, () => runnerControlResume(args));
      ...
      let settleDelay = 1_000;
      let lastSettlingUpdate = 0;
      let cleanupPending = saved.result?.execution_state === "failed" && saved.result?.cleanup_state === "pending";
      observation.exhausted(false);
      for (;;) {
        if (observation.exhausted(cleanupPending, 1000) && cleanupPending) {
          await publish({ status: "recovery_blocked", error: { code: "recovery_observation_budget_exhausted", message: "Execution cleanup has no confirmed settlement within the active observation budget" } });
          return;
        }
        await call(["check-admission", "--id", record.id, "--lease-token", record.lease_token]);
        try {
          const result = cleanupPending
            ? await withRecoveryObservation(observation, () => runnerCleanup({ evalRoot: root, expectedManifestSha256: intent.manifest_sha256 }))
            : await runnerResume({ evalRoot: root, ...(!ordinary ? { resumeRequestId: intent.request_id } : {}), expectedManifestSha256: intent.manifest_sha256 });
          // Productive subject/Judge work has its own activity policy. Start
          // the recovery clock only once failure cleanup is actually pending.
          if (!cleanupPending) observation.exhausted(false);
          cleanupPending = result.execution_state === "failed" && result.cleanup_state === "pending";
          // A failed execution whose managed cleanup/capture is still pending
          // has no authorized new Subject dispatch, but it does need this same
          // observer to reconcile the already-issued control operation.
          // A managed RUN can have accepted dispatch but temporarily lack a
          // controller reply. The detached owner must reattach; publishing a
          // terminal worker receipt here strands fork --start at awaiting_provider.
          if (result.state === "awaiting_provider" || result.judge_status === "in_progress" || cleanupPending) {
            if (Date.now() - lastSettlingUpdate >= 5_000) {
              await publish({ status: "settling", result, error: null });
              lastSettlingUpdate = Date.now();
            }
            await delay(cleanupPending ? observation.nextDelay(1000) : settleDelay);
            settleDelay = settleDelay < 2_000 ? 2_000 : settleDelay < 5_000 ? 5_000 : 10_000;
            continue;
          }
          settleDelay = 250;
      ...
      observation.exhausted();
      await publish({ status: observation.state().remaining_ms === 0 ? "recovery_blocked" : error.code === "managed_run_waiting_for_user" ? "waiting_for_user" : ["control_request_stale", "managed_run_controlled", "runtime_scope_controlled", "runtime_scope_stopped"].includes(error.code) ? "superseded" : "failed", error: errorRecord(error) });
    } finally {
      if (recordValidated) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === entrypoint) {
  ...
  runWorker(file).catch(async error => {
    ...
    // A duplicate observer that never acquired ownership cannot overwrite the
    // active owner's receipt merely because waiting for its lock timed out.
    if (error.code === "runner_lock_timeout") return;
    try {
      await withRunnerLock(file, async () => {
        const saved = await read(file);
