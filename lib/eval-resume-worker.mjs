...
import { assertControlResumeSource, evalRun, runnerControlResume, runnerResume, runnerCleanup, validateControlResumeInput } from "./runner.mjs";
...
export async function requestRunnerContinuation({ evalRoot, kind = "resume", requestId = randomUUID() }) {
  ...
  const terminal = ["completed", "superseded", "recovery_blocked"].includes(current.status);
...
async function runWorker(file) {
  ...
  await withRunnerLock(`${root}.resume-observer`, async () => {
    ...
    if (["completed", "superseded", "recovery_blocked", "failed"].includes(saved.status)) return;
    ...
    try {
      ...
      if (intent.kind === "cleanup") {
        for (;;) {
          ...
          await delay(observation.nextDelay());
        }
      }
      while (!ordinary) {
        if (observation.exhausted()) {
      ...
      for (;;) {
        if (observation.exhausted(cleanupPending) && cleanupPending) {
