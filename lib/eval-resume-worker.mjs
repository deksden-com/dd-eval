...
import { assertControlResumeSource, evalRun, runnerControlResume, runnerControlStatus, runnerResume, runnerCleanup, validateControlResumeInput } from "./runner.mjs";
  ...
  await withRunnerLock(`${root}.resume-observer`, async () => {
    ...
    const publish = async update => {
      if (update.status === "recovery_blocked") {
        ...
        } catch (error) { update.final_observation = { started_at: startedAt, finished_at: new Date().toISOString(), unavailable: true, error: errorRecord(error) }; }
