...
// A transport timeout says that the observer lost its reply, not that the
// provider stopped the Turn.  Replaying a prompt in that state can duplicate
// a Work.  Only an authoritative terminal Turn outcome may enter recovery.
const recoverableDriverCodes = new Set(["turn_not_active", "turn_interrupted"]);
export const isRecoverableDriverError = (error) => recoverableDriverCodes.has(error?.code);
...
async function canonicalResumeUnlocked({ buildRoot, detachTurns = false }) {
  ...
  const driveReferenceFanout = async () => {
    try {
      return await driveFanout({ profile, attempt: path.join(root, "reference"), projectRoot: roots.projectRoot, runtimeRoot: roots.runtimeRoot, runId: lifecycle.run_id, stage, eventsFile: path.join(root, "build", "events.jsonl"), event: { source: "dd-eval://runner", runId: state.revision }, recoverRunning: recovering });
    } catch (error) {
...
export async function driverRuntimeArgs(args, { cwd, env = {}, profile = null } = {}) {
  if (args[0] !== "daemon" || args[1] !== "start") return args;
  const resolved = [...args];
  ...
  // Daemons survive the CLI process that starts them. Resolve the user-local
  // ACP binary while the runner still owns its PATH, rather than asking the
  // detached daemon to rediscover it from an inherited environment.
  if (profile?.harness === "zcode-acp" && !resolved.includes("--zcode-acp-bin")) {
    resolved.push("--zcode-acp-bin", await absoluteExecutable(env.DD_ZCODE_ACP_BIN ?? process.env.DD_ZCODE_ACP_BIN ?? "zcode-acp", cwd, launchEnv));
  }
  return resolved;
}
async function callDriver(profile, args, options) {
  ...
  return await new Promise((resolve, reject) => {
    ...
    child.on("close", (code, signal) => {
      ...
      try { resolve(JSON.parse(stdout.trim())); } catch (error) { reject(new Error(`driver returned invalid JSON: ${error.message}`)); }
    });
  });
}
...
async function captureExecutionCandidate({ projectRoot, runtimeRoot, runId, attempt, stage, journal }) {
  await auditRuntimeIntegrity({ journal, runtimeRoot, output: path.join(attempt, "runtime-integrity.json") });
...
// The runtime home also contains the subject's project and RUN workspaces.
// Reading those paths is normal.  Only the private engine/bin area is
// protected; public `bin/dd-flow` remains the mutation boundary.
export function runtimeIntegrityViolation(command, runtimeRoot) {
  const protectedRoots = [path.join(runtimeRoot, "engines"), path.join(runtimeRoot, "bin")];
  const publicBin = path.join(runtimeRoot, "bin", "dd-flow");
  const protectedPath = (value) => protectedRoots.some((root) => value.includes(root)) && !value.includes(publicBin);
  // Check individual shell segments. Mentioning the public launcher elsewhere
  // must not bless a private engine invocation in a different segment.
  return command.split(/[;\n]+/).some((segment) => {
    const text = segment.trim();
    if (/\bsqlite3\b/.test(text) && text.includes(path.join(runtimeRoot, "db.sqlite")) && /\b(?:insert|update|delete|replace|alter|create|drop|vacuum)\b/i.test(text)) return true;
    if (!protectedPath(text)) return false;
    if (/\b(?:node|nodejs|python(?:3)?|ruby|perl)\b/.test(text)) return true;
    if (/\bsqlite3\b/.test(text) && /\b(?:insert|update|delete|replace|alter|create|drop|vacuum)\b/i.test(text)) return true;
    if (/\b(?:rm|mv|tee)\b|\b(?:sed|perl)\s+-[^\n]*i\b/.test(text)) return true;
    // Copying protected source out for inspection is permitted; copying into
    // a protected root or redirecting there is a mutation.
    if (/\bcp\b/.test(text)) {
      const args = text.replace(/^.*?\bcp\b\s+/, "").trim().split(/\s+/);
      return protectedPath(args.at(-1) ?? "");
    }
    const redirected = text.match(/(?:>|>>)\s*([^\s]+)/g) ?? [];
    return redirected.some((item) => protectedPath(item));
  });
}
async function auditRuntimeIntegrity({ journal, runtimeRoot, output }) {
  const commands = [];
  const journals = await runtimeIntegrityJournals(journal);
  for (const file of journals) for (const event of await readJsonLines(file, "driver journal")) collectCommands(event, commands);
  const violations = commands.filter((command) => runtimeIntegrityViolation(command, runtimeRoot));
  const receipt = { schema_id: "dd-eval/runtime-integrity@1", journals, runtime_root: runtimeRoot, checked_at: now(), status: violations.length ? "failed" : journals.length ? "passed" : "unavailable", journal_available: journals.length > 0, violations };
  await writeJsonAtomic(output, receipt);
  if (violations.length) fail("subject bypassed the public dd-flow runtime boundary; candidate is invalid", "candidate_runtime_tampering");
  return receipt;
}
async function runtimeIntegrityJournals(journal) {
  const drivers = path.dirname(journal), files = new Set();
  if (await exists(journal)) files.add(journal);
  for (const name of ["daemon", "fanout"]) {
    const root = path.join(drivers, name);
    if (name === "daemon") { const file = path.join(root, "events.jsonl"); if (await exists(file)) files.add(file); continue; }
    for (const entry of await readdir(root, { withFileTypes: true }).catch(() => [])) { const file = path.join(root, entry.name, "events.jsonl"); if (entry.isDirectory() && await exists(file)) files.add(file); }
  }
  return [...files].sort();
}
function collectCommands(value, output) {
  if (Array.isArray(value)) { for (const item of value) collectCommands(item, output); return; }
  if (!isObject(value)) return;
  for (const [key, item] of Object.entries(value)) {
    if ((key === "command" || key === "CommandLine") && typeof item === "string") output.push(item);
    else collectCommands(item, output);
  }
}
...
async function finalizeRunProjection({ root, manifest, loaded, results, permits = null }) {
  ...
  const pending = results.some((result) => result.state === "awaiting_provider" || result.state === "awaiting_worker");
...
export function workerUsageSource(profile) {
  ...
  return ["zcode-acp", "grok-acp", "opencode-acp", "antigravity-cli"].includes(profile?.harness) ? "adapter_ingested" : "session_sync";
}
...
async function startIsolatedWorkerDaemon({ profile, attempt, projectRoot, runtimeRoot, key, restart = false }) {
  ...
  if (restart) await rm(stateDir, { recursive: true, force: true });
  if (profile.harness === "codex-desktop") await initializeCodexHome({ projectRoot, runtimeRoot, codexHome });
...
async function stopIsolatedWorkerDaemon({ profile, daemon, projectRoot }) {
  try { return await callDriver(profile, ["daemon", "stop", ...daemon.daemonArgs], { cwd: projectRoot, env: daemon.env }); }
    ...
    // A worker may have returned while a native child is still alive. Release
    // that capacity deterministically; callers retain their primary receipt.
    try { return await callDriver(profile, ["daemon", "stop", ...daemon.daemonArgs, "--cancel-tree"], { cwd: projectRoot, env: daemon.env }); }
    catch (cleanupError) { return { stopped: false, error: cleanupError instanceof Error ? cleanupError.message : String(cleanupError), initial_error: error instanceof Error ? error.message : String(error) }; }
  }
}
...
// Some harnesses retain an intermediate status sentence before the final
// assistant response. The probe is about concurrent completion, so require
// the declared terminal marker rather than rejecting an otherwise complete
// provider Turn for that transport formatting.
export function capacityProbeSucceeded(text, expected) {
  return typeof text === "string" && new RegExp(`(?:^|\\n)${expected.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*$`).test(text.trim());
}

export function capacityProbeResult({ text, expected, completedAt, holdUntil }) {
  if (!capacityProbeSucceeded(text, expected)) return { completed: false, reason: "missing_terminal_marker" };
  if (completedAt < holdUntil) return { completed: false, reason: "probe_returned_before_hold" };
...
async function runZcodeCapacityProbe({ profile, attempt, projectRoot, probe, eventsFile, event }) {
  ...
  try {
    ...
    try {
      ...
      // A lost outer RPC reply cannot be interpreted as zero capacity: every
      // probe may have completed behind the transport boundary.
      throw Object.assign(new Error(`ZCode capacity probe result was not observed: ${error instanceof Error ? error.message : String(error)}`), { code: "capacity_probe_observation_lost" });
    }
    const responses = new Map((Array.isArray(response?.results) ? response.results : []).map((result) => [result.provider_session_id, result]));
    const holdUntil = promptedAt + hold * 1000;
    const results = launches.map((launch) => {
      ...
      return { key: launch.key, session_id: launch.session_id, ...capacityProbeResult({ text: result?.assistant_text, expected, completedAt: Number.isFinite(completedAt) ? completedAt : 0, holdUntil }), ...(Number.isFinite(completedAt) ? { completed_at: new Date(completedAt).toISOString() } : {}), ...(result?.assistant_text ? { assistant_text: result.assistant_text } : {}), ...(result?.error ? { error: result.error } : response?.error ? { error: response.error } : {}) };
    });
...
async function runCapacityProbe({ profile, attempt, projectRoot, runtimeRoot, probe, eventsFile, event }) {
  ...
  const prompted = launches.map((launch) => (async () => {
    ...
    try {
      ...
      return { key: launch.key, session_id: launch.session_id, ...capacityProbeResult({ text: response?.assistant_text, expected, completedAt, holdUntil: promptedAt + hold * 1000 }), completed_at: new Date(completedAt).toISOString() };
...
export function workerPromptPath(work, profile, sessionId) {
  return work?.sessions?.find((item) => item.harness_id === profile.harness && item.native_session_id === sessionId)?.prompt_path;
}

async function runFanoutWorker({ profile, attempt, projectRoot, runtimeRoot, work, eventsFile, event, resumeSessionId = null, wave = null }) {
  const workId = String(work.work_id ?? ""); const startCommand = String(work.start_command ?? "");
  if (!workId || (!startCommand && !resumeSessionId)) fail("ready fan-out Work lacks its exact start command", "fanout_contract_invalid");
  const key = `work-${workId}`;
  let daemon = await startIsolatedWorkerDaemon({ profile, attempt, projectRoot, runtimeRoot, key, restart: Boolean(resumeSessionId) });
  // Keep this visible to the error receipt below. A Work can fail after its
  // Session was created; that failure must never be masked by a ReferenceError.
  ...
  try {
    ...
    } else await appendEvent(eventsFile, { ...event, type: "dev.dd.eval.fanout.worker.resumed", data: { work_id: workId, session_id: sessionId } });
    const cancel = async (failedWorkId) => {
      await callDriver(profile, ["session", "cancel", ...daemon.daemonArgs, "--session-id", sessionId, "--cwd", projectRoot, "--journal", daemon.journal], { cwd: projectRoot, env: daemon.env }).catch(() => {});
      await appendEvent(eventsFile, { ...event, type: "dev.dd.eval.fanout.worker.cancelled_after_peer_failure", data: { work_id: workId, session_id: sessionId, failed_work_id: failedWorkId } });
    };
    if (wave) {
      wave.cancellers.set(workId, cancel);
      if (wave.failure) {
        await cancel(wave.failure.workId);
        fail(`fan-out worker ${workId} was cancelled after peer ${wave.failure.workId} failed`, "fanout_peer_failed");
      }
    }
    // Keep the adapter's normal long-lived Turn deadline. A short runner-local
    // timeout only closes its RPC client while the provider is still working,
    // and must not be mistaken for an interrupted Work.
    const promptWorker = async (prompt) => await callDriver(profile, ["session", "prompt", ...daemon.daemonArgs, "--session-id", sessionId, "--cwd", projectRoot, "--model", profile.model, "--reasoning", profile.reasoning, "--prompt", prompt, "--journal", daemon.journal], { cwd: projectRoot, env: daemon.env });
    const promptDurably = async (initialPrompt) => {
      let prompt = initialPrompt;
      for (let recovery = 0; ; recovery += 1) {
        try { return await promptWorker(prompt); }
        catch (error) {
          const reason = error instanceof Error ? error.message : String(error);
          if (recovery >= 2 || !isRecoverableDriverError(error)) throw error;
          // An interrupted provider Turn can leave its private bridge polling.
          // Cancel only transport state, then continue the same durable Work.
          await appendEvent(eventsFile, { ...event, type: "dev.dd.eval.fanout.worker.turn_recovery", data: { work_id: workId, session_id: sessionId, recovery: recovery + 1, reason } });
          // A terminal provider Turn has nothing left to cancel. Cancellation is
          // only best-effort transport cleanup; the durable Work is still valid.
          await callDriver(profile, ["session", "cancel", ...daemon.daemonArgs, "--session-id", sessionId, "--cwd", projectRoot, "--journal", daemon.journal], { cwd: projectRoot, env: daemon.env }).catch(() => {});
          daemon = await startIsolatedWorkerDaemon({ profile, attempt, projectRoot, runtimeRoot, key });
          const resumed = await commandJson("dd-flow", ["work", "show", workId, "--project-root", projectRoot], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
          const promptPath = workerPromptPath(resumed.work, profile, sessionId);
          if (typeof promptPath !== "string") fail(`fan-out worker ${workId} has no durable prompt for recovery`, "fanout_worker_prompt_missing");
          if (!(await exists(promptPath))) fail(`fan-out worker ${workId} prompt file is missing: ${promptPath}`, "fanout_worker_prompt_missing");
          prompt = `The previous Turn was interrupted. Read the existing authoritative Work prompt at ${promptPath}. Do not call work start or create another Work. Complete this same Work and use its exact work finish command.`;
        }
      }
    };
    let response = await promptDurably(resumeSessionId ? `Resume the already-started ${workId} Work. Read its authoritative durable Work prompt, complete only its unfinished result, invoke its exact standalone work finish command, then stop. Do not run work start, create a Work, or create a child agent.` : fanoutWorkerPrompt({ workId, startCommand }));
    ...
    if (observed.work?.status === "running") {
      ...
      response = await promptDurably(fanoutWorkerRecoveryPrompt({ workId }));
    ...
    await appendEvent(eventsFile, { ...event, type: "dev.dd.eval.fanout.worker.failed", data: { work_id: workId, session_id: sessionId, code: error?.code ?? "fanout_worker_failed", error: error instanceof Error ? error.message : String(error), ...(error?.details ? { details: error.details } : {}) } });
    throw error;
  } finally {
    wave?.cancellers.delete(workId);
    await stopIsolatedWorkerDaemon({ profile, daemon, projectRoot });
  }
}

async function settleFanoutWave({ wave, start }) {
  const control = { failure: null, cancellers: new Map() };
  const workers = wave.map((work) => start(work, control).catch(async (error) => {
    if (!control.failure) {
      control.failure = { workId: work.work_id, error };
      if (!isRecoverableDriverError(error)) await Promise.all([...control.cancellers.entries()]
          .filter(([workId]) => workId !== work.work_id)
          .map(([, cancel]) => cancel(work.work_id)));
    }
    throw error;
  }));
...
async function resumableFanoutWorkers({ projectRoot, runtimeRoot, eventsFile }) {
  const sessions = new Map();
  for (const item of await readEvents(eventsFile)) {
    const workId = item.data?.work_id, sessionId = item.data?.session_id;
    if (!workId || !sessionId) continue;
    if (item.type === "dev.dd.eval.fanout.worker.session_created") sessions.set(workId, sessionId);
    if (["dev.dd.eval.fanout.worker.completed", "dev.dd.eval.fanout.worker.failed"].includes(item.type)) sessions.delete(workId);
  }
  const candidates = [];
  for (const [work_id, session_id] of sessions) {
    const observed = await commandJson("dd-flow", ["work", "show", work_id, "--project-root", projectRoot], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
    if (observed.work?.status === "running") candidates.push({ work_id, session_id });
  }
  return candidates;
}

/** Execute an engine-declared graph without deciding any semantic part of the Stage. */
async function driveFanout({ profile, attempt, projectRoot, runtimeRoot, runId, stage, eventsFile, event, recoverRunning = false }) {
  ...
  for (;;) {
    ...
    if (recoverRunning && Number(fanout.works?.running ?? 0) > 0) {
      const resumable = await resumableFanoutWorkers({ projectRoot, runtimeRoot, eventsFile });
      recoverRunning = false;
      if (resumable.length) {
        await appendEvent(eventsFile, { ...event, type: "dev.dd.eval.fanout.wave.resumed", data: { stage, work_ids: resumable.map((work) => work.work_id) } });
        try { await settleFanoutWave({ wave: resumable, start: (work, wave) => runFanoutWorker({ profile, attempt, projectRoot, runtimeRoot, work: { work_id: work.work_id }, resumeSessionId: work.session_id, eventsFile, event, wave }) }); }
        catch (error) { fail(error.message, error.code ?? "fanout_worker_failed"); }
        status = await fanoutStatus({ projectRoot, runtimeRoot, runId, stage });
        continue;
      }
    }
    ...
    if (ready.length) {
      ...
      try { await settleFanoutWave({ wave, start: (work, control) => runFanoutWorker({ profile, attempt, projectRoot, runtimeRoot, work, eventsFile, event, wave: control }) }); }
      catch (error) { fail(error.message, error.code ?? "fanout_worker_failed"); }
      status = await fanoutStatus({ projectRoot, runtimeRoot, runId, stage });
      continue;
    }
  ...
  const results = await mapLimited(manifest.executions, runProfile.value.concurrency.global, async (execution) => {
    ...
    try {
      ...
      try {
        ...
        try { await callDriver(profile, ["daemon", "stop", ...daemonArgs], { cwd: projectRoot, env: executionEnv }); }
        catch (cleanupError) {
