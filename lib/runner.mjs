...
async function auditRuntimeIntegrity({ journal, runtimeRoot, output }) {
  const commands = [];
  // Adapter journals deliberately use their harness-native JSONL shape, not
  // runner CloudEvents. Keep runner-event parsing strict and inspect the
  // adapter records as ordinary JSON here.
  const journalEvents = await exists(journal) ? await readJsonLines(journal, "driver journal") : [];
  for (const event of journalEvents) collectCommands(event, commands);
  const violations = commands.filter((command) => runtimeIntegrityViolation(command, runtimeRoot));
  const receipt = { schema_id: "dd-eval/runtime-integrity@1", journal, runtime_root: runtimeRoot, checked_at: now(), status: violations.length ? "failed" : "passed", journal_available: journalEvents.length > 0, violations };
...
async function runFanoutWorker({ profile, attempt, projectRoot, runtimeRoot, work, eventsFile, event, resumeSessionId = null, wave = null }) {
  ...
  try {
    ...
    await appendEvent(eventsFile, { ...event, type: "dev.dd.eval.fanout.worker.failed", data: { work_id: workId, session_id, code: error?.code ?? "fanout_worker_failed", error: error instanceof Error ? error.message : String(error), ...(error?.details ? { details: error.details } : {}) } });
    throw error;
  } finally {
...
async function settleFanoutWave({ wave, start }) {
  ...
  const workers = wave.map((work) => start(work, control).catch(async (error) => {
    if (!control.failure) {
      ...
      await Promise.all([...control.cancellers.entries()]
