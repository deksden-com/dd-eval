...
async function canonicalResumeUnlocked({ buildRoot }) {
  ...
  if (state.status === "failed" && state.failure?.code === "driver_failed" && /session\.prompt timed out/.test(String(state.failure.message ?? ""))) {
...
async function runFanoutWorker({ profile, attempt, projectRoot, runtimeRoot, work, eventsFile, event, resumeSessionId = null }) {
  ...
  try {
    ...
    try { response = await promptWorker(resumeSessionId ? `Resume the already-started ${workId} Work. Read its authoritative durable Work prompt, complete only its unfinished result, invoke its exact standalone work finish command, then stop. Do not run work start, create a Work, or create a child agent.` : fanoutWorkerPrompt({ workId, startCommand })); }
      ...
      await callDriver(profile, ["session", "cancel", ...daemon.daemonArgs, "--session-id", sessionId, "--cwd", projectRoot, "--journal", daemon.journal], { cwd: projectRoot, env: daemon.env });
      daemon = await startIsolatedWorkerDaemon({ profile, attempt, projectRoot, runtimeRoot, key });
