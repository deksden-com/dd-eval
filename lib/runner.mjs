      ...
      try {
        ...
        try {
          if (executionCleanupRequired(await readEvents(events), runId, execution, opId)) {
            await settleExecutionDaemon(cancelTree => callDriver(profile, ["daemon", "stop", ...daemonArgs, ...(cancelTree ? ["--cancel-tree"] : [])], { cwd: projectRoot, env: executionEnv }), subjectFailure);
          }
        }
        catch (cleanupError) {
...
async function withExecutionDaemon({ profile, attempt, projectRoot, runtimeRoot, journal, sessionId, action }) {
  ...
  try { return await action({ daemonArgs, env, daemon }); }
    ...
    if (recoveryBridge) {
      try { await settleExecutionDaemon(cancelTree => callDriver(profile, ["daemon", "stop", ...daemonArgs, ...(cancelTree ? ["--cancel-tree"] : [])], { cwd: projectRoot, env }), operationFailure); }
      catch (error) {
...
async function recoverLocked({ evalRoot, executionId = null, fromRecoveryId }) {
  ...
  for (const execution of selected) {
    ...
    try { receipt = await recordOperation({ eventsFile: path.join(root, "events.jsonl"), source: "dd-eval://runner", runId: manifest.run_id, executionId: execution.id, traceId: manifest.run_id, operationId, operation: `execution.${execution.id}.recover`, action: async () => {
      ...
      if (isObservationLoss(error) || ["operation_in_progress", "operation_terminal"].includes(error.code)) throw error;
