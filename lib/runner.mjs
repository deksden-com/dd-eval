  ...
  return await new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      if (code !== 0) {
        ...
        return reject(Object.assign(new Error(failure?.error ?? stderr.trim() ?? stdout.trim() ?? `driver exited ${code}`), { code: failure?.code ?? "driver_failed", ...(failure?.details ? { details: failure.details } : {}) }));
      }
...
async function executeEval({ runProfile, profile, loaded, validated, definition = null, root: suppliedRoot = null, runId: suppliedRunId = null, kind = "scored", executions: suppliedExecutions = null }) {
  ...
  const results = await mapLimited(manifest.executions, runProfile.value.concurrency.global, async (execution) => {
    ...
    try {
      ...
      let prompted; let lifecycle; const boundaries = []; const hitlRounds = new Map(); const hitl = []; const settledFanout = new Set();
      try {
        for (;;) {
