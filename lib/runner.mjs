...
async function canonicalResumeUnlocked({ buildRoot }) {
  ...
  if (state.status === "failed" && state.failure?.code === "driver_failed" && /(?:session\.prompt timed out|turn_(?:not_active|interrupted))/.test(String(state.failure.message ?? ""))) {
  ...
  const prepareHitlResume = async () => {
    ...
    const repeatedAcceptedAnswer = exchange.response_ids.length > 0 && exchange.response_ids.every((id) => answered.has(id));
    if (hitlRounds >= fixture.max_rounds && !samePendingPause && !repeatedAcceptedAnswer) return null;
    if (!samePendingPause && !repeatedAcceptedAnswer) {
      hitlRounds += 1;
    }
    state.reference = { ...state.reference, hitl_rounds: hitlRounds, pending_pause_id: record.pause.id, response_ids: [...answered, ...exchange.response_ids].sort() };
    ...
    await appendEvent(path.join(root, "build", "events.jsonl"), { source: "dd-eval://runner", runId: state.revision, type: repeatedAcceptedAnswer ? "dev.dd.eval.reference.hitl.replayed" : "dev.dd.eval.reference.hitl.matched", data: { stage, round: hitlRounds, pause_id: record.pause.id, response_ids: exchange.response_ids, judge_session_id: judged.session_id, receipt_file: judged.receipt_file, answer_file: answerFile, answer_sha256: sha256(answer) } });
    return prompt;
  };
  if (recovering) {
    ...
    const promptDurably = async (initialPrompt) => {
      ...
      for (let recovery = 0; ; recovery += 1) {
        try { return await promptWorker(prompt); }
          ...
          if (recovery >= 2 || !/(?:session\.prompt timed out|turn_(?:not_active|interrupted))/.test(reason)) throw error;
