...
async function canonicalResumeUnlocked({ buildRoot }) {
  ...
  if (state.status === "failed" && state.failure?.code === "driver_failed" && /(?:session\.prompt timed out|turn_not_active)/.test(String(state.failure.message ?? ""))) {
...
async function runFanoutWorker({ profile, attempt, projectRoot, runtimeRoot, work, eventsFile, event, resumeSessionId = null }) {
  ...
  try {
    ...
    let response;
    try { response = await promptWorker(resumeSessionId ? `Resume the already-started ${workId} Work. Read its authoritative durable Work prompt, complete only its unfinished result, invoke its exact standalone work finish command, then stop. Do not run work start, create a Work, or create a child agent.` : fanoutWorkerPrompt({ workId, startCommand })); }
    catch (error) {
      // An interrupted provider Turn can leave its private bridge polling
      // forever. Cancel only that Turn, restart only its daemon, and continue
      // the same registered Work from its durable prompt/result contract.
      await appendEvent(eventsFile, { ...event, type: "dev.dd.eval.fanout.worker.turn_recovery", data: { work_id: workId, session_id: sessionId, reason: error instanceof Error ? error.message : String(error) } });
      ...
      response = await promptWorker(`The previous Turn was interrupted. Read the existing authoritative Work prompt at ${promptPath}. Do not call work start or create another Work. Complete this same Work and use its exact work finish command.`);
    }
