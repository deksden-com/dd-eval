...
export async function evalJudge({ evalRoot, profileId = null }) {
  ...
  const candidate = await readJson(candidateFile); const receipt = await finalJudge({ root, runId: manifest.run_id, manifest, loaded, profileId: selected, candidate, results });
  await finalizeRunProjection({ root, manifest, loaded, results });
  return { root, receipt };
}
