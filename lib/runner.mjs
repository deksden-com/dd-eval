...
async function canonicalResumeUnlocked({ buildRoot }) {
  ...
  const prepareHitlResume = async () => {
    ...
    await appendEvent(path.join(root, "build", "events.jsonl"), { source: "dd-eval://runner", runId: state.revision, type: "dev.dd.eval.reference.hitl.matched", data: { stage, round: hitlRounds, pause_id: record.pause.id, response_ids: exchange.response_ids, judge_session_id: judged.session_id, receipt_file: judged.receipt_file, answer_file: answerFile, answer_sha256: sha256(answer) } });
    return prompt;
  };
  if (recovering) {
