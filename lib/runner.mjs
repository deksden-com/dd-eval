...
import { chmod, cp, mkdir, mkdtemp, open, readFile, readdir, realpath, rm, stat, symlink, writeFile, access, appendFile, rename } from "node:fs/promises";
...
import { isObservationLoss, isManagedWait, isConclusiveManagedFailure, errorRecord, reportedError } from "./operation-errors.mjs";
...
export async function readRegularFile(file) {
  // One descriptor for type check and read. Never wait for a FIFO writer, and
  // never disguise storage errors as malformed caller JSON. This reader also
  // serves retained state, so only the caller may declare a no-effect retry.
  const handle = await open(file, fsConstants.O_RDONLY | fsConstants.O_NONBLOCK);
  try {
    if (!(await handle.stat()).isFile()) fail(`Input must be a regular file: ${file}`, "input_file_not_regular");
...
export async function canonicalBuild({ profileFile, projectRoot, flowRoot }) {
  ...
  try {
    ...
    const subjectProfile = (await loadProfile(profile.value.subject.profile_id)).value;
    if (subjectProfile.harness === "antigravity-cli" || profile.value.subject.execution) {
      ...
      await assertCheckpointEngine(loaded.inputCheckpoint, await provisionRuntimeEngine(materializedInput.project_root, hookRuntime));
...
export async function resolveEvidenceJournals(result) {
  ...
  if (inventory?.schema_id === "dd-flow/run-observations@1" && path.isAbsolute(inventory.home ?? "")) {
  ...
  const attribution = modelAttribution(unique);
  if (journals.some(item => item.status !== "available" || item.model_observations?.status !== "available" || item.observation_failures?.length)) attribution.observation_completeness = "incomplete";
  const counters = inventory?.tools ?? result.statistics?.usage?.tool_calls ?? result.driver?.evidence?.tool_calls ?? null;
  const status = !journals.some(item => item.status === "available") ? "unavailable"
    : inventory?.tools?.completeness === "complete" && journals.every(item => item.status === "available") ? "complete" : "partial";
...
async function finalizeRunProjection({ root, manifest, loaded, results, permits = null, cleanupOnly = false }) {
  ...
  const pending = unsettled.some(Boolean) || capturePending || results.some((result) => ["awaiting_provider", "awaiting_native_children", "cancelling"].includes(result.state));
  ...
  const cleanupState = cleanupBlocked ? "blocked" : pending ? "pending" : "settled";
  const state = completed ? "completed" : recoveryBlocked ? "recovery_blocked" : pending ? "awaiting_provider" : executionSummary === "failed" ? "completed_with_failures" : executionSummary === "cancelled" ? "cancelled" : "awaiting_provider";
  let candidate = null; let judge = null; let judgeError = null;
  ...
  if (canFinalize && !cleanupOnly) {
    ...
    if (manifest.profile?.judge?.enabled) {
      ...
      catch (error) { judgeError = errorRecord(error); }
  ...
  const judgeStatus = judge ? "completed" : retainedJudgeStatus ?? (!manifest.profile?.judge?.enabled ? "not_requested" : cleanupOnly ? "not_run_cleanup_only" : pending ? "not_run_incomplete_execution" : judgeError ? (judgeError.code === "operation_in_progress" ? "in_progress" : "failed") : "in_progress");
  const published = await withRunnerLock(eventsFile, async () => {
    ...
    const report = buildReport({ root, manifest, state, executionState: executionSummary, cleanupState, results, candidate, judge, judgeStatus, judgeError, events: reportEvents });
...
export function failureEvidenceRevision(result) {
  return hashJson({
    ...
    hitl: (result.hitl ?? []).map(item => ({ stage: item.stage ?? null, round: item.round ?? null, pause_id: item.pause_id ?? null, answer_sha256: item.answer_sha256 ?? null })),
...
export async function captureRecoveryEvidence({ root, manifest, result }) {
  ...
  try {
    if (await exists(path.join(attempt, "managed-runtime.json"))) {
      ...
      if (status.worker?.status === "recovery_blocked" || status.worker?.error?.code === "recovery_observation_budget_exhausted") {
...
export function executionEvidence(result) {
  ...
  return {
    execution: result.execution,
    state: result.state,
    failure: result.state === "failed" ? { code: result.code ?? "execution_failed", message: result.error ?? null, attribution: failureAttribution(result) } : null,
    stage: result.stage ?? null,
    run_id: result.run_id ?? null,
...
export function failureAttribution(code) {
  ...
  if (["unexpected_hitl", "required_hitl_missing"].includes(code)) return "subject";
  return "undetermined";
}
...
function buildReport({ root, manifest, state, executionState = "running", cleanupState = "pending", results, candidate = null, judge = null, judgeStatus = "not_requested", judgeError = null, events = [] }) {
  const executions = results.map(executionEvidence);
  const history = recoveryHistory(events, manifest, results);
  const runValidity = results.some((result) => result.state === "failed" && isInfrastructureFailure(result)) ? "invalid_infrastructure_flow" : "valid";
  return {
    schema_id: "dd-eval/report@2", run_id: manifest.run_id, state, execution_state: executionState, cleanup_state: cleanupState, run_validity: runValidity,
    ...
    judge_status: judgeStatus, ...(judgeError ? { judge_error: judgeError } : {}), ...(candidate ? { candidate } : {}), ...(judge ? { judge } : {})
  };
}
...
export function interactionJudgePrompt(packetFile) {
  return `You are the Interaction Judge. Read ${JSON.stringify(packetFile)}. Decompose the Subject's text into its atomic material decisions; explanations, options and recommendations are context, not separate questions. Match semantic meaning regardless of wording, order or grouping. Proposed options are not exhaustive or binding: a canonical answer may resolve the decision with a different alternative. Do not require it to affirm a proposed option's assumptions or consequences. For example, if the Subject offers automatic or manual delivery, an answer specifying scheduled delivery can resolve the delivery-mode decision without selecting either option. However, an independent question about delivery time remains uncovered unless the answer explicitly states or logically entails it. Select only existing response IDs whose exact answer explicitly states or logically entails a listed covered decision. Descriptors help identify applicability but never add answer content. Never author, paraphrase or strengthen a response. Return matched only when every material decision is covered; then response_ids identify the answers to send. If some decisions are covered and some are not, return unmatched, list both covered_questions and uncovered_questions, and retain response_ids as evidence for the covered subset. If nothing is covered, response_ids must be empty. Reply with exactly one JSON object conforming to the required dd-eval/hitl-match@1 shape in the packet.`;
}
export async function interactionJudge({ runProfile, fixture, question, attempt, stage, subjectProfile, projectRoot, runtimeRoot, contextFile = null, permits = null, evalRunId = null, controller = null, resourceHome = null }) {
  ...
  const prompt = interactionJudgePrompt(packetFile);
...
export function isInfrastructureFailure(code) {
  ...
  if (code && typeof code === "object") return code.details?.lifecycle_outcome?.disposition === "fatal" || isInfrastructureFailure(code.code) || isInfrastructureFailure(code.cause) || isInfrastructureFailure(code.details?.cause);
...
async function cancelExecutionTree({ root, manifest, execution, profile, sessionId, launchUnwound = false }) {
  ...
  if (await exists(managedFile)) {
    ...
    try {
      ...
      // An explicit reconcile renews a failed observation budget once per ID.
      if (retained.worker?.status === "failed") return await commandJson(runtimeBin(runtimeRoot),
...
export async function harnessCapacityCheck({ profileId, maximum, projectRoot = process.cwd(), writeProfile = true, runtimeRoot: suppliedRuntimeRoot = null }) {
  ...
  try {
    ...
    promptReceipt = await providerTurn(profile, ["session", "prompt", ...daemon.daemonArgs, "--session-id", rootSessionId, "--cwd", project, "--model", profile.model, "--reasoning", profile.reasoning, "--prompt", capacityPrompt, "--journal", journal], { cwd: project, env: daemon.env });
...
export async function evalPreflight({ profileFile }) {
  ...
  const profileIds = [...new Set([profile.id, runProfile.value.interaction_judge?.profile_id, ...(runProfile.value.judge?.enabled ? [runProfile.value.judge.profile_id] : [])].filter(Boolean))];
  const profiles = await Promise.all(profileIds.map(async id => id === profile.id ? profile : (await loadProfile(id)).value));
  ...
  try {
    ...
    const receipt = { ok: true, root, definition, profile_file: runProfile.file, input_checkpoint: loaded.inputCheckpoint.value, engine: runtimeEngineIdentity(engine), baseline_admission: baselineAdmission, workspace_hooks: workspaceHooks, prepared_run: prepared, doctors, provider_sessions_created: 0 };
...
export async function evalRun({ profileFile, prepareOnly = false }) {
  ...
  return executeEval({ runProfile, profile, loaded, validated, definition, executions, prepareOnly });
}
...
export async function runnerFork({ evalRoot, executionId = "e2e", from, output, engineVersion, integrityChecksum = null, requestId, start = false }) {
  ...
  assertProfileCapacity(source.subject_profile, [derivedExecution]);
  ...
  const baselineEvidence = await verifyBaselineAdmission({ reference: baselineAdmission, definition: loaded.value.baseline_admission, checkpoint: loaded.inputCheckpoint });
  ...
  const receipt = await withRunnerLock(`${root}.lifecycle`, async () => {
    ...
    const fork = await commandJson(path.join(inheritedEngine.root, engine.entrypoint), ["run", "fork", "--from", snapshot, "--output", managedRoot, "--engine-version", engineVersion, "--request-id", requestId, ...(integrityChecksum ? ["--integrity-checksum", integrityChecksum] : [])], { cwd: root, env: runtimeEnv(toolsRoot) });
    ...
    await writeJsonAtomic(path.join(managedRoot, "managed-runtime.json"), { schema_id: "dd-eval/managed-runtime@1", run_id: fork.fork.run_id, project_root: fork.fork.project_root, runtime_root: fork.fork.dd_flow_home, runtime_budget: executionBudget(runId, source.profile) });
    ...
    await verifyBaselineAdmission({ reference: retainedBaseline, definition: loaded.value.baseline_admission, checkpoint: loaded.inputCheckpoint });
...
export async function assertExecutionEngine(manifest, inputCheckpoint, engine) {
  if (manifest.kind !== "derived") return assertCheckpointEngine(inputCheckpoint, engine);
  ...
  return assertCheckpointEngine({ value: { flow_pack: { engine: { version: pin.package_version, artifact_sha256: pin.integrity_checksum } } } }, engine);
}
...
async function preparedForkRun({ root, manifest, execution, loaded }) {
  ...
  await verifyBaselineAdmission({ reference: baseline, definition: loaded.value.baseline_admission, checkpoint: loaded.inputCheckpoint });
...
async function prepareEvalInputs({ runProfile, profile, loaded, validated, executions: suppliedExecutions, suppliedPack }) {
  ...
  return { executions, interactionFixtures, directBlueprint, blueprint, pack, packRoot, entryPackManifest, focusedEntries };
}

export async function executeEval({ runProfile, profile, loaded, validated, definition = null, root: suppliedRoot = null, runId: suppliedRunId = null, kind = "scored", executions: suppliedExecutions = null, prepareOnly = false, preparedInputs = null }) {
  ...
  const { executions, interactionFixtures, entryPackManifest, pack, packRoot } = prepared;
  await registerRunHome(root);
  await mkdir(path.dirname(root), { recursive: true });
...
export async function launchEvalExecution({ root, manifest, execution, loaded, blueprint, pack = null, packRoot = null, preparedEntry = null, permits, onInfrastructureFailure = () => {} }) {
    ...
    try {
      ...
      if (execution.mode === "e2e") {
        ...
        const admission = await callDriver(profile, ["doctor", "--cwd", projectRoot, "--model", profile.model, "--reasoning", profile.reasoning], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
...
export async function prepareRunnerContinuation(manifest, events = null) {
  ...
  if (events && manifest.executions.every(execution => execution?.id && terminalOperation(events, executionState(events, manifest.run_id, execution).operation_id)?.terminal)) return null;
  ...
  assertProfileCapacity(manifest.subject_profile, manifest.executions);
...
/** Cleanup has no queue/recovery/Judge dispatch. A request renews only existing
 * RUN controls; repeated observation reuses that same idempotency identity. */
export async function runnerCleanup({ evalRoot, requestId = null, expectedManifestSha256 = null }) {
  ...
  return withRunnerLock(`${root}.lifecycle`, async () => {
    ...
    if (!results.some(result => result.state === "failed")) fail("Cleanup requires a retained failed execution", "cleanup_not_required");
    if (requestId !== null) {
...
async function runnerResumeLocked({ evalRoot, resumeRequestId, expectedManifestSha256 }) {
  ...
  const prepared = await prepareRunnerContinuation(manifest, admitted);
  ...
  const runProfile = { value: manifest.profile };
  if (manifest.executions.every(execution => !terminalOperation(events, `${manifest.run_id}:${execution.id}:launch`)?.started)) {
    if (!prepared) { await assertRetainedRunDefinition(manifest, loaded); assertProfileCapacity(profile, manifest.executions); }
    return executePreparedQueue({ root, manifest, loaded, pack, packRoot, runProfile, prepared });
  }
  ...
  for (const execution of manifest.executions) {
    ...
    if (!operation?.started && operationId === `${manifest.run_id}:${execution.id}:launch`) {
      ...
      if (!permits) {
        if (!prepared) { assertProfileCapacity(profile, manifest.executions); await assertInteractionJudgePreflight({ caseRoot: loaded.root, executions: manifest.executions, runProfile }); }
    ...
    try { recovery = await recoverExecution({ root, events, manifest, execution, loaded, blueprint, profile }); }
      ...
      if (!isObservationLoss(error) && !isManagedWait(error) && !isConclusiveManagedFailure(error)) throw error;
      ...
      recovery = { execution: execution.id, execution_operation_id: operationId, attempt: path.join(root, "executions", execution.id), stage: execution.stage, state: "failed", ...errorRecord(error), error: error.message, driver: { controller: error.details?.controller ?? null }, finished_at: now() };
...
export async function runnerStatus({ evalRoot }) {
  ...
  return { root, ...projection, ...(controlStatus ? { state: "stopped", control_status: controlStatus } : {}), manifest, live, runner_attempts: await evalRunnerAttemptsStatus(root), observation: { last_event_at: events.at(-1)?.time ?? null, last_sequence: events.at(-1)?.data.sequence ?? null, pending_operations: Object.entries(projection.operations).filter(([, operation]) => operation.started && !operation.terminal).map(([id]) => id) }, execution_results: await attachModelAttribution(root, manifest, results) };
}
