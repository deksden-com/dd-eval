...
export async function canonicalResume({ buildRoot }) {
  ...
  const prepareHitlResume = async () => {
    ...
    const judgment = await interactionJudge({ runProfile, fixture, question, attempt: path.join(root, "reference"), stage, subjectProfile: profile, projectRoot: roots.projectRoot, runtimeRoot: roots.runtimeRoot, contextFile });
    ...
    hitlRounds += 1; state.reference = { ...state.reference, hitl_rounds: hitlRounds };
  ...
  if (recovering) {
    ...
    if (lifecycle.stage_status === "done") turnPrompt = null;
      ...
      const fanout = await driveFanout({ profile, attempt: path.join(root, "reference"), projectRoot: roots.projectRoot, runtimeRoot: roots.runtimeRoot, runId: lifecycle.run_id, stage, eventsFile: path.join(root, "build", "events.jsonl"), event: { source: "dd-eval://runner", runId: state.revision } });
...
async function registerJudgedTerminalQuestion({ root, revision, loaded, blueprint, runProfile, profile, stage, prompted, lifecycle, roots, contextFile }) {
  ...
  const fixture = await interactionFixture(loaded.root, stage);
  if (fixture.mode !== "required") return lifecycle;
  ...
  return reconcileFlow({ projectRoot: roots.projectRoot, runtimeRoot: roots.runtimeRoot, expectedStage: stage, runId: lifecycle.run_id });
}
...
export function entryLauncher({ stage, entry, projectRoot, runtimeRoot, contextFile, contextSha256, profile }) {
  ...
  return [
    ...
    `\`${command}\``,
    "Use the returned authoritative prompt and perform only this Stage. If `stage start` returns an `orchestration` object with kind `work_fanout`, stop immediately after that lifecycle command: the runner will dispatch only the already-declared child Work and will return this same coordinator Session when they settle. If it needs a material user answer, run the exact `stage pause` lifecycle command from that prompt before showing the question, then stop the Turn. Otherwise finish this Stage, then stop the Turn.",
...
function fanoutWorkerPrompt({ workId, startCommand }) {
  return [
...
async function runCapacityProbe({ profile, attempt, projectRoot, runtimeRoot, probe, eventsFile, event }) {
  ...
  const launches = [...Array(size)].map((_, index) => (async () => {
    ...
    try {
      ...
      const response = await callDriver(profile, ["session", "prompt", ...daemon.daemonArgs, "--session-id", sessionId, "--cwd", projectRoot, "--model", profile.model, "--reasoning", profile.reasoning, "--journal", daemon.journal, "--timeout", String(deadline)], { cwd: projectRoot, env: daemon.env });
...
async function runFanoutWorker({ profile, attempt, projectRoot, runtimeRoot, work, eventsFile, event }) {
  ...
  try {
    ...
    const response = await callDriver(profile, ["session", "prompt", ...daemon.daemonArgs, "--session-id", sessionId, "--cwd", projectRoot, "--model", profile.model, "--reasoning", profile.reasoning, "--prompt", fanoutWorkerPrompt({ workId, startCommand }), "--journal", daemon.journal], { cwd: projectRoot, env: daemon.env });
    const bin = process.env.DD_FLOW_BIN ?? "dd-flow";
    const observed = await commandJson(bin, ["work", "show", workId, "--project-root", projectRoot], { cwd: projectRoot, env: { DD_FLOW_HOME: runtimeRoot } });
    if (observed.work?.status !== "completed") fail(`fan-out worker ${workId} ended without work finish`, "incomplete_worker_turn");
    await appendEvent(eventsFile, { ...event, type: "dev.dd.eval.fanout.worker.completed", data: { work_id: workId, session_id: sessionId } });
    return { work_id: workId, session_id: sessionId, response };
...
async function driveFanout({ profile, attempt, projectRoot, runtimeRoot, runId, stage, eventsFile, event }) {
  ...
  for (;;) {
    ...
    if (fanout.capacity_required && (!Number.isInteger(capacity) || capacity < 1)) fail(`fan-out ${stage} has no available worker capacity`, "subagent_capacity_unavailable");
