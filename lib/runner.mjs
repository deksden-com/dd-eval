...
async function finalizeRunProjection({ root, manifest, loaded, results, permits = null, cleanupOnly = false }) {
  ...
  const cleanupState = pending ? "pending" : "settled";
  const state = completed ? "completed" : pending ? "awaiting_provider" : executionSummary === "failed" ? "completed_with_failures" : executionSummary === "cancelled" ? "cancelled" : "awaiting_provider";
  let candidate = null; let judge = null; let judgeError = null;
  let retainedJudgeStatus = null;
  if (cleanupOnly && !pending && await exists(path.join(root, "reports", "report.json"))) {
...
export async function runnerStatus({ evalRoot }) {
  ...
  return { root, ...projection, manifest, live, runner_attempts: await evalRunnerAttemptsStatus(root), observation: { last_event_at: events.at(-1)?.time ?? null, last_sequence: events.at(-1)?.data.sequence ?? null, pending_operations: Object.entries(projection.operations).filter(([, operation]) => operation.started && !operation.terminal).map(([id]) => id) }, execution_results: await attachModelAttribution(root, manifest, storedExecutionResults(events, manifest)) };
}
