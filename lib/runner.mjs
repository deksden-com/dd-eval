...
async function providerTurn(profile, args, options, permits = null) {
  const invoke = async () => {
    ...
    const prompt = callDriver(profile, bounded, options);
    const stateDir = argValue(bounded, "--state-dir");
    let watchdog = null, stopWatchdog = null;
    if (profile.harness === "antigravity-cli" && bounded[0] === "session" && bounded[1] === "prompt" && stateDir) {
      let timer;
      watchdog = new Promise((_, reject) => {
        timer = setInterval(async () => {
          try {
            const status = await callDriver(profile, ["daemon", "status", "--state-dir", stateDir], options);
            const last = Date.parse(status.last_activity_at ?? "");
            if (Number.isFinite(last) && Date.now() - last > 600_000) {
              await callDriver(profile, ["session", "cancel", "--state-dir", stateDir, "--session-id", argValue(bounded, "--session-id")], options).catch(() => {});
              reject(Object.assign(new Error("AGY prompt exceeded its native-activity window"), { code: "subject_liveness_timeout", details: { last_activity_at: status.last_activity_at } }));
            }
          } catch { /* the prompt transport remains authoritative */ }
        }, 30_000);
      });
      stopWatchdog = () => clearInterval(timer);
    }
    const receipt = watchdog ? await Promise.race([prompt, watchdog]).finally(stopWatchdog) : await prompt;
  ...
  const results = await mapLimited(manifest.executions, runProfile.value.concurrency.global, async (execution) => {
    ...
    try {
      ...
      try {
        ...
        const cancelTree = ["daemon_timeout", "subject_liveness_timeout"].includes(subjectFailure?.code) && profile.harness === "antigravity-cli";
      ...
      if (["daemon_timeout", "subject_liveness_timeout"].includes(error?.code) && profile.harness === "antigravity-cli") {
