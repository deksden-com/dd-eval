...
async function promptExistingSession({ profile, attempt, projectRoot, runtimeRoot, sessionId, prompt, journal }) {
  return await withExecutionDaemon({ profile, attempt, projectRoot, runtimeRoot, journal, sessionId, action: async ({ daemonArgs, env }) => await providerTurn(profile, ["session", "prompt", ...daemonArgs, "--session-id", sessionId, "--cwd", projectRoot, "--model", profile.model, "--reasoning", profile.reasoning, "--prompt", prompt, "--journal", journal], { cwd: projectRoot, env }) });
}
...
async function recoverExecution({ root, events, manifest, execution, loaded, blueprint, profile, recovery = null, terminalOnly = false }) {
  ...
  if (recovery && ["running", "paused"].includes(record?.status)) {
    await promptExistingSession({ profile, attempt, projectRoot, runtimeRoot, sessionId, prompt: recoveryPrompt({ recovery, stage: currentStage, paused: record.status === "paused" }), journal: path.join(attempt, "drivers", "subject.events.jsonl") });
