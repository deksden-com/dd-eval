...
async function runZcodeCapacityProbe({ profile, attempt, projectRoot, probe, eventsFile, event }) {
  ...
  try {
    ...
    const results = launches.map((launch) => {
      ...
      return { key: launch.key, session_id: launch.session_id, completed: result?.assistant_text?.trim() === expected, ...(result?.assistant_text ? { assistant_text: result.assistant_text } : {}), ...(result?.error ? { error: result.error } : response?.error ? { error: response.error } : {}) };
    });
