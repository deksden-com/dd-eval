...
// The evaluation runtime is not a product workspace. A subject may inspect it,
// but it must not repair flow state by deleting artifacts or importing private
// engine modules. Public dd-flow commands are the only mutation boundary.
async function auditRuntimeIntegrity({ journal, runtimeRoot, output }) {
  ...
  for (const event of await readJsonLines(journal, "driver journal")) collectCommands(event, commands);
  const violations = commands.filter((command) => {
    if (!command.includes(runtimeRoot)) return false;
    if (/\b(?:rm|mv|cp|sed|perl|python(?:3)?|node)\b/.test(command)) return true;
    return /ensureVnextMergeRequest|vnext-[\w-]+\.js/.test(command) && !/\/bin\/dd-flow\b/.test(command);
  });
  const receipt = { schema_id: "dd-eval/runtime-integrity@1", journal, runtime_root: runtimeRoot, checked_at: now(), status: violations.length ? "failed" : "passed", violations };
