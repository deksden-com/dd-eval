...
async function callDriver(profile, args, options) {
  ...
  if (stateIndex >= 0 && args[0] === "session" && ["create", "start", "prompt", "fork"].includes(args[1])) {
    await writeJsonAtomic(path.join(command[stateIndex + 1], "client-operations", `${sha256(operationId)}.json`), { operation_id: operationId, command: args.slice(0, 2), requested_at: now(), request_sha256: hashJson(command) });
  }
