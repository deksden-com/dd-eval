...
export async function evalRun({ profileFile }) {
  ...
  const loaded = await loadCaseV6(runProfile.value.case_id);
  const definition = await committedDefinitionIdentity();
  ...
  if (needsEntryPack && loaded.value.status !== "runnable") fail("focused or segment execution requires a runnable case with an accepted entry pack");
...
export async function runnerResume({ evalRoot }) {
  const root = path.resolve(evalRoot); const manifest = await readJson(path.join(root, "manifest.json")); const loaded = await loadCaseV6(manifest.case_id); const profile = manifest.subject_profile;
  let blueprint;
