...
async function driveFanout({ profile, attempt, projectRoot, runtimeRoot, runId, stage, eventsFile, event, nativeReceipt, sessionId }) {
  let status = await fanoutStatus({ projectRoot, runtimeRoot, runId, stage });
  if (!status.orchestration) return null;
  if (nativeReceipt && sessionId) {
    const children = directNativeChildren(nativeReceipt, sessionId);
    if (children.length) {
...
async function executeEval({ runProfile, profile, loaded, validated, definition = null, root: suppliedRoot = null, runId: suppliedRunId = null, kind = "scored", executions: suppliedExecutions = null }) {
  ...
  const results = await mapLimited(manifest.executions, runProfile.value.concurrency.global, async (execution) => {
    ...
    try {
      ...
      try {
        for (;;) {
          ...
          if (lifecycle.stage_status === "running") {
            const fanout = await driveFanout({ profile, attempt, projectRoot, runtimeRoot, runId: lifecycle.run_id, stage: currentStage, eventsFile: events, event: { source: "dd-eval://runner", runId, executionId: execution.id, traceId: runId }, nativeReceipt: prompted, sessionId });
      ...
      const evidence = { attempt: failureAttempt, session_id: subjectSessionFor(observedEvents, execution.id), stage: observedEvents.findLast((event) => event.executionid === execution.id && event.type === "dev.dd.eval.execution.context_prepared")?.data?.stage ?? execution.stage, lifecycle: error.lifecycle ?? null };
      ...
      const failure = { ...evidence, execution: execution.id, state: "failed", ...errorRecord(error), error: error instanceof Error ? error.message : String(error), ...(error?.hitl ? { hitl: [error.hitl] } : {}), started_at: startedAt, finished_at: now() };
      await appendEvent(events, { source: "dd-eval://runner", runId, executionId: execution.id, traceId: runId, type: "dev.dd.eval.execution.failed", data: { state: "failed", ...failure } });
