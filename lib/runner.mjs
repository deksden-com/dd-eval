...
export function recoverySourceFromEvents(events, executionId, fromRecoveryId) {
  const matches = events.filter(event => event.type === "dev.dd.eval.execution.failed"
    && (!executionId || event.executionid === executionId)
    && event.data?.recovery?.recovery_id === fromRecoveryId)
...
export function recoveryOperationId(events, runId, executionId, recoveryId) {
  ...
  const operations = reduceEvents(events).operations, current = operations[base];
  if (!current?.terminal || current.terminal === "completed") return base;
  const retries = Object.keys(operations).filter(id => new RegExp(`^${base.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}:retry:[1-9][0-9]*$`).test(id));
  return `${base}:retry:${retries.length + 1}`;
}
