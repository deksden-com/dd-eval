...
const recoverableDriverCodes = new Set(["turn_timeout", "turn_not_active", "turn_interrupted", "daemon_timeout", "rpc_timeout"]);
const isRecoverableDriverError = (error) => recoverableDriverCodes.has(error?.code);
...
export function workerPromptPath(work, profile, sessionId) {
  const registeredId = profile.harness === "codex-desktop" ? sessionId : `${profile.harness}:${sessionId}`;
  return work?.sessions?.find((item) => item.session_id === registeredId)?.prompt_path;
}
