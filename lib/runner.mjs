...
export async function fixturesValidate({ caseId, revision }) {
  ...
  const entries = {};
  for (const key of ["e2e", ...pack.flow.contour]) {
...
async function canonicalCandidatePack(root, state, blueprint) {
  const contour = (await loadCaseV6(state.case_id)).value.flow.contour;
  const required = ["e2e", ...contour];
  ...
  const pack = await writeEntryPack({ caseDir: path.join(root, state.case_id), revision: state.revision, inputCheckpoint: { id: inputCheckpoint.id, sha256: inputCheckpoint.sha256 }, flow: { contour, terminal_stage: contour.at(-1), flow_commit: state.materialized_input?.flow_commit ?? null, flow_manifest_sha256: state.materialized_input?.flow_manifest_sha256 ?? null }, stageBlueprint: blueprint, entries, e2e, authoring: { profile_id: state.profile, build_trace_sha256: await manifestHash(buildTrace) } });
  pack.entries = Object.fromEntries(["e2e", ...contour].map((key) => [key, state.entries[key]]));
...
async function registerJudgedTerminalQuestion({ root, revision, loaded, blueprint, runProfile, profile, stage, prompted, lifecycle, roots, contextFile }) {
  ...
  const bin = process.env.DD_FLOW_BIN ?? "dd-flow";
  await commandJson(bin, ["stage", "pause", lifecycle.run_id, "--stage", stage, "--work", workId, "--question-stdin", "--project-root", roots.projectRoot], { cwd: roots.projectRoot, env: { DD_FLOW_HOME: roots.runtimeRoot }, input: prompted.assistant_text });
...
export async function runServerMerge({ profile, projectRoot, runtimeRoot, runId, env = {}, onProgress = null }) {
  ...
  const agent = await materializeMergeAgentProfile({ profile, runtimeRoot }); const bin = process.env.DD_FLOW_BIN ?? "dd-flow";
  const receipt = await commandJson(bin, ["merge", "serve", "--agent-profile", agent.id, "--once", "--max-parallel-projects", "1", "--progress-jsonl"], {
    cwd: projectRoot, env: { ...env, DD_FLOW_HOME: runtimeRoot, ...agent.env }, onProgress
  });
...
async function captureCandidate({ projectRoot, runtimeRoot, runId, attempt, stage }) {
  ...
  if (!(await exists(manifestFile))) {
    try {
      created = await commandJson(bin, ["run", "snapshot", "create", runId, ...modeArgs, "--project-root", projectRoot, "--output", output], { cwd: projectRoot, env: { DD_FLOW_HOME: runtimeRoot } });
    } catch (error) {
...
export function workerUsageSource(profile) {
  ...
  return profile?.harness === "zcode-acp" ? "adapter_ingested" : "session_sync";
}
...
async function runFanoutWorker({ profile, attempt, projectRoot, runtimeRoot, work, eventsFile, event }) {
  ...
  try {
    ...
    const usage = workerUsageSource(profile) === "adapter_ingested"
      ...
      : await commandJson(bin, ["session", "usage", "sync", "--project-root", projectRoot, "--session-id", sessionId], { cwd: projectRoot, env: { DD_FLOW_HOME: runtimeRoot } });
...
async function withExecutionDaemon({ profile, attempt, projectRoot, runtimeRoot, journal, action }) {
  // A normal execution daemon records a clean terminal state when its owning
  // controller exits. Reconnection must therefore use a fresh disposable
  // bridge state while addressing the same provider Session.
  const daemonState = path.join(attempt, "drivers", "recovery", randomUUID()); const daemonArgs = ["--state-dir", daemonState];
