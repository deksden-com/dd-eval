...
import { chmod, cp, mkdir, open, readFile, readdir, rm, stat, symlink, writeFile, access, appendFile, rename } from "node:fs/promises";
...
import { reconcileDriverReplies, recoverDriverReply, assertDaemonReplaceable, recoverySettlement } from "./driver-recovery.mjs";
...
async function startReferenceDaemon(profile, roots, root) {
  ...
  try { await callDriver(profile, args, { cwd: roots.projectRoot, env }); }
    ...
    if (String(error?.message ?? "").includes("daemon_state_terminal")) {
      ...
      await reconcileDriverReplies(daemonState);
      await assertDaemonReplaceable(daemonState);
      await rename(daemonState, `${daemonState}.history-${randomUUID()}`);
...
async function canonicalResumeUnlocked({ buildRoot, detachTurns = false }) {
  ...
  const contextFile = path.join(root, "reference", "stage-context", `${stage}-${randomUUID()}.json`);
  ...
  const launcherFile = path.join(root, "reference", "launchers", `${path.basename(contextFile, ".json")}.md`);
  await writeFile(launcherFile, `${launcher}\n`);
  await appendEvent(path.join(root, "build", "events.jsonl"), { source: "dd-eval://runner", runId: state.revision, type: "dev.dd.eval.reference.context_prepared", data: { stage, context_file: contextFile, launcher_file: launcherFile, materialized_context_sha256: contextSha256 } });
  ...
  const driveReferenceFanout = async () => {
    try {
      return await driveFanout({ profile, attempt: path.join(root, "reference"), projectRoot: roots.projectRoot, runtimeRoot: roots.runtimeRoot, runId: lifecycle.run_id, stage, eventsFile: path.join(root, "build", "events.jsonl"), event: { source: "dd-eval://runner", runId: state.revision }, nativeReceipt: prompted, sessionId });
    } catch (error) {
  ...
  const prepareHitlResume = async () => {
    ...
    if (answeredPause) {
      ...
      const prompt = await resumePrompt({ lifecycle, stage, question, answerFile: answeredPause.answer_file, answerSha256: answeredPause.answer_sha256, runtimeRoot: roots.runtimeRoot, projectRoot: roots.projectRoot });
    ...
    const prompt = await resumePrompt({ lifecycle, stage, question, answerFile, answerSha256, runtimeRoot: roots.runtimeRoot, projectRoot: roots.projectRoot });
...
function harnessConfigKey(harness) { return ({ "codex-desktop": "codex-desktop", "zcode-acp": "zcode-acp", "grok-acp": "grok-acp", "antigravity-cli": "antigravity-cli", "opencode-server": "opencode-server", "droid-cli": "droid-cli", "droid": "droid-cli" })[harness] ?? fail(`Unsupported harness: ${harness}`, "harness_config_invalid"); }
...
export async function driverAdapterInvocation(profile, { cwd, env = {} } = {}) {
  const configHome = env.DD_FLOW_CONFIG_HOME ?? env.DD_FLOW_HOME;
  if (typeof configHome === "string" && path.isAbsolute(configHome)) {
    const bundled = path.join(configHome, "harness-runtime", "bin", driverFor(profile));
    if (await exists(bundled)) return { executable: process.execPath, prefix: [bundled] };
  }
  if (typeof configHome === "string" && path.isAbsolute(configHome)) {
    const config = await readJson(path.join(configHome, "harnesses.json"));
    const key = harnessConfigKey(profile.harness); const entry = config?.harnesses?.[key];
...
export async function callDriver(profile, args, options) {
  ...
  const invoke = () => new Promise((resolve, reject) => {
    const child = spawn(adapter.executable, [...adapter.prefix, ...command, "--json"], { cwd: options.cwd, env: { ...process.env, ...options.env, DD_EVAL_OPERATION_ID: operationId }, stdio: ["ignore", "pipe", "pipe"] }); let stdout = ""; let stderr = "";
...
async function captureCandidate({ projectRoot, runtimeRoot, runId, attempt, stage, output = path.join(attempt, "candidate") }) {
  const bin = "dd-flow";
  ...
  const successor = continuationStage(lifecycle, stage);
  const expected = successor ? { purpose: "stage_entry", stage_entry: successor } : { purpose: "candidate", stage_entry: null };
...
async function captureExecutionCandidate({ projectRoot, runtimeRoot, runId, attempt, stage }) {
  ...
  statistics.observation = await observationSummary(path.join(attempt, "drivers", "subject.events.jsonl"));
  await writeJsonAtomic(path.join(attempt, "statistics.json"), statistics);
  return { candidate, statistics };
}
async function captureStageBoundary({ root, manifest, execution, lifecycle, attempt, projectRoot, runtimeRoot, stage }) {
  const record = stageRecord(lifecycle, stage);
  if (record?.status !== "done") fail("stage boundary requires a completed stage", "stage_boundary_incomplete");
  const boundaryKey = sha256(canonicalJson({ run_id: lifecycle.run_id, stage, attempt: record.attempt ?? null, started_at: record.started_at ?? null, completed_at: record.completed_at ?? null }));
  const eventsFile = path.join(root, "events.jsonl");
  const prior = (await readEvents(eventsFile)).find(event => event.executionid === execution.id && event.type === "dev.dd.eval.stage.boundary_captured" && event.data?.boundary_key === boundaryKey);
  if (prior) return prior.data;
  const checkpoint = await captureCandidate({ projectRoot, runtimeRoot, runId: lifecycle.run_id, attempt, stage, output: path.join(attempt, "boundaries", `${stage}-${boundaryKey}`) });
  const data = { stage, run_id: lifecycle.run_id, boundary_key: boundaryKey, checkpoint };
  await appendEvent(eventsFile, { source: "dd-eval://runner", runId: manifest.run_id, executionId: execution.id, traceId: manifest.run_id, type: "dev.dd.eval.stage.boundary_captured", data, beforeAppend: events => !events.some(event => event.executionid === execution.id && event.type === "dev.dd.eval.stage.boundary_captured" && event.data?.boundary_key === boundaryKey) });
  return data;
}
...
async function appendRunEventOnce({ eventsFile, runId, type, data, guard = () => true }) {
...
async function finalizeRunProjection({ root, manifest, loaded, results, permits = null }) {
  ...
  for (const result of results) {
    ...
    if (!result.recovery?.recovery_id && !result.incomplete_evidence) await captureIncompleteEvidence(result);
  ...
  results = storedExecutionResults(projectedEvents, manifest);
  const resultHash = hashJson(results);
  ...
  if (!pending) {
    const frozen = await withRunnerLock(`${root}.candidate`, () => withRunnerLock(eventsFile, async () => {
      const current = storedExecutionResults(await readEvents(eventsFile), manifest);
      if (hashJson(current) !== resultHash) return null;
  ...
  if (state !== "awaiting_provider") await appendRunEventOnce({ eventsFile, runId: manifest.run_id, type: "dev.dd.eval.completed", guard: prior => hashJson(storedExecutionResults(prior, manifest)) === resultHash, data: { state, executions: results.map((result) => ({ execution: result.execution, state: result.state })) } });
  ...
  const published = await withRunnerLock(eventsFile, async () => {
    ...
    if (hashJson(storedExecutionResults(reportEvents, manifest)) !== resultHash) return null;
...
async function captureIncompleteEvidence(evidence) {
  const projectRoot = path.join(evidence.attempt, "project"); const runtimeRoot = path.join(evidence.attempt, "dd-flow-home");
  const snapshotRoot = path.join(evidence.attempt, "failure-evidence", randomUUID());
  try {
    const lifecycle = await reconcileFlow({ projectRoot, runtimeRoot, expectedStage: evidence.stage, runId: null });
    evidence.run_id = lifecycle.run_id; evidence.lifecycle = lifecycle;
    const captured = await commandJson("dd-flow", ["run", "snapshot", "create", lifecycle.run_id, "--incomplete", "--project-root", projectRoot, "--output", snapshotRoot], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
    const manifestFile = path.join(snapshotRoot, "snapshot.json");
    evidence.incomplete_evidence = { snapshot: snapshotRoot, manifest: manifestFile, manifest_sha256: sha256(await readFile(manifestFile)), consistency: captured.consistency, restorable: false };
  } catch (captureError) {
    evidence.incomplete_evidence = { snapshot: null, missing: "Frozen RUN snapshot unavailable", failure: errorRecord(captureError) };
  }
}
...
async function captureRecoveryEvidence({ root, manifest, result }) {
  const attempt = result.attempt ?? path.join(root, "executions", result.execution);
  ...
  if (!isInfrastructureFailure(result.code) || !(await exists(projectRoot)) || !(await exists(runtimeRoot))) return null;
  try {
    const lifecycle = result.lifecycle ?? await reconcileFlow({ projectRoot, runtimeRoot, expectedStage: result.stage, runId: result.run_id ?? null });
    const runId = lifecycle.run_id;
    if (typeof runId !== "string") return null;
    const began = await commandJson("dd-flow", ["run", "recovery", "begin", runId, "--project-root", projectRoot, "--interruption-json", JSON.stringify(interruption)], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
    const recovery = began.recovery; const recoveryId = recovery?.recovery_id;
    if (typeof recoveryId !== "string") fail("dd-flow recovery begin returned no recovery id", "recovery_capture_invalid");
    const settlement = await recoverySettlement(path.join(attempt, "drivers"));
    const sealed = await commandJson("dd-flow", ["run", "recovery", "seal", runId, "--project-root", projectRoot, "--recovery-id", recoveryId, "--settlement-json", JSON.stringify(settlement)], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
    const output = path.join(attempt, "recovery-evidence", recoveryId);
    const snapshot = await exists(path.join(output, "snapshot.json"))
      ? await readJson(path.join(output, "snapshot.json"))
      : await commandJson("dd-flow", ["run", "snapshot", "create", runId, "--recovery-id", recoveryId, "--project-root", projectRoot, "--output", output], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
    await commandJson("dd-flow", ["run", "recovery", "capture", runId, "--project-root", projectRoot, "--recovery-id", recoveryId, "--capture-path", output], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
    const manifestFile = path.join(output, "snapshot.json");
    return { recovery_id: recoveryId, generation: recovery.generation ?? null, run_id: runId, interruption: recovery.interruption ?? (recovery.interruption_json ? JSON.parse(recovery.interruption_json) : interruption), settlement: sealed.recovery?.settlement ?? settlement, snapshot: output, manifest: manifestFile, manifest_sha256: sha256(await readFile(manifestFile)), consistency: snapshot.consistency ?? "sealed_writer_barrier_required" };
  } catch (error) {
...
async function performFinalJudge({ root, runId, manifest, loaded, profileId, candidate: suppliedCandidate = null, permits = null, results = null }) {
  ...
  const journal = path.join(judgeRoot, "events.jsonl"); const daemonState = path.join(judgeRoot, "daemon"); const daemonArgs = ["--state-dir", daemonState]; const codexHome = path.join(judgeRoot, "codex-home"); const roots = await judgeHarnessRoots(root, results); const judgeEnv = { DD_FLOW_CONFIG_HOME: roots.runtimeRoot, ...(profile.harness === "codex-desktop" ? { CODEX_HOME: codexHome } : {}) };
...
async function interactionJudge({ runProfile, fixture, question, attempt, stage, subjectProfile, projectRoot, runtimeRoot, contextFile = null, permits = null }) {
  ...
  const journal = path.join(root, "events.jsonl"); const daemonState = path.join(root, "daemon"); const daemonArgs = ["--state-dir", daemonState]; const codexHome = path.join(root, "codex-home"); const judgeEnv = { DD_FLOW_CONFIG_HOME: runtimeRoot, ...(profile.harness === "codex-desktop" ? { CODEX_HOME: codexHome } : {}) };
...
export function requiresTreeCancellation(code) {
  return ["subject_liveness_timeout", "profile_integrity_violation", "profile_drift", "profile_mismatch", "agy_profile_drift", "session_identity_mismatch", "foreign_session", "droid_child_identity_invalid", "droid_hook_identity_invalid", "droid_turn_identity_mismatch", "agy_terminal_result_missing"].includes(code);
}

export function recoveryBridgeStopArgs(daemonArgs, failureCode) {
  return ["daemon", "stop", ...daemonArgs, ...(requiresTreeCancellation(failureCode) ? ["--cancel-tree"] : [])];
}
...
async function executionDispatchBarrier(operation) {
  ...
  await appendEvent(context.eventsFile, { source: "dd-eval://runner", runId: context.runId, executionId: context.executionId,
    ...
    beforeAppend: events => assertExecutionDispatch(executionState(events, manifest.run_id, execution), context.operationId) });
}
...
async function cancelExecutionTree({ root, manifest, execution, profile, sessionId, launchUnwound = false }) {
  ...
  return cancelOwnedDaemon({ stateDir, projectRoot, sessionId,
    stop: () => callDriver(profile, ["daemon", "stop", "--state-dir", stateDir, "--cancel-tree"], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) }) });
}
...
function failedChildPrompt({ stage }) {
  return [
    `One or more ${stage} child Work items reached a terminal non-success state.`,
    "Do not wait indefinitely, relaunch a sibling, or mark the Stage complete. Read the Work graph and retained evidence, then choose one legal action: diagnose and retry an explicitly failed Work, create a scoped repair Work, or block this Stage with the concrete failure reason.",
    "A child that ended without work finish is not success: preserve its evidence and resolve its Work explicitly before continuing the coordinator."
  ].join("\n");
}
...
export async function capacityCodexChildren(codexHome, rootSessionId) {
  ...
  for (const file of files) {
    ...
    // The native leaf transcript is authoritative even when the parent has no
    // legacy SubAgentActivity item. Use its last lifecycle event, not its text.
...
export async function harnessCapacityCheck({ profileId, maximum, projectRoot = process.cwd(), writeProfile = true }) {
  ...
  const journal = path.join(attempt, "events.jsonl"); const daemon = { daemonArgs: ["--state-dir", path.join(attempt, "daemon"), ...(["grok-acp", "antigravity-cli", "opencode-server", "droid-cli", "droid"].includes(profile.harness) ? ["--no-flow"] : [])], journal, env: profile.harness === "codex-desktop" ? { CODEX_HOME: path.join(attempt, "codex-home") } : {} };
...
async function driveFanout({ profile, attempt, projectRoot, runtimeRoot, runId, stage, eventsFile, event, nativeChildren = [], sessionId }) {
  let status = await fanoutStatus({ projectRoot, runtimeRoot, runId, stage });
  if (!status.orchestration) return null;
  const actionableChildren = nativeChildren.filter(child => child.status !== "settled_by_root");
  // The Session transcript is cumulative. A fan-out stage must reconcile only
  // children first observed after that stage began, otherwise a valid
  // historical child is mistaken for a new review worker.
  if (actionableChildren.length && sessionId && (Number(status.orchestration.works?.created) || Number(status.orchestration.works?.running) || Number(status.orchestration.works?.completed))) {
    const children = actionableChildren;
    if (children.length) {
      const observationsFile = path.join(attempt, `native-children-${stage}-${randomUUID()}.json`);
      await writeJsonAtomic(observationsFile, { harness_id: profile.harness, parent_session_id: sessionId, children });
      const reconciled = await commandJson("dd-flow", ["stage", "fanout", "reconcile", runId, "--stage", stage, "--observations-file", observationsFile, "--project-root", projectRoot], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
      await appendEvent(eventsFile, { ...event, type: "dev.dd.eval.fanout.native_reconciled", data: { stage, observations_file: observationsFile, result: reconciled } });
      const issues = (reconciled.issues ?? []).filter(issue => issue.code !== "child_check_in_progress");
      if (issues.length) {
        const error = new Error(`Native child execution cannot be reconciled safely: ${issues.map(issue => issue.code).join(", ")}. Inspect retained observations and Work results; do not relaunch or declare success automatically.`);
        error.code = "fanout_reconciliation_required"; error.details = { stage, observations_file: observationsFile, issues };
        throw error;
      }
      if (children.some(child => ["running", "unknown"].includes(child.status))) {
        await appendEvent(eventsFile, { ...event, type: "dev.dd.eval.fanout.native_waiting", data: { stage, children } });
        return { state: "awaiting_native_children", status };
      }
  ...
  for (;;) {
    ...
    if ((Number(fanout.works?.failed) || Number(fanout.works?.cancelled)) > 0) {
      return { state: "failed_children", status, continuation: failedChildPrompt({ stage }) };
    }
...
/** Retain native-child violations at the stage that observed them. */
async function observeNativeChildren({ profile, attempt, projectRoot, runtimeRoot, flowRunId, stage, eventsFile, event, children, sessionId }) {
  children = children.filter(child => child.status !== "settled_by_root");
  if (!children.length || !sessionId) return;
  const observationsFile = path.join(attempt, `native-children-observed-${stage}-${randomUUID()}.json`);
  await writeJsonAtomic(observationsFile, { harness_id: profile.harness, parent_session_id: sessionId, children });
  const observed = await commandJson("dd-flow", ["stage", "native", "observe", flowRunId, "--stage", stage, "--observations-file", observationsFile, "--project-root", projectRoot], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
  await appendEvent(eventsFile, { ...event, type: "dev.dd.eval.native_children.observed", data: { stage, observations_file: observationsFile, result: observed } });
  const issues = Array.isArray(observed.issues) ? observed.issues : [];
  if (issues.length) {
    const error = new Error(`Native child delegation cannot be reconciled safely: ${issues.map((issue) => issue.code).join(", ")}. Inspect retained observations; do not relaunch or infer a Work binding.`);
    error.code = issues.some((issue) => issue.code === "unbound_native_delegation_detected") ? "unbound_native_delegation_detected" : "fanout_reconciliation_required";
    error.details = { stage, observations_file: observationsFile, issues };
    throw error;
  }
}
...
export async function evalPreflight({ profileFile }) {
  ...
  try {
    ...
    const receipt = { ok: true, root, definition, profile_file: runProfile.file, input_checkpoint: loaded.inputCheckpoint.value, engine: runtimeEngineIdentity(engine), baseline_admission: baselineAdmission, doctors, provider_sessions_created: 0 };
  ...
  const results = await mapLimited(manifest.executions, runProfile.value.concurrency.global, async (execution) => {
    ...
    try {
      ...
      if (execution.mode === "e2e") {
        ...
        restored.baseline_admission = await runBaselineAdmission({ caseRoot: loaded.root, definition: loaded.value.baseline_admission, projectRoot, outputRoot: path.join(attempt, "baseline-admission"), checkpoint: loaded.inputCheckpoint });
        await commandJson(runtimeBin(runtimeRoot), ["project", "register", "--root", projectRoot], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
        ...
        restored.baseline_admission = await runBaselineAdmission({ caseRoot: loaded.root, definition: loaded.value.baseline_admission, projectRoot: baselineProject, outputRoot: path.join(attempt, "baseline-admission"), checkpoint: loaded.inputCheckpoint });
      }
      ...
      let contextSha256 = sha256(await readFile(contextFile)); launcher = entryLauncher({ stage: currentStage, entry: currentEntry, projectRoot, runtimeRoot, contextFile, contextSha256, profile });
      ...
      await appendEvent(events, { source: "dd-eval://runner", runId, executionId: execution.id, traceId: runId, type: "dev.dd.eval.execution.context_prepared", data: { stage: currentStage, context_file: contextFile, baseline_admission: restored.baseline_admission ?? null, runtime_engine: runtimeEngineIdentity(restored.engine), semantic_package_sha256: slice.semantic_package_sha256, context_slice_sha256: slice.context_slice_sha256, materialized_context_sha256: contextSha256, launcher_file: path.join(attempt, "launchers", `${currentStage}.md`) } });
      ...
      const hitlRounds = new Map(); const settledFanout = new Set(); let subjectFailure = null;
      const observedNativeChildren = new Set(); let stageNativeBaseline = new Set();
      try {
        for (;;) {
          ...
          const currentNativeChildren = directNativeChildren(prompted, sessionId);
          const stageNativeChildren = nativeChildrenSince(currentNativeChildren, stageNativeBaseline);
          for (const child of currentNativeChildren) observedNativeChildren.add(child.session_id);
          lifecycle = await reconcileFlow({ projectRoot, runtimeRoot, expectedStage: currentStage, runId: currentEntry.snapshot.run_id });
          await observeNativeChildren({ profile, attempt, projectRoot, runtimeRoot, flowRunId: lifecycle.run_id, stage: currentStage, eventsFile: events, event: { source: "dd-eval://runner", runId, executionId: execution.id, traceId: runId }, children: stageNativeChildren, sessionId });
          if (lifecycle.stage_status === "running") {
            const fanout = await driveFanout({ profile, attempt, projectRoot, runtimeRoot, runId: lifecycle.run_id, stage: currentStage, eventsFile: events, event: { source: "dd-eval://runner", runId, executionId: execution.id, traceId: runId }, nativeChildren: stageNativeChildren, sessionId });
          ...
          if (lifecycle.stage_status === "paused") {
            ...
            launcher = await resumePrompt({ lifecycle, stage: currentStage, question, answerFile, answerSha256: sha256(answer), runtimeRoot, projectRoot });
          ...
          const repairContinuation = lifecycle.status?.continuation;
          const transitionedToRepair = repairContinuation?.kind === "continue_stage" && repairContinuation.stage !== currentStage;
          if ((!transitionedToRepair && lifecycle.stage_status !== "done") || !["e2e", "segment"].includes(execution.mode) || (!transitionedToRepair && currentStage === execution.terminal_stage)) break;
          ...
          if (!transitionedToRepair) {
            const boundary = await captureStageBoundary({ root, manifest, execution, lifecycle, attempt, projectRoot, runtimeRoot, stage: currentStage });
            boundaries.push({ ...boundary, context_slice_sha256: slice.context_slice_sha256, materialized_context_sha256: contextSha256 });
          }
          let successor = continuationStage(lifecycle, currentStage); if (!successor) break;
          if (stageExecutor(successor, lifecycle) === "merge_server") {
            ...
            await materializeTaskInput(loaded.root, blueprint, currentStage, projectRoot); contextFile = path.join(attempt, "stage-context", `${currentStage}-${randomUUID()}.json`);
            ...
            await appendEvent(events, { source: "dd-eval://runner", runId, executionId: execution.id, traceId: runId, type: "dev.dd.eval.execution.context_prepared", data: { stage: currentStage, context_file: contextFile, materialized_context_sha256: contextSha256, executor: "merge_server" } });
            ...
            if (!server) fail("merge-server did not return its lifecycle", "merge_server_incomplete");
            lifecycle = server.lifecycle; prompted = { merge_server: server.receipt };
            if (lifecycle.stage_status === "done") {
              launcher = null;
              await appendEvent(events, { source: "dd-eval://runner", runId, executionId: execution.id, traceId: runId, type: "dev.dd.eval.merge.server.completed", data: { stage: "merge", agent_profile_id: server.agent_profile_id, receipt: server.receipt } });
              break;
            }
            successor = continuationStage(lifecycle, "merge");
            if (successor !== "code") fail("merge-server returned without completion or source repair", "merge_server_incomplete");
            await appendEvent(events, { source: "dd-eval://runner", runId, executionId: execution.id, traceId: runId, type: "dev.dd.eval.merge.server.source_repair", data: { stage: "merge", next_stage: successor, receipt: server.receipt } });
          }
          ...
          stageNativeBaseline = new Set(observedNativeChildren);
          currentEntry = { snapshot: { run_id: lifecycle.run_id } };
          await materializeTaskInput(loaded.root, blueprint, currentStage, projectRoot);
          contextFile = path.join(attempt, "stage-context", `${currentStage}-${randomUUID()}.json`);
          ...
          const launcherFile = path.join("launchers", `${path.basename(contextFile, ".json")}.md`);
          await writeFile(path.join(attempt, launcherFile), `${launcher}\n`);
          await appendEvent(events, { source: "dd-eval://runner", runId, executionId: execution.id, traceId: runId, type: "dev.dd.eval.execution.context_prepared", data: { stage: currentStage, attempt: lifecycle.status?.continuation?.attempt ?? null, context_file: contextFile, response_file: `${contextFile}.stage-start-response.json`, runtime_engine: runtimeEngineIdentity(restored.engine), semantic_package_sha256: slice.semantic_package_sha256, context_slice_sha256: slice.context_slice_sha256, materialized_context_sha256: contextSha256, launcher_file: launcherFile } });
        }

        // Establish the semantic outcome before cleanup can lose its response.
        if (lifecycle.stage_status !== "done") {
        ...
        // A transport timeout or observation gap means the outcome is unknown,
        // so preserve that live tree for reconciliation. A confirmed AGY
        // provider exit has no live provider tree left; cancel-tree then only
        // clears the owned daemon's retained active projection for capture.
        const cancelTree = requiresTreeCancellation(subjectFailure?.code);
        try {
          if (executionCleanupRequired(await readEvents(events), runId, execution, opId)) {
            await callDriver(profile, ["daemon", "stop", ...daemonArgs, ...(cancelTree ? ["--cancel-tree"] : [])], { cwd: projectRoot, env: executionEnv });
          }
        }
        catch (cleanupError) {
          const settled = ["daemon_timeout", "daemon_not_running"].includes(cleanupError?.code)
            && !executionCleanupRequired(await readEvents(events), runId, execution, opId);
          if (!settled) {
...
async function withExecutionDaemon({ profile, attempt, projectRoot, runtimeRoot, journal, sessionId, action }) {
  ...
  // only appropriate after the original process is known to be unavailable.
  // Reuse its state directory: AGY/OpenCode/Grok keep native Session stores
  // there, so an empty directory is not a same-Session recovery.
  ...
  let daemon;
  try {
    daemon = await callDriver(profile, ["daemon", "status", ...primaryArgs, "--cwd", projectRoot, "--journal", journal], { cwd: projectRoot, env });
  } catch (error) {
    if (error.code !== "daemon_not_running") throw error;
    await reconcileDriverReplies(primaryState);
    await assertDaemonReplaceable(primaryState);
    daemonArgs = primaryArgs; recoveryBridge = true;
    await callDriver(profile, ["daemon", "start", ...daemonArgs, ...(sessionId ? ["--session-id", sessionId] : []), "--cwd", projectRoot, "--journal", journal], { cwd: projectRoot, env });
    daemon = await callDriver(profile, ["daemon", "status", ...daemonArgs, "--cwd", projectRoot, "--journal", journal], { cwd: projectRoot, env });
  }
  let operationFailure;
  try { return await action({ daemonArgs, env, daemon }); }
  catch (error) { operationFailure = error; throw error; }
  finally {
    if (recoveryBridge) {
      // The bridge exists only for an explicit recovery.  If its productive
      // Turn fails with a confirmed liveness/identity/provider-exit condition,
      // closing it normally must not leave an owned active tree behind and
      // prevent the outer recovery barrier from being sealed.
      try { await callDriver(profile, recoveryBridgeStopArgs(daemonArgs, operationFailure?.code), { cwd: projectRoot, env }); }
      catch (error) {
        if (!operationFailure) throw error;
        operationFailure.details = { ...operationFailure.details, cleanup_failure: errorRecord(error), daemon_state: daemonArgs[1] };
      }
    }
  }
}
async function promptExistingSession({ profile, attempt, projectRoot, runtimeRoot, sessionId, prompt, journal, operationId }) {
  return await withExecutionDaemon({ profile, attempt, projectRoot, runtimeRoot, journal, sessionId, action: async ({ daemonArgs, env }) => await providerTurn(profile, ["session", "prompt", ...daemonArgs, "--session-id", sessionId, "--cwd", projectRoot, "--model", profile.model, "--reasoning", profile.reasoning, "--prompt", prompt, "--journal", journal], { cwd: projectRoot, env, ...(operationId ? { operationId } : {}) }) });
}

export async function prepareRecoveryDelivery({ attempt, recovery, stage, sessionId, harness, paused = false, completed = false }) {
  const prompt = recoveryPrompt({ recovery, stage, paused, completed });
  const packet = {
    schema_id: "dd-eval/recovery-delivery@1", recovery_id: recovery.recovery_id,
    run_id: recovery.run_id, generation: recovery.generation, stage, paused, ...(completed ? { completed: true } : {}),
...
export function recoveryPrompt({ recovery, stage, paused = false, completed = false }) {
  ...
  return [
    ...
    "Before acting, inspect the retained RUN state, Work receipts, logs and partial artifacts. Preserve accepted work.",
    ...
    completed ? `The ${stage} stage already completed. After acceptance, return control to the runner for the next legal stage. Preserve the completed Work and its result; do not repeat it or launch its successor yourself.`
      : paused ? "This Work is paused for a user answer. After acceptance, stop without productive work, inventing an answer, or issuing stage resume." : `Continue only the unresolved ${stage} operation. Do not create undeclared Work or change requirements.`,
  ].join("\n");
}
async function inspectExistingSession({ profile, attempt, projectRoot, runtimeRoot, sessionId, journal }) {
  return await withExecutionDaemon({ profile, attempt, projectRoot, runtimeRoot, journal, sessionId, action: async ({ daemonArgs, env }) => await callDriver(profile, ["session", "inspect", ...daemonArgs, "--session-id", sessionId, "--cwd", projectRoot, "--model", profile.model, "--reasoning", profile.reasoning, "--journal", journal], { cwd: projectRoot, env }) });
}
...
async function recoverExecution({ root, events, manifest, execution, loaded, blueprint, profile, recovery = null, terminalOnly = false }) {
  ...
  const resolvedEngine = await commandJson(process.env.DD_FLOW_BIN ?? "dd-flow", ["engine", "resolve", "--project-root", projectRoot], { cwd: projectRoot, env: { DD_FLOW_HOME: runtimeRoot } });
  await assertCheckpointEngine(loaded.inputCheckpoint, resolvedEngine.selection?.selected);
  const admission = events.find(event => event.executionid === execution.id && event.type === "dev.dd.eval.execution.context_prepared")?.data?.baseline_admission;
  await verifyBaselineAdmission({ reference: admission, definition: loaded.value.baseline_admission, checkpoint: loaded.inputCheckpoint });
  await reconcileDriverReplies(path.join(attempt, "drivers", "daemon"));
  ...
  if (terminalOnly) assertTerminalReconciliation(execution, currentStage, record);
  if (recovery) {
    if (!["running", "paused", "done"].includes(record?.status)) fail("recovery has no recognized retained stage state", "recovery_stage_invalid");
    const delivery = await prepareRecoveryDelivery({ attempt, recovery, stage: currentStage, sessionId, harness: profile.harness, paused: record.status === "paused", completed: record.status === "done" });
    await appendEvent(path.join(root, "events.jsonl"), { source: "dd-eval://runner", runId: manifest.run_id, executionId: execution.id, traceId: manifest.run_id, type: "dev.dd.eval.execution.recovery_prepared", data: { recovery_id: recovery.recovery_id, generation: recovery.generation, packet_file: delivery.file, packet_sha256: delivery.sha256, operation_id: delivery.packet.operation_id } });
    await promptExistingSession({ profile, attempt, projectRoot, runtimeRoot, sessionId, prompt: delivery.packet.prompt, operationId: delivery.packet.operation_id, journal: path.join(attempt, "drivers", "subject.events.jsonl") });
    const accepted = await commandJson("dd-flow", ["run", "recovery", "inspect", recovery.run_id, "--project-root", projectRoot], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
    if (accepted.recovery?.recovery_id !== recovery.recovery_id || accepted.recovery?.status !== "recovered") fail("native Session did not acknowledge the prepared recovery", "recovery_acceptance_missing");
    await appendEvent(path.join(root, "events.jsonl"), { source: "dd-eval://runner", runId: manifest.run_id, executionId: execution.id, traceId: manifest.run_id, type: "dev.dd.eval.execution.recovery_prompted", data: { recovery_id: recovery.recovery_id, generation: recovery.generation ?? null, stage: currentStage } });
  ...
  const needsEntry = needsAttemptContext(record, preparedContextFor(events, execution.id, currentStage));
  if (record?.status === "done") {
    ...
    if (["e2e", "segment"].includes(execution.mode) && currentStage !== execution.terminal_stage) {
      await captureStageBoundary({ root, manifest, execution, lifecycle, attempt, projectRoot, runtimeRoot, stage: currentStage });
  ...
  if (record?.status === "paused") {
    ...
    let answerFile; let answerSha256 = null;
    if (priorMatch) {
      answerFile = priorMatch.data.answer_file;
      answerSha256 = priorMatch.data.answer_sha256 ?? null;
      ...
      const contextFile = preparedContextFor(events, execution.id, currentStage).context_file ?? (currentStage === execution.stage ? path.join(attempt, "stage-context.json") : null);
      if (!contextFile || !(await exists(contextFile))) fail("HITL recovery has no retained stage context", "stage_context_missing");
      const judgment = await interactionJudge({ runProfile, fixture, question, attempt, stage: currentStage, subjectProfile: profile, projectRoot, runtimeRoot, contextFile });
    ...
    const prompt = await resumePrompt({ lifecycle, stage: currentStage, question, answerFile, answerSha256, runtimeRoot, projectRoot });
  ...
  if (record?.status === "running") {
    const nativeReceipt = await inspectExistingSession({ profile, attempt, projectRoot, runtimeRoot, sessionId, journal: path.join(attempt, "drivers", "subject.events.jsonl") });
    if (needsEntry && nativeReceipt.settled !== true) return { execution: execution.id, stage: currentStage, state: "awaiting_provider", attempt, session_id: sessionId };
    if (!needsEntry) {
  ...
  if ((record?.status === "done" || needsEntry) && ["e2e", "segment"].includes(execution.mode)) {
    const successor = needsEntry ? currentStage : continuationStage(lifecycle, currentStage); if (!successor || stages.indexOf(successor) > stages.indexOf(execution.terminal_stage)) fail(`execution ${execution.id} is beyond its terminal boundary`, "resume_state_invalid");
    ...
    const contextFile = path.join(attempt, "stage-context", `${successor}-${randomUUID()}.json`); const slice = await materializeStageSlice({ blueprint, stage: successor, roots: { project: roots.project, workspace: roots.workspace, run: roots.run }, output: contextFile }); const contextSha256 = sha256(await readFile(contextFile));
    ...
    const launcherFile = path.join("launchers", `${path.basename(contextFile, ".json")}.md`);
    await writeFile(path.join(attempt, launcherFile), `${prompt}\n`);
    await appendEvent(path.join(root, "events.jsonl"), { source: "dd-eval://runner", runId: manifest.run_id, executionId: execution.id, traceId: manifest.run_id, type: "dev.dd.eval.execution.context_prepared", data: { stage: successor, attempt: stageRecord(lifecycle, successor)?.attempt ?? null, context_file: contextFile, response_file: `${contextFile}.stage-start-response.json`, semantic_package_sha256: slice.semantic_package_sha256, context_slice_sha256: slice.context_slice_sha256, materialized_context_sha256: contextSha256, launcher_file: launcherFile, recovered: true } });
    if (stageExecutor(successor, lifecycle) === "merge_server") {
      await runServerMerge({ profile, projectRoot, runtimeRoot, runId: lifecycle.run_id, env: runtimeEnv(runtimeRoot) });
    } else {
      let nextSessionId = sessionId;
      const journal = path.join(attempt, "drivers", "subject.events.jsonl");
      if (stageSessionMode(lifecycle) === "new_session") {
        const created = await withExecutionDaemon({ profile, attempt, projectRoot, runtimeRoot, journal, sessionId, action: async ({ daemonArgs, env }) => providerTurn(profile, ["session", "create", ...daemonArgs, "--cwd", projectRoot, "--model", profile.model, "--reasoning", profile.reasoning, "--journal", journal], { cwd: projectRoot, env }) });
        assertObservedProfile(created, profile, "recovered successor Session");
        nextSessionId = created.provider_session_id ?? created.session_id;
        if (typeof nextSessionId !== "string" || nextSessionId === sessionId) fail("harness did not create the required new successor Session", "session_identity_mismatch");
        await appendEvent(path.join(root, "events.jsonl"), { source: "dd-eval://runner", runId: manifest.run_id, executionId: execution.id, traceId: manifest.run_id, type: "dev.dd.eval.subject.successor_session_created", data: { stage: successor, session_id: nextSessionId } });
      }
      await promptExistingSession({ profile, attempt, projectRoot, runtimeRoot, sessionId: nextSessionId, prompt, journal });
    }
...
async function recoverLocked({ evalRoot, executionId = null, fromRecoveryId }) {
  ...
  for (const execution of selected) {
    ...
    await recoverySettlement(path.join(attempt, "drivers"));
    ...
    try { receipt = await recordOperation({ eventsFile: path.join(root, "events.jsonl"), source: "dd-eval://runner", runId: manifest.run_id, executionId: execution.id, traceId: manifest.run_id, operationId, operation: `execution.${execution.id}.recover`, action: async () => {
      ...
      if (typeof sessionId !== "string") fail("recovery has no retained Subject Session", "resume_session_missing");
      const selectedEngine = await commandJson(process.env.DD_FLOW_BIN ?? "dd-flow", ["engine", "resolve", "--project-root", projectRoot], { cwd: projectRoot, env: { DD_FLOW_HOME: runtimeRoot } });
      await assertCheckpointEngine(loaded.inputCheckpoint, selectedEngine.selection?.selected);
      await verifyBaselineAdmission({ reference: events.find(event => event.executionid === execution.id && event.type === "dev.dd.eval.execution.context_prepared")?.data?.baseline_admission, definition: loaded.value.baseline_admission, checkpoint: loaded.inputCheckpoint });
      return await withExecutionDaemon({ profile, attempt, projectRoot, runtimeRoot, journal: path.join(attempt, "drivers", "subject.events.jsonl"), sessionId, action: async ({ daemon, env }) => {
        if (typeof daemon?.daemon_id !== "string" || !daemon.daemon_id.trim()) fail("replacement daemon has no physical identity", "recovery_identity_missing");
        const prepared = await commandJson("dd-flow", ["run", "recovery", "resume", recovery.run_id, "--project-root", projectRoot, "--recovery-id", recovery.recovery_id, "--daemon-id", daemon.daemon_id, "--harness", profile.harness, "--native-session-id", sessionId], { cwd: projectRoot, env });
        const result = await recoverExecution({ root, events, manifest, execution, loaded, blueprint, profile, recovery: { ...recovery, ...prepared } });
        return { ...result, recovery: { ...recovery, resumed_at: now() } };
      } });
    } }); } catch (error) {
      if (isObservationLoss(error) || !isInfrastructureFailure(error.code) || ["operation_in_progress", "operation_terminal"].includes(error.code)) throw error;
...
export async function runnerCancel({ evalRoot, executionId = null }) {
  ...
  const results = storedExecutionResults(await readEvents(eventsFile), manifest);
  let finalized;
  if (cancelled.every(item => item.settled)) finalized = await finalizeRunProjection({ root, manifest, loaded: await loadCase(manifest.case_id), results });
  return { root, run_id: manifest.run_id, cancelled, state: finalized?.state ?? "cancelling" };
}
