    ...
    try {
      ...
      try {
        for (;;) {
          ...
          const successor = continuationStage(lifecycle, currentStage); if (!successor) break;
...
async function recoverExecution({ root, events, manifest, execution, loaded, blueprint, profile }) {
  ...
  if (record?.status === "done" && ["e2e", "segment"].includes(execution.mode)) {
    const successor = continuationStage(lifecycle, currentStage); if (!successor || stages.indexOf(successor) > stages.indexOf(execution.terminal_stage)) fail(`execution ${execution.id} is beyond its terminal boundary`, "resume_state_invalid");
