...
async function observeNativeChildren({ profile, attempt, projectRoot, runtimeRoot, runId, stage, eventsFile, event, children, sessionId }) {
  ...
  await writeJsonAtomic(observationsFile, { harness_id: profile.harness, parent_session_id: sessionId, children });
  const observed = await commandJson("dd-flow", ["stage", "native", "observe", runId, "--stage", stage, "--observations-file", observationsFile, "--project-root", projectRoot], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
    ...
    try {
      ...
      try {
        for (;;) {
          ...
          await observeNativeChildren({ profile, attempt, projectRoot, runtimeRoot, runId, stage: currentStage, eventsFile: events, event: { source: "dd-eval://runner", runId, executionId: execution.id, traceId: runId }, children: stageNativeChildren, sessionId });
