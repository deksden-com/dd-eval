...
async function prepareCanonicalInput({ root, sourceProjectRoot, flowRoot, inputCheckpoint }) {
  ...
  await commandText("git", ["-C", target, "-c", "user.name=dd-eval", "-c", "user.email=eval@localhost", "commit", "--quiet", "-m", `dd-eval: materialize ${inputCheckpoint.value.id} flow pack`], { cwd: root, env: { GIT_AUTHOR_DATE: sourceDate, GIT_COMMITTER_DATE: sourceDate } });
  ...
  const priorExclude = await readFile(exclude, "utf8").catch(() => "");
  if (!priorExclude.split(/\r?\n/).includes(".dd-eval/")) await writeFile(exclude, `${priorExclude}${priorExclude.endsWith("\n") || !priorExclude ? "" : "\n"}.dd-eval/\n`);
...
export async function canonicalBuild({ profileFile, projectRoot, flowRoot }) {
  ...
  try {
    ...
    const entry = await createBootstrapEntry({ root, home, loaded, revision, blueprint, sourceProjectRoot: materializedInput.project_root });
    const state = { schema_id: "dd-eval/canonical-build-state@1", case_id: loaded.value.id, revision, status: "awaiting_reference_resume", profile: profile.value.id, profile_file: profile.file, source_project_root: path.resolve(projectRoot), source_preflight: sourcePreflight, definition, input_checkpoint: { id: loaded.inputCheckpoint.value.id, sha256: loaded.inputCheckpoint.sha256, file: loaded.inputCheckpoint.file, value: loaded.inputCheckpoint.value }, materialized_input: materializedInput, blueprint_sha256: hashJson(blueprint), current_stage: "specify", reference: { session_id: null, daemon_state: null, run_id: null }, entries: { e2e: "entries/e2e.json", specify: "entries/specify.json" }, created_at: now() };
...
async function canonicalCandidatePack(root, state, blueprint) {
  ...
  const pack = await writeEntryPack({ caseDir: path.join(root, state.case_id), revision: state.revision, inputCheckpoint: { id: inputCheckpoint.id, sha256: inputCheckpoint.sha256 }, flow: { contour: stages, terminal_stage: stages.at(-1), flow_commit: state.materialized_input?.flow_commit ?? null, flow_manifest_sha256: state.materialized_input?.flow_manifest_sha256 ?? null }, stageBlueprint: blueprint, entries, e2e, authoring: { profile_id: state.profile, build_trace_sha256: await manifestHash(buildTrace) } });
...
export async function canonicalQualify({ buildRoot, profileFile }) {
  ...
  if (state.status === "waiting_for_entry_review") {
    const previousReceipt = await readJson(path.join(root, state.qualification?.receipt ?? ""));
    if (!qualificationSucceeded(previousReceipt?.result)) {
      state.status = "entries_captured";
      state.qualification_failure = { ...state.qualification, at: now() };
  ...
  try {
    ...
    const qualified = qualificationSucceeded(result);
    const receipt = { schema_id: "dd-eval/qualification-receipt@1", qualification_id: qualificationId, status: qualified ? "qualified" : "failed", profile_file: profile.file, result, created_at: now() };
    ...
    if (!qualified) {
      ...
      await writeCanonicalState(root, state, { source: "dd-eval://runner", runId: state.revision, type: "dev.dd.eval.canonical.qualification_failed", data: { state: state.status, qualification_id: qualificationId, failures: result.executions.filter((execution) => execution.state !== "candidate_ready").map((execution) => ({ execution: execution.execution, code: execution.code })) } });
...
async function captureCandidate({ projectRoot, runtimeRoot, runId, attempt, stage }) {
  ...
  const created = await exists(manifestFile) ? { reused: true } : await commandJson(bin, ["run", "snapshot", "create", runId, ...modeArgs, "--project-root", projectRoot, "--output", output], { cwd: projectRoot, env: { DD_FLOW_HOME: runtimeRoot } });
  const bytes = await readFile(manifestFile);
  const manifest = JSON.parse(bytes.toString("utf8"));
...
async function runCapacityProbe({ profile, attempt, projectRoot, runtimeRoot, probe, eventsFile, event }) {
  ...
  const deadlineAt = startedAt + deadline * 1000;
  const launches = await Promise.all([...Array(size)].map((_, index) => (async () => {
    ...
    try {
      ...
      return { key, daemon, session_id: sessionId, index };
    } catch (error) { if (daemon) await stopIsolatedWorkerDaemon({ profile, daemon, projectRoot }); return { key, completed: false, error: error instanceof Error ? error.message : String(error) }; }
  })()));
  const launchedAt = Date.now();
  const outcomes = launches.filter((launch) => launch.completed === false);
  const remainingMs = deadlineAt - launchedAt;
  if (remainingMs <= 0) {
    await Promise.all([...handles.entries()].map(async ([key, handle]) => {
    ...
    await appendEvent(eventsFile, { ...event, type: "dev.dd.eval.fanout.probe.completed", data: { requested: size, available_slots: 0, deadline_seconds: deadline, launch_completed_at: new Date(launchedAt).toISOString(), hold_seconds: hold, results: launches } });
    return 0;
  }
  const prompted = launches.map((launch) => (async () => {
    ...
    try {
      ...
      const response = await callDriver(profile, ["session", "prompt", ...launch.daemon.daemonArgs, "--session-id", launch.session_id, "--cwd", projectRoot, "--model", profile.model, "--reasoning", profile.reasoning, "--prompt", prompt(launch.index + 1), "--journal", launch.daemon.journal, "--timeout", String(remainingSeconds)], { cwd: projectRoot, env: launch.daemon.env });
  ...
  let timeoutId;
  const timeout = new Promise((resolve) => { timeoutId = setTimeout(resolve, remainingMs, "timeout"); });
  ...
  await appendEvent(eventsFile, { ...event, type: "dev.dd.eval.fanout.probe.completed", data: { requested: size, available_slots: availableSlots, deadline_seconds: deadline, launch_completed_at: new Date(launchedAt).toISOString(), hold_seconds: hold, results } });
