...
export function storedExecutionResults(events, manifest) {
  return manifest.executions.map((execution) => {
    const recovered = events.findLast((event) => event.executionid === execution.id && event.type === "dev.dd.eval.operation.completed" && typeof event.data?.operation_id === "string" && event.data.operation_id.startsWith(`${manifest.run_id}:${execution.id}:launch:recover:`));
    const recoveryFailure = events.findLast(event => event.executionid === execution.id && event.type === "dev.dd.eval.execution.failed" && event.data?.recovery_parent_id);
    if (recoveryFailure && (!recovered || events.indexOf(recoveryFailure) > events.indexOf(recovered))) return { ...recoveryFailure.data, execution: execution.id, state: "failed" };
    if (recovered?.data?.result) return recovered.data.result;
    ...
    const failure = events.findLast((event) => event.executionid === execution.id && event.type === "dev.dd.eval.execution.failed");
    if (launched && !failure?.data?.recovery?.recovery_id) return launched;
    if (failure) return { ...failure.data, execution: execution.id, state: "failed" };
    const operation = terminalOperation(events, `${manifest.run_id}:${execution.id}:launch`);
    if (operation?.terminal === "failed") return { execution: execution.id, stage: execution.stage, state: "failed", code: operation.result?.error?.code ?? "execution_failed", error: operation.result?.error?.message ?? null };
...
async function finalizeRunProjection({ root, manifest, loaded, results, permits = null }) {
  ...
  for (const result of results) {
    ...
    await appendEvent(eventsFile, { source: "dd-eval://runner", runId: manifest.run_id, executionId: result.execution, traceId: manifest.run_id, type: "dev.dd.eval.execution.failed", data: result });
  }
  ...
  const judgeStatus = judge ? "completed" : !manifest.profile?.judge?.enabled ? "not_requested" : pending ? "not_run_incomplete_execution" : "not_run";
  const reportEvents = await readEvents(eventsFile);
  const projection = reduceEvents(reportEvents); const report = buildReport({ root, manifest, state, results, candidate, judge, judgeStatus, events: reportEvents });
...
function classifyInterruption(error) {
  ...
  if (/terminal_result_missing|process_crash|sig(?:term|kill|segv|abrt)/.test(text)) return { category: "process_crash", source: "harness", retryable: true, error: record };
...
async function captureRecoveryEvidence({ root, manifest, result }) {
  ...
  if (typeof result.recovery_parent_id === "string") interruption.recovery_parent_id = result.recovery_parent_id;
...
export async function evalPreflight({ profileFile }) {
  ...
  try {
    ...
    assertCheckpointEngine(loaded.inputCheckpoint, engine);
    ...
    const receipt = { ok: true, root, definition, profile_file: runProfile.file, input_checkpoint: loaded.inputCheckpoint.value, engine: runtimeEngineIdentity(engine), doctors, provider_sessions_created: 0 };
  ...
  const results = await mapLimited(manifest.executions, runProfile.value.concurrency.global, async (execution) => {
    ...
    try {
      ...
      try {
        ...
        const cancelTree = subjectFailure?.code === "subject_liveness_timeout";
...
async function recoverLocked({ evalRoot, executionId = null, fromRecoveryId }) {
  ...
  for (const execution of selected) {
    ...
    try { receipt = await recordOperation({ eventsFile: path.join(root, "events.jsonl"), source: "dd-eval://runner", runId: manifest.run_id, executionId: execution.id, traceId: manifest.run_id, operationId, operation: `execution.${execution.id}.recover`, action: async () => {
      ...
      const failed = { execution: execution.id, attempt, stage: execution.stage, state: "failed", code: error.code ?? "execution_failed", error: error.message, recovery_parent_id: recovery.recovery_id };
...
export async function runnerCancel({ evalRoot, executionId = null }) {
  ...
  for (const execution of selected) {
    ...
    const receipt = await recordOperation({ eventsFile: path.join(root, "events.jsonl"), source: "dd-eval://runner", runId: manifest.run_id, executionId: execution.id, traceId: manifest.run_id, operationId, operation: `execution.${execution.id}.cancel`, action: async () => {
      return await withExecutionDaemon({ profile, attempt, projectRoot, runtimeRoot, journal, sessionId, action: async ({ daemonArgs, env }) => await callDriver(profile, ["session", "cancel", ...daemonArgs, "--session-id", sessionId, "--cwd", projectRoot, "--model", profile.model, "--reasoning", profile.reasoning, "--journal", journal], { cwd: projectRoot, env }) });
    } });
