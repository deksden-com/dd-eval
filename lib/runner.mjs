...
async function prepareCanonicalInput({ root, sourceProjectRoot, flowRoot, inputCheckpoint }) {
  ...
  await commandText("git", ["-C", target, "checkout", "-B", `eval-input-${inputCheckpoint.value.id}`, inputCheckpoint.value.source.commit], { cwd: root });
  ...
  const [sourceCommit, overlayManifest] = await Promise.all([
  ...
  return {
    ...
    branch: `eval-input-${inputCheckpoint.value.id}`,
    flow_root: flowRoot,
