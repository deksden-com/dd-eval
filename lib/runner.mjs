...
import { observeManagedRun } from "./managed-flow-client.mjs";
...
async function canonicalResumeUnlocked({ buildRoot, detachTurns = false }) {
  ...
  if (!state.reference.run_id) {
    ...
    if (!runId) {
      const prepared = await commandJson(runtimeBin(runtimeRoot), ["run", "prepare-vnext-specify", "--project-root", projectRoot, "--slug", "reference-subject", "--execution-routing-file", routing], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
...
export async function launchEvalExecution({ root, manifest, execution, loaded, blueprint, pack = null, packRoot = null, permits, onInfrastructureFailure = () => {} }) {
    ...
    try {
      ...
      if (!restored.run_id) {
        const prepared = await commandJson(runtimeBin(runtimeRoot), ["run", "prepare-vnext-specify", "--project-root", projectRoot, "--slug", "eval-subject", "--execution-routing-file", executionRoutingFile], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
