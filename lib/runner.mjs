...
export function driverRuntimeArgs(args, { cwd, env = {} } = {}) {
  if (args[0] !== "daemon" || args[1] !== "start" || typeof env.DD_FLOW_HOME !== "string" || !path.isAbsolute(env.DD_FLOW_HOME)) return args;
  const resolved = [...args];
  if (!resolved.includes("--dd-flow-bin")) resolved.push("--dd-flow-bin", process.env.DD_FLOW_BIN ?? "dd-flow");
...
async function callDriver(profile, args, options) {
  ...
  return await new Promise((resolve, reject) => {
    const command = driverProfileArgs(profile, driverRuntimeArgs(args, options));
