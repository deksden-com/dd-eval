...
function candidateMatches(candidate, manifest, results) {
  ...
  const expected = results.map((result) => ({ execution: result.execution, stage: result.stage ?? null, run_id: result.run_id ?? null, session_id: result.session_id ?? null, outcome: result.state, manifest_sha256: result.candidate?.manifest_sha256 ?? null })).sort((left, right) => left.execution.localeCompare(right.execution));
  const actual = (candidate.executions ?? []).map((result) => ({ execution: result.execution, stage: result.stage ?? null, run_id: result.run_id ?? null, session_id: result.session_id ?? null, outcome: result.outcome, manifest_sha256: result.checkpoint?.manifest_sha256 ?? null })).sort((left, right) => left.execution.localeCompare(right.execution));
...
async function frozenCandidate({ root, manifest, results }) {
  ...
  if (await exists(file)) {
    ...
    if (!candidateMatches(candidate, manifest, results)) {
      // Legacy failure text: existing candidate does not match completed executions.
      // Recovery now records a separate immutable revision instead, while
      // retaining that original candidate untouched.
      // Candidate evidence is immutable. A later explicit recovery may reach a
      // different terminal projection, but it must never overwrite the first
      // failed-run candidate or its judge input.
...
async function finalizeRunProjection({ root, manifest, loaded, results, permits = null }) {
  ...
  for (const result of results) {
    if (result.state !== "failed" || result.incomplete_evidence) continue;
    ...
    if (!result.recovery?.recovery_id) await captureIncompleteEvidence(result);
...
async function captureRecoveryEvidence({ root, manifest, result }) {
  ...
  try {
    ...
    const settlement = { settled: true, reason: "runner_confirmed_daemon_stop", captured_at: now(), interruption: interruption.category, session_id: result.session_id ?? null };
    ...
    const snapshot = await commandJson("dd-flow", ["run", "snapshot", "create", runId, "--recovery-id", recoveryId, "--project-root", projectRoot, "--output", output], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
...
export async function evalJudge({ evalRoot, profileId = null }) {
  ...
  const resultFile = path.join(root, "judge", "result.json");
...
export async function runnerRecover({ evalRoot, executionId = null }) {
  ...
  for (const execution of selected) {
    ...
    const operationId = `${manifest.run_id}:${execution.id}:launch:recover:${recovery.recovery_id}:${randomUUID()}`;
    const receipt = await recordOperation({ eventsFile: path.join(root, "events.jsonl"), source: "dd-eval://runner", runId: manifest.run_id, executionId: execution.id, traceId: manifest.run_id, operationId, operation: `execution.${execution.id}.recover`, action: async () => {
