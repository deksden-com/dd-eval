...
async function recoverExecution({ root, events, manifest, execution, loaded, blueprint, profile, recovery = null, terminalOnly = false }) {
  ...
  if (recovery) {
    ...
    const fanout = record.status === "running" ? await fanoutStatus({ projectRoot, runtimeRoot, runId: lifecycle.run_id, stage: currentStage }) : null;
    const delivery = await prepareRecoveryDelivery({ attempt, recovery, stage: currentStage, sessionId, harness: profile.harness, paused: record.status === "paused", completed: record.status === "done", orchestration: fanout?.orchestration });
