...
export async function resolveEvidenceJournals(result) {
  ...
  for (const journal of [...paths].sort()) {
    try {
      if (roots.has(journal) && !inside(await realpath(roots.get(journal)), await realpath(journal))) {
        journals.push({ journal, status: "unavailable", reason: "journal_outside_published_state_dir" });
        continue;
      }
      ...
      journals.push({ journal, status: "available", model_observations: { file: models, status: modelStatus }, observation_failures: summary.observation_failures ?? [] });
    } catch (error) { journals.push({ journal, status: "unavailable", reason: error.code === "ENOENT" ? "journal_missing" : errorRecord(error) }); }
  }
  ...
  return { journals, attribution, tools: { status: journals.some(item => item.status === "available") ? "partial" : "unavailable", sources: journals, counters: result.statistics?.usage?.tool_calls ?? result.driver?.evidence?.tool_calls ?? null } };
}
