...
async function createBootstrapEntry({ root, home, loaded, revision, blueprint, sourceProjectRoot }) {
  ...
  try { await commandJson(bin, ["run", "snapshot", "bootstrap", "create", "--project-root", sourceProjectRoot, "--output", snapshotRoot], { cwd: sourceProjectRoot, env: { DD_FLOW_HOME: transientHome } }); }
...
async function startReferenceDaemon(profile, roots, root) {
  ...
  await callDriver(profile, ["daemon", "start", ...daemonArgs, "--cwd", roots.projectRoot, "--journal", journal], { cwd: roots.projectRoot, env });
...
export async function canonicalResume({ buildRoot }) {
  ...
  const daemon = await startReferenceDaemon(profile, roots, root);
  ...
  let sessionId = state.reference.session_id;
  if (!sessionId) {
    ...
    state.reference = { ...state.reference, session_id: sessionId, daemon_state: daemon.daemonState };
  ...
  let prompted; let lifecycle; let turnPrompt = launcher; let hitlRounds = 0;
  for (;;) {
    ...
    if (lifecycle.stage_status !== "paused") break;
    ...
    if (fixture.mode === "forbidden" || hitlRounds >= fixture.max_rounds || !questionPath) break;
    const question = await readFile(questionPath, "utf8");
    const judgment = await interactionJudge({ runProfile, fixture, question, attempt: path.join(root, "reference"), stage, subjectProfile: profile, contextFile });
    if (judgment.verdict.status !== "matched" || judgment.verdict.response_ids.length === 0 || judgment.verdict.uncovered_questions.length > 0) break;
    ...
    hitlRounds += 1; turnPrompt = await resumePrompt({ lifecycle, stage, question, answer, runtimeRoot: roots.runtimeRoot, projectRoot: roots.projectRoot });
    await writeFile(path.join(root, "reference", "launchers", `${stage}-resume-${hitlRounds}.md`), `${turnPrompt}\n`);
    await appendEvent(path.join(root, "build", "events.jsonl"), { source: "dd-eval://runner", runId: state.revision, type: "dev.dd.eval.reference.hitl.matched", data: { stage, round: hitlRounds, pause_id: record.pause.id, response_ids: judgment.verdict.response_ids, judge_session_id: judgment.session_id } });
  }
...
async function finalJudge({ root, runId, manifest, loaded, profileId, candidate: suppliedCandidate = null, permits = null, results = null }) {
  ...
  const journal = path.join(judgeRoot, "events.jsonl"); const daemonState = path.join(judgeRoot, "daemon"); const daemonArgs = ["--state-dir", daemonState]; const judgeEnv = profile.harness === "codex-desktop" ? { CODEX_HOME: path.join(judgeRoot, "codex-home") } : {};
...
async function interactionJudge({ runProfile, fixture, question, attempt, stage, subjectProfile, contextFile = null, permits = null }) {
  ...
  const journal = path.join(root, "events.jsonl"); const daemonState = path.join(root, "daemon"); const daemonArgs = ["--state-dir", daemonState]; const judgeEnv = profile.harness === "codex-desktop" ? { CODEX_HOME: path.join(root, "codex-home") } : {};
...
async function resumePrompt({ lifecycle, stage, question, answer, runtimeRoot, projectRoot }) {
  ...
  const command = `printf %s ${JSON.stringify(answer)} | DD_FLOW_HOME=${JSON.stringify(runtimeRoot)} dd-flow stage resume ${lifecycle.run_id} --stage ${stage} --work ${pause.work_id} --answer-stdin --project-root ${JSON.stringify(projectRoot)} --json`;
  return ["A canonical user answer has been approved for the registered pause.", "Your first technical action must be this exact standalone lifecycle command (one pipe is intentional because it supplies stdin):", `\`${command}\``, "Then follow the continuation returned by dd-flow. Continue the same Stage and Work; do not call stage start or repeat preparation.", "", `<user_question>\n${question}\n</user_question>`, ""].join("\n");
}
...
function isInfrastructureFailure(code) {
  return ["driver_failed", "profile_drift", "hook_preflight_failed", "flow_reconciliation_failed", "snapshot_missing", "snapshot_checksum_mismatch", "snapshot_restore_mismatch", "driver_protocol"].includes(code);
}
      ...
      try {
        for (;;) {
          ...
          if (lifecycle.stage_status === "paused") {
            ...
            const question = await readFile(questionPath, "utf8");
            const judgment = await interactionJudge({ runProfile, fixture, question, attempt, stage: currentStage, subjectProfile: profile, contextFile, permits });
            ...
            const responses = new Map(fixture.responses.map((response) => [response.id, response.answer])); const answer = judgment.verdict.response_ids.map((id) => responses.get(id)).join("\n\n");
            hitlRounds.set(currentStage, rounds + 1); launcher = await resumePrompt({ lifecycle, stage: currentStage, question, answer, runtimeRoot, projectRoot });
...
async function recoverExecution({ root, events, manifest, execution, loaded, blueprint, profile }) {
  ...
  if (record?.status === "paused") {
    ...
    const judgment = await interactionJudge({ runProfile, fixture, question, attempt, stage: currentStage, subjectProfile: profile, contextFile: (await exists(contextFile)) ? contextFile : null });
    if (judgment.verdict.status !== "matched" || judgment.verdict.response_ids.length === 0 || judgment.verdict.uncovered_questions.length > 0) fail(`Interaction Judge could not match HITL at ${currentStage}`, "unmatched_hitl");
    const responses = new Map(fixture.responses.map((response) => [response.id, response.answer])); const answer = judgment.verdict.response_ids.map((id) => responses.get(id)).join("\n\n");
    const prompt = await resumePrompt({ lifecycle, stage: currentStage, question, answer, runtimeRoot, projectRoot });
