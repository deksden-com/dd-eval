...
async function runFanoutWorker({ profile, attempt, projectRoot, runtimeRoot, work, eventsFile, event }) {
  ...
  try {
    ...
    const promptWorker = async (prompt) => await callDriver(profile, ["session", "prompt", ...daemon.daemonArgs, "--session-id", sessionId, "--cwd", projectRoot, "--model", profile.model, "--reasoning", profile.reasoning, "--timeout", "120", "--prompt", prompt, "--journal", daemon.journal], { cwd: projectRoot, env: daemon.env });
    let response;
