...
function continuationStage(lifecycle, completedStage) {
  const projected = lifecycle?.status?.continuation;
  const expected = nextStage(completedStage);
  if (!projected) return expected;
  if (projected.kind === "terminal") return null;
  if (projected.kind !== "start_stage" || typeof projected.stage !== "string") {
    fail(`dd-flow did not expose a legal successor after ${completedStage}: ${projected.kind ?? "unknown"}`, "flow_reconciliation_failed");
  }
  // The runner knows its requested segment only as a guard. The CLI owns the
  // actual legal transition and recovery may therefore never invent a path.
  if (expected && projected.stage !== expected) {
    fail(`dd-flow successor ${projected.stage} contradicts selected flow successor ${expected}`, "flow_reconciliation_failed");
  }
...
async function executeEval({ runProfile, profile, loaded, validated, definition = null, root: suppliedRoot = null, runId: suppliedRunId = null, kind = "scored", executions: suppliedExecutions = null }) {
  ...
  const results = await mapLimited(manifest.executions, runProfile.value.concurrency.global, async (execution) => {
    ...
    try {
      ...
      const failure = { execution: execution.id, state: "failed", ...errorRecord(error), error: error instanceof Error ? error.message : String(error), ...(error?.hitl ? { hitl: [error.hitl] } : {}), started_at: startedAt, finished_at: now() };
      await appendEvent(events, { source: "dd-eval://runner", runId, executionId: execution.id, traceId: runId, type: "dev.dd.eval.execution.failed", data: { state: "failed", ...failure } });
...
async function promptExistingSession({ profile, attempt, projectRoot, runtimeRoot, sessionId, prompt, journal }) {
  return await withExecutionDaemon({ profile, attempt, projectRoot, runtimeRoot, journal, action: async ({ daemonArgs, env }) => await providerTurn(profile, ["session", "prompt", ...daemonArgs, "--session-id", sessionId, "--cwd", projectRoot, "--model", profile.model, "--reasoning", profile.reasoning, "--prompt", prompt, "--journal", journal], { cwd: projectRoot, env }) });
}
