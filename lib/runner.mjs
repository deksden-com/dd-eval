...
function recoveryPrompt({ recovery, stage }) {
  return [
...
async function recoverExecution({ root, events, manifest, execution, loaded, blueprint, profile, recovery = null, terminalOnly = false }) {
  ...
  if (recovery && record?.status === "running") {
    await promptExistingSession({ profile, attempt, projectRoot, runtimeRoot, sessionId, prompt: recoveryPrompt({ recovery, stage: currentStage }), journal: path.join(attempt, "drivers", "subject.events.jsonl") });
...
export async function runnerRecover({ evalRoot, executionId = null }) {
  return withRunnerLock(`${path.resolve(evalRoot)}.lifecycle`, () => recoverLocked({ evalRoot, executionId }));
}
...
async function recoverLocked({ evalRoot, executionId = null }) {
  ...
  const selected = manifest.executions.filter((execution) => (!executionId || execution.id === executionId) && prior.find((result) => result.execution === execution.id)?.state === "failed");
  if (selected.length === 0 && prior.some(result => (!executionId || result.execution === executionId) && result.recovery?.resumed_at)) return { root, run_id: manifest.run_id, reused: true, executions: prior };
  ...
  for (const execution of selected) {
    ...
    try { receipt = await recordOperation({ eventsFile: path.join(root, "events.jsonl"), source: "dd-eval://runner", runId: manifest.run_id, executionId: execution.id, traceId: manifest.run_id, operationId, operation: `execution.${execution.id}.recover`, action: async () => {
      await commandJson("dd-flow", ["run", "recovery", "resume", recovery.run_id, "--project-root", projectRoot, "--recovery-id", recovery.recovery_id], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
      const result = await recoverExecution({ root, events: await readEvents(path.join(root, "events.jsonl")), manifest, execution, loaded, blueprint, profile, recovery });
