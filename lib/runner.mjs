...
export async function canonicalResume({ buildRoot }) {
  ...
  while (turnPrompt) {
    ...
    state.reference = { ...state.reference, run_id: lifecycle.run_id };
    lifecycle = await registerJudgedTerminalQuestion({ root, loaded, blueprint, runProfile, profile, stage, prompted, lifecycle, roots, contextFile });
...
async function registerJudgedTerminalQuestion({ root, loaded, blueprint, runProfile, profile, stage, prompted, lifecycle, roots, contextFile }) {
  ...
  await appendEvent(path.join(root, "build", "events.jsonl"), { source: "dd-eval://runner", runId: path.basename(root), type: "dev.dd.eval.reference.hitl.registered_from_judged_turn", data: { stage, work_id: workId, judge_session_id: judgment.session_id } });
...
export function entryLauncher({ stage, entry, projectRoot, runtimeRoot, contextFile, contextSha256, profile }) {
  ...
  return [
    ...
    `\`${command}\``,
    "Use the returned authoritative prompt and perform only this Stage. If it needs a material user answer, run the exact `stage pause` lifecycle command from that prompt before showing the question, then stop the Turn. Otherwise finish this Stage, then stop the Turn.",
