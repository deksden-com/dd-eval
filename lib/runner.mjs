...
async function buildRunCandidate({ runId, manifest, results }) {
  const executions = results.map((result) => ({
    ...
    reached_stage: result.state === "candidate_ready" ? result.stage ?? null : result.lifecycle?.status?.run?.current_stage ?? result.lifecycle?.run?.current_stage ?? null,
...
function candidateMatches(candidate, manifest, results) {
  ...
  const expected = results.map((result) => ({ execution: result.execution, stage: result.stage ?? null, run_id: result.run_id ?? null, session_id: result.session_id ?? null, outcome: result.state, recovery_id: result.recovery?.recovery_id ?? null, manifest_sha256: result.candidate?.manifest_sha256 ?? null })).sort((left, right) => left.execution.localeCompare(right.execution));
  const actual = (candidate.executions ?? []).map((result) => ({ execution: result.execution, stage: result.stage ?? null, run_id: result.run_id ?? null, session_id: result.session_id ?? null, outcome: result.outcome, recovery_id: result.recovery?.recovery_id ?? null, manifest_sha256: result.checkpoint?.manifest_sha256 ?? null })).sort((left, right) => left.execution.localeCompare(right.execution));
...
async function freezeCandidateLocked({ root, manifest, results }) {
  ...
  if (await exists(file)) {
    ...
    if (!candidateMatches(candidate, manifest, results)) {
      ...
      next.parent_candidate_sha256 = candidate.immutable_hash;
...
async function captureRecoveryEvidence({ root, manifest, result }) {
  ...
  try {
    ...
    await commandJson("dd-flow", ["run", "recovery", "seal", runId, "--project-root", projectRoot, "--recovery-id", recoveryId, "--settlement-json", JSON.stringify(settlement)], { cwd: projectRoot, env: runtimeEnv(runtimeRoot) });
    ...
    const manifestFile = path.join(output, "snapshot.json");
    return { recovery_id: recoveryId, generation: recovery.generation ?? null, run_id: runId, interruption, settlement, snapshot: output, manifest: manifestFile, manifest_sha256: sha256(await readFile(manifestFile)), consistency: snapshot.consistency ?? "sealed_writer_barrier_required" };
  } catch (error) {
...
export function recoveryHistory(events, manifest, results) {
  ...
  return manifest.executions.map(({ id }) => {
    ...
    const segments = Object.values(operations).filter(operation => operation.id === launchId || operation.id.startsWith(`${launchId}:recover:`)).map(operation => {
      ...
      return {
        operation_id: operation.id, recovery_id: operation.id === launchId ? null : operation.id.slice(`${launchId}:recover:`.length),
    ...
    for (const event of events.filter(event => event.executionid === id && event.type === "dev.dd.eval.execution.failed")) {
      ...
      const segment = event.data.recovery_parent_id ? `${launchId}:recover:${event.data.recovery_parent_id}` : launchId;
...
export function isInfrastructureFailure(code) {
  ...
  if (code === "agy_terminal_result_missing") return true;
      ...
      try {
        for (;;) {
          ...
          if (!transitionedToRepair) {
            const checkpoint = await captureCandidate({ projectRoot, runtimeRoot, runId: lifecycle.run_id, attempt, stage: currentStage, output: path.join(attempt, "boundaries", `${currentStage}-${randomUUID()}`) });
            boundaries.push({ stage: currentStage, run_id: lifecycle.run_id, checkpoint, context_slice_sha256: slice.context_slice_sha256, materialized_context_sha256: contextSha256 });
            await appendEvent(events, { source: "dd-eval://runner", runId, executionId: execution.id, traceId: runId, type: "dev.dd.eval.stage.boundary_captured", data: { stage: currentStage, run_id: lifecycle.run_id, checkpoint } });
          }
        ...
        // A confirmed liveness failure is the only timeout outcome that
        // authorizes cancellation. A transport timeout or an observation gap
        // means the outcome is unknown, so preserve the live tree for
        // reconciliation instead of destroying the evidence.
        const cancelTree = ["subject_liveness_timeout", "profile_integrity_violation", "profile_drift", "profile_mismatch", "agy_profile_drift", "session_identity_mismatch", "foreign_session", "droid_child_identity_invalid", "droid_hook_identity_invalid", "droid_turn_identity_mismatch"].includes(subjectFailure?.code);
        try {
...
async function withExecutionDaemon({ profile, attempt, projectRoot, runtimeRoot, journal, sessionId, action }) {
  ...
  try { return await action({ daemonArgs, env, daemon }); }
    ...
    if (recoveryBridge) {
      try { await callDriver(profile, ["daemon", "stop", ...daemonArgs], { cwd: projectRoot, env }); }
      catch (error) {
...
export async function prepareRecoveryDelivery({ attempt, recovery, stage, sessionId, harness, paused = false }) {
  const prompt = recoveryPrompt({ recovery, stage, paused });
  const packet = {
    schema_id: "dd-eval/recovery-delivery@1", recovery_id: recovery.recovery_id,
    run_id: recovery.run_id, generation: recovery.generation, stage, paused,
...
export function recoveryPrompt({ recovery, stage, paused = false }) {
...
async function recoverExecution({ root, events, manifest, execution, loaded, blueprint, profile, recovery = null, terminalOnly = false }) {
  ...
  if (recovery && ["running", "paused"].includes(record?.status)) {
    const delivery = await prepareRecoveryDelivery({ attempt, recovery, stage: currentStage, sessionId, harness: profile.harness, paused: record.status === "paused" });
...
async function recoverLocked({ evalRoot, executionId = null, fromRecoveryId }) {
  ...
  const prior = storedExecutionResults(await readEvents(path.join(root, "events.jsonl")), manifest);
  const source = selectRecoverySource(prior, executionId, fromRecoveryId);
  ...
  for (const execution of selected) {
    const failure = prior.find((result) => result.execution === execution.id);
    ...
    const operationId = `${manifest.run_id}:${execution.id}:launch:recover:${recovery.recovery_id}`;
    let receipt;
  ...
  const results = storedExecutionResults(await readEvents(path.join(root, "events.jsonl")), manifest);
  const finalized = await finalizeRunProjection({ root, manifest, loaded, results });
  return { root, run_id: manifest.run_id, executions: results, recovered, ...(finalized.candidate ? { candidate: finalized.candidate } : {}), ...(finalized.judge ? { judge: finalized.judge } : {}), state: finalized.state };
}
