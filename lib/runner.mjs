...
async function runZcodeCapacityProbe({ profile, attempt, projectRoot, probe, eventsFile, event }) {
  ...
  try {
    ...
    const results = launches.map((launch) => {
      ...
      return { key: launch.key, session_id: launch.session_id, completed: capacityProbeSucceeded(result?.assistant_text, expected), ...(result?.assistant_text ? { assistant_text: result.assistant_text } : {}), ...(result?.error ? { error: result.error } : response?.error ? { error: response.error } : {}) };
    });
...
async function runCapacityProbe({ profile, attempt, projectRoot, runtimeRoot, probe, eventsFile, event }) {
  ...
  const prompted = launches.map((launch) => (async () => {
    ...
    try {
      ...
      const expected = `AGENT-${String(launch.index + 1).padStart(2, "0")}`;
      return { key: launch.key, session_id: launch.session_id, completed: capacityProbeSucceeded(response?.assistant_text, expected) };
