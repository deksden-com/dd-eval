...
export async function canonicalResume({ buildRoot }) {
  ...
  const prepareHitlResume = async () => {
    ...
    if (fixture.mode === "forbidden" || hitlRounds >= fixture.max_rounds || !questionPath) return null;
    ...
    if (judgment.verdict.status !== "matched" || judgment.verdict.response_ids.length === 0 || judgment.verdict.uncovered_questions.length > 0) return null;
    ...
    hitlRounds += 1; state.reference = { ...state.reference, hitl_rounds: hitlRounds }; const prompt = await resumePrompt({ lifecycle, stage, question, answer, runtimeRoot: roots.runtimeRoot, projectRoot: roots.projectRoot });
  ...
  while (turnPrompt) {
    ...
    state.reference = { ...state.reference, run_id: lifecycle.run_id };
    turnPrompt = lifecycle.stage_status === "paused" ? await prepareHitlResume() : null;
  }
