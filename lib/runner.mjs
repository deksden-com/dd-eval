...
export async function resolveEvidenceJournals(result) {
  ...
  for (const controller of controllers) {
    for (const session of controller.sessions ?? []) {
      if (typeof session.journal === "string" && path.isAbsolute(session.journal)) paths.add(path.resolve(session.journal));
    }
  }
  ...
  const journals = [], observations = [], observationFiles = new Set();
...
async function finalizeRunProjection({ root, manifest, loaded, results, permits = null }) {
  ...
  const judgeStatus = judge ? "completed" : !manifest.profile?.judge?.enabled ? "not_requested" : pending ? "not_run_incomplete_execution" : judgeError ? (judgeError.code === "operation_in_progress" ? "in_progress" : "failed") : "in_progress";
  const published = await withRunnerLock(eventsFile, async () => {
    ...
    const report = buildReport({ root, manifest, state, results, candidate, judge, judgeStatus, judgeError, events: reportEvents });
...
function buildReport({ root, manifest, state, results, candidate = null, judge = null, judgeStatus = "not_requested", judgeError = null, events = [] }) {
...
async function executePreparedQueue({ root, manifest, loaded, pack, packRoot, runProfile }) {
    ...
    return { run_id: manifest.run_id, root, executions: finalized.results, judge_status: finalized.report.judge_status, ...(finalized.judge ? { judge: finalized.judge } : {}), state: finalized.state };
}
...
async function runnerResumeLocked({ evalRoot, resumeRequestId, expectedManifestSha256 }) {
  ...
  return { root, run_id: manifest.run_id, executions: finalized.results, judge_status: finalized.report.judge_status, ...(finalized.candidate ? { candidate: finalized.candidate } : {}), ...(finalized.judge ? { judge: finalized.judge } : {}), state: finalized.state };
}
