...
import { cp, mkdir, open, readFile, readdir, rm, stat, writeFile, access } from "node:fs/promises";
...
function canonicalEntry({ caseId, revision, stage, snapshot, blueprint, engine }) {
...
async function createBootstrapEntry({ root, home, loaded, revision, blueprint, sourceProjectRoot }) {
  ...
  try {
    ...
    const entry = canonicalEntry({ caseId: loaded.value.id, revision, stage: "specify", snapshot, blueprint, engine });
...
function qualificationCellIdentity({ state, pack, profile, subject, execution }) {
  const entry = execution.mode === "e2e"
    ? { e2e: pack.hashes.e2e_sha256, focused_entries: pack.hashes.focused_entries }
    : { focused_entry: pack.hashes.focused_entries[execution.stage] };
  return hashJson({
...
export async function canonicalQualificationRecover({ buildRoot, receiptFile }) {
  ...
  if (!profile.value.selection.e2e || profile.value.selection.focused_stages.some((stage) => !stageSet.has(stage)) || profile.value.selection.focused_stages.length !== requiredFocused.size) fail("qualification receipt does not cover every focused stage and E2E", "qualification_profile_incomplete");
...
export async function canonicalQualify({ buildRoot, profileFile }) {
  ...
  if (pending.length === 0) {
    ...
    return { build: root, state, receipt: summary.receipt, next: { kind: "entry_review", entries: ["e2e", ...stages] } };
  }
...
async function restoreStageSnapshot({ home, entry, stage, projectRoot, runtimeRoot }) {
  ...
  if (entry.snapshot.kind === "bootstrap") {
    ...
    await materializeRuntimeEngine({ home, engine: entry.engine, runtimeRoot, projectRoot });
...
export function driverProfileArgs(profile, args) {
  if (!['doctor', 'session'].includes(args[0])) return args;
  const resolved = [...args];
  for (const key of ["provider", "model", "reasoning", "mode"]) {
...
async function absoluteExecutable(command, cwd) {
...
export async function driverRuntimeArgs(args, { cwd, env = {} } = {}) {
  ...
  const resolved = [...args];
  if (!resolved.includes("--dd-flow-bin")) resolved.push("--dd-flow-bin", await absoluteExecutable(process.env.DD_FLOW_BIN ?? "dd-flow", cwd));
