...
export function isInfrastructureFailure(code) {
  ...
  if (["storage_write_failed", "stage_inputs_changed", "lifecycle_outcome_conflict", "lifecycle_outcome_unknown", "work_start_publication_failed", "journal_conflict", "invocation_outcome_pending"].includes(code)) return true;
...
export async function runnerFork({ evalRoot, executionId = "e2e", from, output, engineVersion, integrityChecksum = null, requestId, start = false }) {
  ...
  const snapshotManifest = await readJson(path.join(snapshot, "snapshot.json"));
  ...
  const intent = { schema_id: "dd-eval/runner-fork-intent@1", request_id: requestId, source_eval: sourceRoot, source_execution: executionId, snapshot, snapshot_sha256: sha256(await readFile(path.join(snapshot, "snapshot.json"))), engine_version: engineVersion, integrity_checksum: integrityChecksum, target_stage: snapshotManifest.stage_entry };
  const intentFile = path.join(root, ".fork-intent.json");
  const receiptFile = path.join(root, "fork.json");
  ...
  const receipt = await withRunnerLock(`${root}.lifecycle`, async () => {
    if (await exists(receiptFile)) {
      ...
      if (hashJson(receipt.intent) !== hashJson(intent)) fail("fork output belongs to another request", "fork_request_conflict");
      return receipt;
    }
...
export async function executeEval({ runProfile, profile, loaded, validated, definition = null, root: suppliedRoot = null, runId: suppliedRunId = null, kind = "scored", executions: suppliedExecutions = null, prepareOnly = false }) {
  ...
  return withRunnerLock(`${path.resolve(root)}.lifecycle`, async () => {
    ...
    return executePreparedQueue({ root, manifest, loaded, pack, packRoot, runProfile });
  });
}


async function executePreparedQueue({ root, manifest, loaded, pack, packRoot, runProfile }) {
