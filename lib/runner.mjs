...
async function finalizeRunProjection({ root, manifest, loaded, results, permits = null }) {
  ...
  for (const result of results) {
    ...
    if (result.recovery?.recovery_id) delete result.incomplete_evidence;
  ...
  const capturePending = results.some((result) => result.state === "failed" && !result.recovery?.recovery_id
    && (!result.recovery?.capture_error || result.recovery.capture_error.code === "recovery_capture_pending"));
...
async function executePreparedQueue({ root, manifest, loaded, pack, packRoot, runProfile }) {
    ...
    return { run_id: manifest.run_id, root, executions: finalized.results, ...(finalized.judge ? { judge: finalized.judge } : {}), state: finalized.state };
}
