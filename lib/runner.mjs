...
async function runFanoutWorker({ profile, attempt, projectRoot, runtimeRoot, work, eventsFile, event, resumeSessionId = null }) {
  ...
  for (;;) {
    ...
    if (recoverRunning && Number(fanout.works?.running ?? 0) > 0) {
      ...
      if (resumable.length) {
        ...
        const settled = await Promise.allSettled(resumable.map(({ work_id, session_id }) => runFanoutWorker({ profile, attempt, projectRoot, runtimeRoot, work: { work_id }, resumeSessionId: session_id, eventsFile, event })));
