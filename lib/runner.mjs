...
async function driveFanout({ profile, attempt, projectRoot, runtimeRoot, runId, stage, eventsFile, event, nativeChildren = [], sessionId }) {
  ...
  if (nativeChildren.length && sessionId && (Number(status.orchestration.works?.created) || Number(status.orchestration.works?.running) || Number(status.orchestration.works?.completed))) {
