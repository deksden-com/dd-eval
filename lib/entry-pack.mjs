...
const stages = ["specify", "protocolize", "plan", "plan-review", "code", "code-review", "merge"];
...
export function validateStageContext(value, expectedStage = null) {
  ...
  // Accepted entry packs remain immutable historical eval inputs.  A
  // materialized slice is instead a CLI-owned envelope (below), linked by
  // independent source and materialized hashes.
  if (!["dd-eval/stage-context@1", "dd-flow/stage-context@1"].includes(context.schema_id)) fail("stage context must use dd-eval/stage-context@1 or dd-flow/stage-context@1");
...
export async function materializeStageSlice({ blueprint, stage, roots, output }) {
  ...
  const rendered = { ...source, schema_id: "dd-flow/stage-context@1", source_context_schema_id: source.schema_id, source_context_sha256: semanticContextHash(source), task_input: source.task_input.map((entry) => ({ ...entry, path: path.resolve(roots.project, entry.path) })), sources: await Promise.all((source.sources ?? []).map(resolve)), roots: { project: roots.project, ...(roots.workspace ? { workspace: roots.workspace } : {}), ...(roots.run ? { run: roots.run } : {}) } };
...
export function validateEntry(value, expectedStage = null) {
  ...
  // Historical entries pinned the authoring engine. It is provenance only;
  // execution resolves its own engine and new entries omit this field.
  if (entry.engine !== undefined) {
    const engine = object(entry.engine, "stage entry engine");
    if (engine.schema_id !== "dd-eval/engine-snapshot@1") fail("stage entry engine must use dd-eval/engine-snapshot@1");
    rel(engine.locator, "stage entry engine locator");
    nonempty(engine.package_name, "stage entry engine package_name"); nonempty(engine.package_version, "stage entry engine package_version"); nonempty(engine.engine_version, "stage entry engine engine_version");
    if (!/^[a-f0-9]{64}$/.test(engine.integrity_checksum ?? "")) fail("stage entry engine integrity_checksum must be sha256");
  }
...
export async function writeEntryPack({ caseDir, revision, inputCheckpoint, flow, stageBlueprint, entries, authoring, status = "candidate" }) {
  const accepted = {};
  ...
  const pack = {
    ...
    entries: Object.fromEntries(Object.keys(accepted).map((stage) => [stage, accepted[stage].path])),
    hashes: { stage_context_sha256: hashJson(stageBlueprint), focused_entries: Object.fromEntries(Object.entries(accepted).map(([stage, entry]) => [stage, entry.sha256])) },
