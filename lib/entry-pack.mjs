...
export function validateStageContext(value, expectedStage = null) {
  ...
  for (const item of context.task_input) {
    object(item, "stage task input"); nonempty(item.role, "stage task input role"); rel(item.path, "stage task input path"); nonempty(item.sha256, "stage task input sha256");
  }
...
export async function materializeStageSlice({ blueprint, stage, roots, output }) {
  ...
  const rendered = { ...source, task_input: source.task_input.map((entry) => ({ ...entry, path: path.resolve(roots.project, entry.path) })), sources: (source.sources ?? []).map(resolve), roots: { project: roots.project, ...(roots.workspace ? { workspace: roots.workspace } : {}), ...(roots.run ? { run: roots.run } : {}) } };
...
export function validateEntry(value, expectedStage = null) {
  ...
  object(entry.snapshot, "stage entry snapshot");
  if (entry.stage === "specify" && entry.snapshot.run_id !== null) fail("SPECIFY bootstrap entry must not have a RUN");
  if (entry.stage !== "specify" && typeof entry.snapshot.run_id !== "string") fail(`${entry.stage} entry requires a RUN`);
  for (const key of ["semantic_package_sha256", "context_slice_sha256"]) nonempty(entry[key], `stage entry ${key}`);
  return entry;
}
...
export async function writeEntryPack({ caseDir, revision, inputCheckpoint, flow, stageBlueprint, entries, e2e, authoring }) {
  const accepted = {};
