...
export async function promptSessionWithBridge(bridge, options) {
  ...
  return { harness: "codex-desktop", provider_session_id: sessionId, adapter_session_id: sessionId, turn_id: turnId, status: "completed", turn: completed, ...(thread ? { thread } : {}), ...(assistantText ? { assistant_text: assistantText } : {}), observed_profile: profile(options) };
}

async function ensureThreadLoaded(bridge, sessionId, options, cwd) {
  const value = await bridge.request("thread/read", { threadId: sessionId }, 10_000);
...
async function waitForTurn(bridge, sessionId, turnId, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    ...
    if (Date.now() >= nextStoredReadAt) {
      const stored = await readTurn(bridge, sessionId, turnId);
      if (stored && stored.status !== "inProgress") return terminalTurn({ turn: stored }, turnId);
      nextStoredReadAt = Date.now() + 5_000;
    }
...
async function readTurn(bridge, sessionId, turnId) {
  if (!turnId) return null;
  try {
    const result = await bridge.request("thread/read", { threadId: sessionId, includeTurns: true }, 10_000);
    const thread = result?.thread ?? result;
    return Array.isArray(thread?.turns) ? thread.turns.find((turn) => turn?.id === turnId) ?? null : null;
  } catch { return null; }
}
