...
export class CodexBridge {
  constructor(options = {}) { this.options = options; this.nextId = 1; this.pending = new Map(); this.turns = new Map(); this.finalMessages = new Map(); this.descendants = new Map(); this.journal = new Journal(options.journal); this.stderr = ""; }
...
export async function promptSessionWithBridge(bridge, options) {
  ...
  return { harness: "codex-desktop", provider_session_id: sessionId, adapter_session_id: sessionId, turn_id: turnId, status: "completed", turn: completed, ...(thread ? { thread } : {}), descendants: directChildren(bridge, sessionId), ...(assistantText ? { assistant_text: assistantText } : {}), observed_profile: { model: thread?.model ?? options.model ?? null, reasoning: thread?.reasoningEffort ?? null } };
}
...
export async function inspectSessionWithBridge(bridge, options) {
  ...
  return { harness: "codex-desktop", provider_session_id: sessionId, adapter_session_id: sessionId, status: thread.status ?? null, thread, descendants: directChildren(bridge, sessionId), observed_profile: { ...profile(options), model: thread.model ?? options.model ?? null, reasoning: thread.reasoningEffort ?? options.reasoning ?? null } };
}
