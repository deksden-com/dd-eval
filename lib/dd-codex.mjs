...
async function waitForTurn(bridge, sessionId, turnId, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    ...
    if (Date.now() >= nextStoredReadAt) {
      ...
      if (stored?.turn && isTerminalTurnStatus(stored.turn.status)) return terminalTurn({ turn: stored.turn }, turnId);
      ...
      if (stored?.threadStatus === "idle") {
        ...
        // worker session.
        if (hydrated?.turn && isTerminalTurnStatus(hydrated.turn.status)) return terminalTurn({ turn: hydrated.turn }, turnId);
      }
      ...
      if (final && Date.now() - final.completedAt >= 1_000) {
        const hydrated = await readTurn(bridge, sessionId, turnId, true);
        if (hydrated?.turn) return terminalTurn({ turn: hydrated.turn }, turnId);
        return terminalTurn({ turn: { id: turnId, status: "completed", synthetic: true, final_message: final.item } }, turnId);
      }
      nextStoredReadAt = Date.now() + 15_000;
    }
