...
export function parseCodexCliVersion(value) {
  const match = String(value).trim().match(/^codex-cli\s+(\S+)$/m);
  if (!match) fail("Codex CLI did not report its version", "codex_version_unavailable");
  return match[1];
}
...
function commandText(bin, args) {
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", (code) => code === 0 ? resolve(stdout) : reject(Object.assign(new Error(`Codex CLI version failed: ${stderr.trim() || code}`), { code: "codex_version_unavailable" })));
  });
}
...
export class CodexBridge {
  constructor(options = {}) { this.options = options; this.nextId = 1; this.pending = new Map(); this.turns = new Map(); this.finalMessages = new Map(); this.descendants = new Map(); this.journal = new Journal(options.journal); this.stderr = ""; this.requestedProfiles = new Map(); this.modelObservations = Promise.resolve(); }

  async start() {
    const bin = this.options.bin ?? process.env.DD_CODEX_BIN ?? "codex";
    this.child = spawn(bin, ["app-server"], { cwd: this.options.cwd ? absolute(this.options.cwd, "--cwd") : process.cwd(), env: { ...process.env, ...(this.options.env ?? {}) }, detached: process.platform !== "win32", stdio: ["pipe", "pipe", "pipe"] });
  ...
  receive(line) {
    ...
    this.observeTopology(message);
    if (["model/rerouted", "thread/settings/updated"].includes(message.method)) {
      const params = message.params ?? {}, id = params.threadId;
      let root = id; const seen = new Set();
      while (root && !this.requestedProfiles.has(root) && !seen.has(root)) { seen.add(root); root = this.descendants.get(root)?.parent_provider_session_id; }
      if (root && this.requestedProfiles.has(root)) {
        const settings = params.threadSettings ?? {};
        const rerouted = message.method === "model/rerouted";
        this.modelObservations = this.modelObservations.then(() => observeModel({ journal: this.options.journal, harness: "codex-desktop", sessionId: id,
          parentSessionId: this.descendants.get(id)?.parent_provider_session_id ?? null, requested: this.requestedProfiles.get(root),
          observed: { model: rerouted ? params.toModel : settings.model, provider: settings.modelProvider, reasoning: settings.effort },
          source: message.method, evidence: rerouted ? "response" : "configured", reason: params.reason ?? null, previousModel: params.fromModel ?? null
        })).catch(error => { this.modelObservationError = error; this.rejectAll(error); });
      }
    }
    if (message.id === undefined && (message.method?.startsWith("item/") || message.method === "turn/started" || message.method === "thread/tokenUsage/updated")) {
      this.activity ??= new Map();
      const visited = new Set();
      let id = message.params?.threadId;
      while (id && !visited.has(id)) {
        visited.add(id); this.activity.set(id, (this.activity.get(id) ?? 0) + 1);
        id = this.descendants.get(id)?.parent_provider_session_id;
      }
    }
    ...
    if (request?.method === "thread/read") {
      ...
      this.journal.write("inbound", { jsonrpc: message.jsonrpc, id: message.id, result: { thread: {
        id: thread.id ?? null, status: thread.status ?? null, model: thread.model ?? null, modelProvider: thread.modelProvider ?? null, reasoningEffort: thread.reasoningEffort ?? null, updatedAt: thread.updatedAt ?? null,
  ...
  // Collaboration child Threads are provider facts.  Keep only objects that
  // explicitly carry both a thread id and parent-thread id; this deliberately
  // ignores model prose and unrelated turn/item identifiers.
  observeTopology(value, depth = 0) {
    if (!value || typeof value !== "object" || depth > 8) return;
    if (Array.isArray(value)) { for (const item of value) this.observeTopology(item, depth + 1); return; }
    const id = value.threadId ?? value.id ?? null;
    const parent = value.parentThreadId ?? value.parent_thread_id ?? null;
    if (typeof id === "string" && id && typeof parent === "string" && parent) {
      this.descendants.set(id, { provider_session_id: id, parent_provider_session_id: parent, status: value.status?.type ?? value.status ?? "unknown", source: "codex.app_server" });
    }
    for (const child of Object.values(value)) this.observeTopology(child, depth + 1);
  }

  directChildren(sessionId) {
    return [...this.descendants.values()].filter((child) => child.parent_provider_session_id === sessionId);
  }
  ...
  request(method, params = {}, timeoutMs = this.options.timeoutMs ?? 120_000) {
    ...
    return new Promise((resolve, reject) => {
      const timer = observedTimeout(() => { this.pending.delete(id); reject(Object.assign(new Error(`${method} timed out`), { code: "rpc_timeout" })); }, timeoutMs, { onGap: gap => this.journal.write("observation_gap", { ...gap, method, request_id: id }) });
  ...
  async close() {
    ...
    await Promise.race([new Promise((resolve) => this.child.once("exit", resolve)), new Promise((resolve) => setTimeout(resolve, 2_000))]);
    await stopProcessGroup(this.child, 2_000);
    await this.modelObservations;
...
function directChildren(bridge, sessionId) { return typeof bridge?.directChildren === "function" ? bridge.directChildren(sessionId) : []; }

export async function doctor(options = {}) {
  const codex_cli = parseCodexCliVersion(await commandText(options.bin ?? process.env.DD_CODEX_BIN ?? "codex", ["--version"]));
  return await withBridge(options, async () => ({ harness: "codex-desktop", app_server: "available", observed_runtime: { codex_cli, dd_harness_contract: "dd-codex-harness@1" }, requested_profile: profile(options), observed_at: now() }));
}

async function observeCodexProfile(bridge, options, id, observed, source) {
  bridge.requestedProfiles?.set(id, profile(options));
  await bridge.modelObservations;
  if (bridge.modelObservationError) throw bridge.modelObservationError;
  await observeModel({ journal: options.journal ?? bridge.options?.journal, harness: "codex-desktop", sessionId: id,
    parentSessionId: bridge.descendants?.get(id)?.parent_provider_session_id ?? null, requested: profile(options), observed, source });
  return observed;
}
...
export async function createSessionWithBridge(bridge, options) {
  ...
  return { harness: "codex-desktop", provider_session_id: threadId(result), adapter_session_id: threadId(result), requested_profile: profile(options), observed_profile: await observeCodexProfile(bridge, options, threadId(result), { model: result.model ?? null, reasoning: result.reasoningEffort ?? null, provider: result.modelProvider ?? null }, "thread/start.response"), cwd: result.cwd ?? cwd };
}
...
export async function startSessionWithBridge(bridge, options) {
  ...
  bridge.requestedProfiles?.set(sessionId, profile(options));
  ...
  return { harness: "codex-desktop", provider_session_id: sessionId, adapter_session_id: sessionId, turn_id: turnId, status: "started", requested_profile: profile(options), observed_profile: { model: null, reasoning: null } };
}
...
export async function promptSessionWithBridge(bridge, options) {
  ...
  return { harness: "codex-desktop", provider_session_id: sessionId, adapter_session_id: sessionId, turn_id: turnId, status: "completed", turn: completed, ...(thread ? { thread } : {}), descendants: directChildren(bridge, sessionId), ...(assistantText ? { assistant_text: assistantText } : {}), requested_profile: profile(options), observed_profile: await observeCodexProfile(bridge, options, sessionId, { model: thread?.model ?? null, reasoning: thread?.reasoningEffort ?? null, provider: thread?.modelProvider ?? null }, "thread/read.after_turn") };
}
...
async function waitForTurn(bridge, sessionId, turnId, timeoutMs, options, cwd) {
  const clock = new ObservationClock({ timeoutMs, onGap: gap => bridge.journal?.write("observation_gap", { ...gap, session_id: sessionId, turn_id: turnId }) });
  ...
  while (!clock.sample(bridge.activity?.get(sessionId))) {
    const known = bridge.turns.get(turnId) ?? bridge.turns.get(sessionId); if (known?.status === "completed") return terminalTurn(known.value, turnId, bridge);
    if (Date.now() >= nextStoredReadAt) {
      const stored = await readTurn(bridge, sessionId, turnId, false);
      if (stored?.turn && isAuthoritativeTerminal(bridge, turnId, stored.turn.status, stored.threadStatus, failureNotBefore)) return terminalTurn({ turn: stored.turn }, turnId, bridge);
      ...
      if (persisted && isAuthoritativeTerminal(bridge, turnId, persisted.status, stored?.threadStatus, failureNotBefore)) return terminalTurn({ turn: persisted }, turnId, bridge);
      ...
      if (stored?.threadStatus === "notLoaded") {
        ...
        if (hydrated?.turn && isAuthoritativeTerminal(bridge, turnId, hydrated.turn.status, hydrated.threadStatus, failureNotBefore)) return terminalTurn({ turn: hydrated.turn }, turnId, bridge);
      }
      ...
      if (final && stored?.threadStatus === "idle" && Date.now() - final.completedAt >= 1_000) {
        const hydrated = await readTurn(bridge, sessionId, turnId, true);
        if (hydrated?.turn && isAuthoritativeTerminal(bridge, turnId, hydrated.turn.status, hydrated.threadStatus, failureNotBefore)) return terminalTurn({ turn: hydrated.turn }, turnId, bridge);
        return terminalTurn({ turn: { id: turnId, status: "completed", synthetic: true, final_message: final.item } }, turnId, bridge);
      }
...
function terminalTurn(value, turnId, bridge) {
  // A terminal event may arrive while an older snapshot RPC is in flight.
  const observed = bridge.turns.get(turnId);
  if (observed?.status === "completed") value = observed.value;
...
export async function inspectSessionWithBridge(bridge, options) {
  ...
  return { harness: "codex-desktop", provider_session_id: sessionId, adapter_session_id: sessionId, status: thread.status ?? null, thread, descendants: directChildren(bridge, sessionId), requested_profile: profile(options), observed_profile: await observeCodexProfile(bridge, options, sessionId, { model: thread.model ?? null, reasoning: thread.reasoningEffort ?? null, provider: thread.modelProvider ?? null }, "thread/read") };
}
...
export async function cancelSessionWithBridge(bridge, options) {
  ...
  // Interrupt acknowledgement is not a provider terminal receipt. Reconcile
  // observes the Thread before the runner records terminal cancellation.
  return { harness: "codex-desktop", provider_session_id: sessionId, adapter_session_id: sessionId, turn_id: turnId, cancelled: true, settled: false };
}
