...
async function waitForTurn(bridge, sessionId, turnId, timeoutMs, options, cwd) {
  ...
  while (Date.now() < deadline) {
    ...
    if (Date.now() >= nextStoredReadAt) {
      ...
      if (stored?.turn && isTerminalTurnStatus(stored.turn.status) && (stored.turn.status === "completed" || Date.now() >= failureNotBefore)) return terminalTurn({ turn: stored.turn }, turnId);
      ...
      if (persisted && isTerminalTurnStatus(persisted.status) && (persisted.status === "completed" || Date.now() >= failureNotBefore)) return terminalTurn({ turn: persisted }, turnId);
