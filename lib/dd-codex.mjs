...
export async function promptSessionWithBridge(bridge, options) {
  ...
  return { harness: "codex-desktop", provider_session_id: sessionId, adapter_session_id: sessionId, turn_id: turnId, status: "completed", turn: completed, ...(thread ? { thread, assistant_text: latestAssistantText(thread) } : {}), observed_profile: profile(options) };
}
...
async function readCompletedThread(bridge, sessionId) {
  for (let attempt = 0; attempt < 8; attempt += 1) {
    try { const result = await bridge.request("thread/read", { threadId: sessionId }, 10_000); return result?.thread ?? result; }
...
function latestAssistantText(value) {
  const messages = value?.messages ?? value?.items ?? [];
