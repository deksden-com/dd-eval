...
export async function authorizeRetainedDaemonResume(root, previous, sessionId, config) {
  const sessions = [...(previous.sessions ?? []), ...(previous.root_session_id ? [previous.root_session_id] : [])];
...
export async function recoverySettlement(driversRoot) {
  ...
  async function visit(directory) {
    ...
    const daemonRoot = entries.some(entry => entry.name === "daemon.json" && entry.isFile());
    for (const entry of entries) {
      ...
      // Native stores contain package/auth symlinks, not adapter daemon
      // receipts. Do not traverse their contents as a daemon inventory.
      if (entry.isDirectory()) {
        if (daemonRoot && ["gemini", "grok-home", "home", "xdg-data", "xdg-cache", "xdg-config"].includes(entry.name)) continue;
        await visit(file);
      }
        ...
        if (state.config?.resourceHome && state.resource_process?.id) {
          ...
          for (const record of processes.filter(record => record.id === state.resource_process.id || [record.stdout_path, record.stderr_path].some(file => typeof file === "string" && path.resolve(file).startsWith(`${attemptRoot}${path.sep}`)))) {
            ...
            if (Number.isInteger(record.pid) && record.pid > 0) {
              ...
              if (typeof record.pid_started_at === "string" && record.pid_started_at.trim()) {
                const observed = (await processSnapshot()).find(item => item.pid === record.pid);
                if (!observed || record.pid_started_at.trim().replace(/\s+/g, " ") !== observed.started) continue;
              }
