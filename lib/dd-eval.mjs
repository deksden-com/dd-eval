...
function validateAssessment(value, caseId) {
  ...
  for (const scope of [...stages, "e2e"]) {
    ...
    for (const key of ["required_outcomes", "accepted_alternatives", "known_risks"]) if (!Array.isArray(golden[key])) fail(`assessment ${scope} golden.${key} must be an array`);
  }
  return value;
}
...
export async function judgePrepare({ evalRoot, executionId, rejudge = false }) {
  ...
  const packet = [
    ...
    "", "## Required Judge result", "Return JSON only. Score every listed outcome and flow criterion from 0 through 4; use `score: null, not_applicable: true` only with a rationale. Findings must state evidence and practical impact. Do not return weights or calculated totals.", "```json", resultContract.trim(), "```"
  ].join("\n");
  ...
  await writeJson(contextPath, { scope, judgment, assessment_sha256: context.manifest.definition.assessment_sha256, methodology_sha256: context.manifest.definition.methodology_sha256, candidate: lifecycle.candidate });
  ...
  await saveState(context); return { ok: true, packet: packetPath, sha256: await materialHash(packetPath), judgment };
}
