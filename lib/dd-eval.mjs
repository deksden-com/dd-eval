...
async function loadStarterSessions(loaded, requiredHarnesses = new Set(["codex-desktop"])) {
  ...
  for (const stage of stages) {
    ...
    for (const [harness, alternate] of Object.entries(entry.by_harness ?? {})) {
      ...
      if (alternate.parent_session_id !== loaded.checkpoints[stage].subject_by_harness?.[harness]?.checkpoint_session_id) fail(`${harness} starter parent is invalid for ${stage}`);
    }
    for (const harness of requiredHarnesses) if (harness !== "codex-desktop" && !entry.by_harness?.[harness]?.session_id) fail(`starter session ${stage} is missing for ${harness}`);
  }
  return registry;
}
  ...
  if (mode === "scored") {
    for (const stage of stages) {
      ...
      for (const harness of subjectHarnesses) if (harness !== "codex-desktop") {
        if (!loaded.checkpoints[stage].subject_by_harness?.[harness]?.checkpoint_session_id) fail(`canonical checkpoint ${stage} has no accepted ${harness} Subject`);
        const alternate = loaded.checkpoints[stage].harness_evidence?.[harness]?.runtime_snapshot?.locator;
...
export function checkpointForHarness(checkpoint, harness = "codex-desktop") {
  const runtimeSnapshot = harness === "codex-desktop" ? checkpoint.runtime_snapshot : checkpoint.harness_evidence?.[harness]?.runtime_snapshot;
  const subject = harness === "codex-desktop" ? checkpoint.subject : checkpoint.subject_by_harness?.[harness];
...
export async function captureCanonicalCheckpoint({ caseId, stage, revision = "REV-001", projectRoot, flowRunId, runtimeHome, canonicalSubjectSessionId, checkpointSubjectSessionId, agentId = null, harness = "codex-desktop", engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
...
export async function acceptCanonicalCheckpoint({ caseId, stage, recordFile, reviewFile }) {
  ...
  if ((checkpoint.harness ?? "codex-desktop") === "codex-desktop") await writeJson(destination, accepted);
  else {
...
export async function setStarterSession({ caseId, stage, sessionId, parentSessionId, harness = "codex-desktop" }) {
  ...
  if (!["codex-desktop", "zcode-acp", "grok-acp"].includes(harness)) fail("--harness must be codex-desktop, zcode-acp, or grok-acp");
  const protectedSessionId = harness === "codex-desktop"
    ? checkpoint.subject.checkpoint_session_id
    : checkpoint.subject_by_harness?.[harness]?.checkpoint_session_id;
  ...
  const registry = !existing || existing.schema_id !== "dd-eval/starter-sessions@3" || existing.case_id !== loaded.definition.id || existing.revision !== expectedRevision
  ...
  if (harness === "codex-desktop") registry.sessions[stage] = { ...registry.sessions[stage], session_id: sessionId, parent_session_id: parentSessionId, harness };
    ...
    entry.by_harness = { ...(entry.by_harness ?? {}), [harness]: { session_id: sessionId, parent_session_id: parentSessionId, harness } };
  }
...
export async function prepare({ caseId, source, output = null, controllerProfileId, subjectProfileId, judgeProfileId, stageList, segment, e2e = false, scenario = null, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  ...
  try {
    ...
    for (const execution of executions) {
      ...
      execution.subject_seed_mode = execution.kind === "e2e" ? "clean_session" : "shared_stage_starter";
    ...
    const launch = await Promise.all(executions.map(async (execution) => {
      ...
      return {
        id: execution.id,
        next_action: execution.subject_seed_mode === "clean_session" ? "create_subject" : "fork_subject",
