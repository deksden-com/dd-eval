...
const stages = ["specify", "protocolize", "plan", "plan-review", "code"];
...
function validateCase(definition, caseId) {
  ...
  if (definition.e2e.stop_boundary !== "code_completed") fail("e2e.stop_boundary must be code_completed");
...
export async function setStarterSession({ caseId, stage, sessionId, parentSessionId }) {
  ...
  const registry = await exists(destination) ? await readJson(destination) : { schema_id: "dd-eval/starter-sessions@3", case_id: loaded.definition.id, revision: [...revisions][0], sessions: {} };
  if (registry.schema_id !== "dd-eval/starter-sessions@3" || registry.case_id !== loaded.definition.id || registry.revision !== [...revisions][0]) fail("existing starter session registry does not match this canonical chain");
