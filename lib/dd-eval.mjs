...
export async function addSession({ evalRoot, executionId, role, sessionId, parentSessionId, agentId = null, harness, adapterSessionId = null, observedProfile = null }) {
  ...
  context.sessions.sessions.push({ execution_id: executionId, role, harness, runtime_family: harness === "zcode-acp" ? "zcode" : "codex", session_id: sessionId, provider_session_id: sessionId, adapter_session_id: adapterSessionId, parent_session_id: parentSessionId ?? null, agent_id: agentId, observed_profile: observedProfile, expected_profile: expectedProfile ? { id: expectedProfile.id, harness: expectedProfile.harness, provider: expectedProfile.provider ?? null, model: expectedProfile.model, reasoning: expectedProfile.reasoning, mode: expectedProfile.mode ?? null } : null, recorded_at: now() }); await saveState(context); return { ok: true, idempotent: false };
}
