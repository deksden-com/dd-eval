...
export async function prepare({ caseId, source, output, controllerProfileId, subjectProfileId, judgeProfileId, stageList, e2e = false, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  ...
  const profileIds = { controller: controllerProfileId, subject: subjectProfileId, judge: judgeProfileId };
  for (const [role, profileId] of Object.entries(profileIds)) { if (!profileId) fail(`--${role}-profile is required`); if (!context.definition.profiles[role].includes(profileId)) fail(`${role} profile is not allowed by this case: ${profileId}`); }
  ...
  if (profiles.subject.runtime?.dd_flow_cli?.engine_commit && profiles.subject.runtime.dd_flow_cli.engine_commit !== context.definition.compatibility.engine_commit) fail("Subject profile engine commit does not match the case");
