...
async function subjectProfileObservation(context, execution, sessionList) {
  ...
  const byId = new Map((sessionList.sessions ?? []).map((item) => [item.session_id, item]));
  const observations = await Promise.all(recorded.map(async (recordedSession) => {
    const provider = byId.get(recordedSession.session_id);
    if (!provider) return { session_id: recordedSession.session_id, status: "missing", expected };
    const actual = await observedProfile(provider);
    const matches = actual.model === expected.model && actual.reasoning === expected.reasoning;
    return { session_id: recordedSession.session_id, status: matches ? "matched" : "mismatch", expected, actual };
  }));
  return { status: observations.length > 0 && observations.every((item) => item.status === "matched") ? "matched" : "invalid", expected, observations };
}
