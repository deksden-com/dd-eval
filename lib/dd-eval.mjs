...
async function observedProfile(session) {
  if (typeof session?.transcript_path !== "string" || !(await exists(session.transcript_path))) return { model: session?.model ?? null, reasoning: null, source: "run_sessions" };
  ...
  return { model: session.model ?? null, reasoning: null, source: "run_sessions" };
}
...
async function subjectProfileObservation(context, execution, sessionList) {
  ...
  const observations = await Promise.all(productive.map(async (provider) => {
    ...
    const matches = (provider.harness ?? receipt?.harness ?? "codex-desktop") === expected.harness
