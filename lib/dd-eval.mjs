...
async function loadProfile(profileId) {
  ...
  if (!["codex-desktop", "zcode-acp", "grok-acp", "opencode-server"].includes(profile.harness)) fail(`unsupported profile harness: ${profile.harness}`);
  ...
  if (["zcode-acp", "grok-acp", "opencode-server"].includes(profile.harness) && (!profile.provider || !profile.mode)) fail(`${profile.harness} profile ${profileId} requires provider and mode`);
  const runtime_family = profile.harness === "zcode-acp" ? "zcode" : profile.harness === "grok-acp" ? "grok" : profile.harness === "opencode-server" ? "opencode" : "codex";
  const driver = profile.harness === "zcode-acp" ? "dd-zcode" : profile.harness === "grok-acp" ? "dd-grok" : profile.harness === "opencode-server" ? "dd-opencode" : "codex-native";
...
async function loadStarterSessions(loaded, requiredHarnesses = new Set(["codex-desktop"])) {
  ...
  for (const stage of stages) {
    ...
    for (const [harness, alternate] of Object.entries(entry.by_harness ?? {})) {
      if (!["zcode-acp", "grok-acp", "opencode-server"].includes(harness)) fail(`unsupported alternate starter harness: ${harness}`);
      ...
      if (!(harness === "zcode-acp" && seedMode === "deterministic_replay") && !(harness === "opencode-server" && seedMode === "archive_native_fork") && seedMode !== "native_fork") fail(`${harness} starter ${stage} has unsupported seed mode: ${seedMode}`);
...
export async function captureCanonicalCheckpoint({ caseId, stage, revision = "REV-001", projectRoot, flowRunId, runtimeHome, canonicalSubjectSessionId, checkpointSubjectSessionId, agentId = null, harness = "codex-desktop", sessionArchive = null, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  ...
  if (!["codex-desktop", "zcode-acp", "grok-acp", "opencode-server"].includes(harness)) fail("--harness must be codex-desktop, zcode-acp, grok-acp, or opencode-server");
...
export async function setStarterSession({ caseId, stage, sessionId, parentSessionId, harness = "codex-desktop", sessionArchive = null, seedMode = "native_fork" }) {
  ...
  if (!["codex-desktop", "zcode-acp", "grok-acp", "opencode-server"].includes(harness)) fail("--harness must be codex-desktop, zcode-acp, grok-acp, or opencode-server");
  if (seedMode !== "native_fork" && !(harness === "zcode-acp" && seedMode === "deterministic_replay") && !(harness === "opencode-server" && seedMode === "archive_native_fork")) fail("unsupported --seed-mode for harness");
...
export function subjectSeedMode({ harness, kind, starter = null }) {
  ...
  return harness === "zcode-acp" && starter?.seed_mode === "deterministic_replay" ? "deterministic_replay" : "shared_stage_starter";
}
...
export async function addSession({ evalRoot, executionId, role, sessionId, parentSessionId, agentId = null, harness, adapterSessionId = null, daemonId = null, observedProfile = null }) {
  ...
  const runtimeFamily = harness === "zcode-acp" ? "zcode" : harness === "grok-acp" ? "grok" : harness === "opencode-server" ? "opencode" : "codex";
