...
export async function validateInput({ caseId, source, requireMode: requiredMode = "authoring" }) {
  ...
  return { ...loaded, starters, sourceRoot, checkpoint, sourceTree: await run("git", ["-C", sourceRoot, "rev-parse", `${checkpoint.source.commit}^{tree}`]) };
}
...
function subjectContinuation({ stage, projectRoot, ddFlowHome, flowRunId, packet, focused }) {
  const start = `DD_FLOW_HOME=${shell(ddFlowHome)} dd-flow stage start ${shell(flowRunId)} --stage ${shell(stage)} --project-root ${shell(projectRoot)} --json`;
...
export async function prepare({ caseId, source, output = null, controllerProfileId, subjectProfileId, judgeProfileId, stageList, segment, e2e = false, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  ...
  try {
    ...
    for (const { execution, checkpoint, definition, checkpointStage } of pendingCheckpoints) {
      ...
      promptReceipts[execution.id].subject = await writePacket(promptReceipts[execution.id].subject.path, subjectContinuation({ stage: checkpointStage, projectRoot: execution.project_root, ddFlowHome: runtimeHome, flowRunId: execution.flow_run_id, packet: staticPacket, focused: execution.kind !== "e2e" }), `${definition.subject_packet}+generated-runtime-continuation`);
    }
