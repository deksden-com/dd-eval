...
async function sessionArchiveReceipt(archive, sessionId, label) {
  ...
  return { locator, sha256: await archiveHash(root) };
}
async function verifySessionArchive(receipt, sessionId, label) {
  ...
  const actual = await sessionArchiveReceipt(root, sessionId, label);
...
async function loadProfile(profileId) {
  ...
  if (!["codex-desktop", "zcode-acp", "grok-acp"].includes(profile.harness)) fail(`unsupported profile harness: ${profile.harness}`);
  ...
  if (["zcode-acp", "grok-acp"].includes(profile.harness) && (!profile.provider || !profile.mode)) fail(`${profile.harness} profile ${profileId} requires provider and mode`);
  return { runtime_family: profile.harness === "zcode-acp" ? "zcode" : profile.harness === "grok-acp" ? "grok" : "codex", driver: profile.harness === "zcode-acp" ? "dd-zcode" : profile.harness === "grok-acp" ? "dd-grok" : "codex-native", ...profile };
}
...
async function loadStarterSessions(loaded, requiredHarnesses = new Set(["codex-desktop"])) {
  ...
  for (const stage of stages) {
    ...
    for (const [harness, alternate] of Object.entries(entry.by_harness ?? {})) {
      if (!["zcode-acp", "grok-acp"].includes(harness)) fail(`unsupported alternate starter harness: ${harness}`);
      ...
      if (!(["zcode-acp"].includes(harness) && seedMode === "deterministic_replay") && seedMode !== "native_fork") fail(`${harness} starter ${stage} has unsupported seed mode: ${seedMode}`);
      if (harness === "grok-acp") await verifySessionArchive(alternate.session_archive, alternate.session_id, `${harness} starter archive ${stage}`);
    }
  ...
  if (mode === "scored") {
    for (const stage of stages) {
      ...
      for (const harness of subjectHarnesses) if (harness !== "codex-desktop") {
        ...
        if (harness === "grok-acp") await verifySessionArchive(selected.subject.session_archive, selected.subject.checkpoint_session_id, `canonical checkpoint ${stage} Grok archive`);
      }
    }
  }
...
export function checkpointForHarness(checkpoint, harness = "codex-desktop") {
  ...
  if (harness === "grok-acp" && (typeof subject.session_archive?.locator !== "string" || !/^[a-f0-9]{64}$/.test(subject.session_archive?.sha256 ?? ""))) fail(`canonical checkpoint ${checkpoint.stage} is missing ${harness} session archive`);
...
async function acceptedCheckpoint(caseDir, file, executionId, harness = "codex-desktop") {
  ...
  if (harness === "grok-acp") await verifySessionArchive(checkpoint.subject.session_archive, checkpoint.subject.checkpoint_session_id, `canonical checkpoint ${executionId} Grok archive`);
...
export async function captureCanonicalCheckpoint({ caseId, stage, revision = "REV-001", projectRoot, flowRunId, runtimeHome, canonicalSubjectSessionId, checkpointSubjectSessionId, agentId = null, harness = "codex-desktop", sessionArchive = null, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  ...
  if (!["codex-desktop", "zcode-acp", "grok-acp"].includes(harness)) fail("--harness must be codex-desktop, zcode-acp, or grok-acp");
  ...
  const archiveReceipt = harness === "grok-acp" ? await sessionArchiveReceipt(sessionArchive, checkpointSubjectSessionId, "--session-archive") : null;
...
export async function setStarterSession({ caseId, stage, sessionId, parentSessionId, harness = "codex-desktop", sessionArchive = null, seedMode = "native_fork" }) {
  ...
  if (!["codex-desktop", "zcode-acp", "grok-acp"].includes(harness)) fail("--harness must be codex-desktop, zcode-acp, or grok-acp");
  if (seedMode !== "native_fork" && !(harness === "zcode-acp" && seedMode === "deterministic_replay")) fail("--seed-mode must be native_fork, or deterministic_replay for zcode-acp");
  ...
  const archiveReceipt = harness === "grok-acp" ? await sessionArchiveReceipt(sessionArchive, sessionId, "--session-archive") : null;
...
export async function addSession({ evalRoot, executionId, role, sessionId, parentSessionId, agentId = null, harness, adapterSessionId = null, daemonId = null, observedProfile = null }) {
  ...
  context.sessions.sessions.push({ execution_id: executionId, role, harness, runtime_family: harness === "zcode-acp" ? "zcode" : harness === "grok-acp" ? "grok" : "codex", session_id: sessionId, provider_session_id: sessionId, adapter_session_id: adapterSessionId, daemon_id: daemonId, parent_session_id: parentSessionId ?? null, agent_id: agentId, observed_profile: observedProfile, expected_profile: expectedProfile ? { id: expectedProfile.id, harness: expectedProfile.harness, provider: expectedProfile.provider ?? null, model: expectedProfile.model, reasoning: expectedProfile.reasoning, mode: expectedProfile.mode ?? null } : null, recorded_at: now() }); await saveState(context); return { ok: true, idempotent: false };
}
