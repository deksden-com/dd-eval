...
function validateCase(definition, caseId) {
  ...
  if (definition.schema_id !== "dd-eval/case@4") fail("case must use dd-eval/case@4");
...
export async function judgePrepare({ evalRoot, executionId }) {
  ...
  const resultContract = await readFile(path.join(repoRoot, "schemas", "judge-result.v1.schema.json"), "utf8");
  const packet = ["# Evaluation packet", "", await readFile(await requireFile(loaded.caseDir, definition.judge_packet, "Judge packet"), "utf8"), "", "## Candidate receipt", "```json", JSON.stringify(candidate, null, 2), "```", "", "## Interaction evidence", "If the receipt includes `run/intake/hitl/...`, inspect those captured question/answer pairs. Treat an additional question as candidate behavior, not an infrastructure failure: assess whether it was materially justified and whether its answer was already available in the Subject's received context.", "", "## Required Judge result", "Return JSON only, valid against this schema. Each `criteria` entry needs an `id` and `status` (`pass`, `partial`, `fail` or `not_applicable`); `hard_invariants` is an array.", "```json", resultContract.trim(), "```", "", "## Rubric", await readFile(await requireFile(loaded.caseDir, definition.rubric, "rubric"), "utf8"), "", "## Case expectations", JSON.stringify(references.expectations, null, 2), "", "## Accepted oracle", JSON.stringify(references.oracle, null, 2)].join("\n");
