...
export async function captureCanonicalCheckpoint({ caseId, stage, revision = "REV-001", projectRoot, flowRunId, runtimeHome, canonicalSubjectSessionId, checkpointSubjectSessionId, agentId = null, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  ...
  const home = evalHome(); const checkpointRoot = path.join(home, "canonical", definition.id, revision, "checkpoints", `${stage}-entry`);
...
export async function setStarterSession({ caseId, stage, sessionId, parentSessionId }) {
  ...
  if (parentSessionId !== checkpoint.subject.checkpoint_session_id) fail("--parent-session-id does not match the protected checkpoint Session");
  ...
  registry.sessions[stage] = { session_id: sessionId, parent_session_id: parentSessionId };
...
export async function prepare({ caseId, source, output = null, controllerProfileId, subjectProfileId, judgeProfileId, stageList, segment, e2e = false, scenario = null, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  ...
  try {
    ...
    for (const execution of executions) {
      ...
      execution.subject_starter_session_id = execution.kind === "e2e" ? null : context.starters.sessions[checkpointStage].session_id;
...
export async function addSession({ evalRoot, executionId, role, sessionId, parentSessionId, agentId = null }) {
  ...
  const expectedProfile = role === "subject" ? context.manifest.profiles.subject : role === "judge" ? context.manifest.profiles.judge : null;
  context.sessions.sessions.push({ execution_id: executionId, role, session_id: sessionId, parent_session_id: parentSessionId ?? null, agent_id: agentId, expected_profile: expectedProfile ? { id: expectedProfile.id, model: expectedProfile.model, reasoning: expectedProfile.reasoning } : null, recorded_at: now() }); await saveState(context); return { ok: true, idempotent: false };
}
...
async function subjectProfileObservation(context, execution, sessionList) {
  ...
  const productiveIds = new Set(productive.map((item) => item.session_id));
  const observations = await Promise.all(productive.map(async (provider) => {
    const actual = await observedProfile(provider);
    const matches = actual.model === expected.model && actual.reasoning === expected.reasoning;
    return { session_id: provider.session_id, agent_id: provider.agent_id ?? null, worker_id: provider.worker_id ?? null, status: matches ? "matched" : "mismatch", expected, actual };
  }));
  for (const recordedSession of recorded) if (!productive.some((item) => item.session_id === recordedSession.session_id)) observations.push({ session_id: recordedSession.session_id, agent_id: recordedSession.agent_id ?? null, worker_id: null, status: "missing", expected, actual: { model: null, reasoning: null, source: "missing" } });
  return { status: observations.length > 0 && observations.every((item) => item.status === "matched") && recorded.every((item) => productiveIds.has(item.session_id)) ? "matched" : "invalid", expected, observations };
}
