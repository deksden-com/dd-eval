...
function efficiency(flow) {
  ...
  const stages = Array.isArray(flow?.status?.index?.stage_runs) ? flow.status.index.stage_runs : [];
  ...
  return { status: usage ? "available" : "unavailable", elapsed_ms, active_stage_ms, tokens: usage ? tokens : null, tool_calls: usage?.tool_calls ?? null, sessions, source: usage ? { observed_at: usage.observed_at ?? null, coverage: usage.coverage ?? null } : null };
}
