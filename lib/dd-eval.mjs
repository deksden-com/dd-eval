...
export async function validateInput({ caseId, source, requireMode: requiredMode = "authoring" }) {
  ...
  const subjectHarnesses = new Set((await Promise.all(loaded.definition.profiles.subject.map(loadProfile))).map((profile) => profile.harness));
  if (mode === "scored") {
    for (const stage of stages) {
...
export async function setStarterSession({ caseId, stage, sessionId, parentSessionId, harness = "codex-desktop", sessionArchive = null, seedMode = "native_fork" }) {
  ...
  if (harness === "codex-desktop") registry.sessions[stage] = { ...registry.sessions[stage], session_id: sessionId, parent_session_id: parentSessionId, harness };
    ...
    const entry = record(registry.sessions[stage], `starter session ${stage}`);
