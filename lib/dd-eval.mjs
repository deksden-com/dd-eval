...
export async function prepare({ caseId, source, output = null, controllerProfileId, subjectProfileId, judgeProfileId, stageList, segment, e2e = false, scenario = null, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  ...
  try {
    ...
    for (const execution of executions) {
      ...
      if (execution.kind === "e2e") promptReceipts[execution.id].prime = await copyPacket(context.caseDir, "prompts/subject-prime.md", path.join(packetRoot, "prime.md"));
...
export async function sync({ evalRoot, executionId, projectRoot, flowRunId, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  ...
  const status = await ddFlow(engine, ["run", "status", runId, "--project-root", root], options); const sessionList = await ddFlow(engine, ["stat", "run", "sessions", "ls", "--run", runId, "--project-root", root], options); const stage = execution.kind === "e2e" ? "code" : execution.kind; const usageScope = execution.kind === "e2e" ? [] : ["--stage", execution.kind.split("+").join(",")]; const usage = await ddFlow(engine, ["stat", "usage", "--run", runId, "--project-root", root, ...usageScope], options);
...
export async function stageCheckpoint({ evalRoot, executionId, stage }) {
  ...
  await writeJson(path.join(destination, "receipt.json"), receipt); lifecycle.boundaries ??= {}; lifecycle.boundaries[target] = path.relative(context.root, path.join(destination, "receipt.json")); lifecycle.status = "running"; await saveState(context);
