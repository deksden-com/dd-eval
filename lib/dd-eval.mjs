...
export async function sync({ evalRoot, executionId, projectRoot, flowRunId, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  ...
  lifecycle.status = profileObservation.status === "invalid" ? "invalid_infrastructure_flow" : stageState === "paused" || status.run?.status === "waiting_for_user" ? "waiting_for_user" : stageState === "done" ? "candidate_ready" : "running"; context.state.status = lifecycle.status === "waiting_for_user" ? "running" : context.state.status === "prepared" ? "running" : lifecycle.status === "invalid_infrastructure_flow" ? "invalid_infrastructure_flow" : context.state.status;
