...
export async function loadCase(caseId) {
  ...
  for (const stage of stages) {
    ...
    for (const key of ["rubric", "oracle", "interactions"]) await requireJson(caseDir, entry[key], `stage ${stage} ${key}`);
    const checkpoint = await requireJson(caseDir, definition.canonical_checkpoints[stage], `canonical checkpoint ${stage}`);
    if (checkpoint.schema_id !== "dd-eval/canonical-stage-checkpoint@1" || checkpoint.stage !== stage || !["pending_capture", "accepted"].includes(checkpoint.status)) fail(`invalid canonical checkpoint ${stage}`);
    if (checkpoint.status === "accepted" && (typeof checkpoint.runtime_snapshot !== "string" || !checkpoint.runtime_snapshot || !record(checkpoint.subject, `checkpoint ${stage} subject`).session_id)) fail(`accepted canonical checkpoint ${stage} is incomplete`);
  }
  for (const key of ["subject_packet", "judge_packet"]) await requireFile(caseDir, definition.e2e[key], `e2e ${key}`);
  for (const key of ["rubric", "oracle", "interactions"]) await requireJson(caseDir, definition.e2e[key], `e2e ${key}`);
  if (definition.priming) {
    await requireFile(caseDir, definition.priming.controller_packet, "priming controller packet");
    for (const [key, label] of [["subject_baselines", "subject baseline"], ["judge_baselines", "judge baseline"]]) for (const file of definition.priming[key]) await requireJson(caseDir, file, label);
  }
...
async function restoreCheckpoint({ checkpoint, projectRoot, runtimeHome, engine }) {
  ...
  try { return JSON.parse(output); } catch { fail(`dd-flow snapshot restore did not return JSON: ${output}`); }
}
...
async function acceptedCheckpoint(caseDir, file, executionId) {
  ...
  if (checkpoint.schema_id !== "dd-eval/canonical-stage-checkpoint@1" || checkpoint.stage !== executionId || checkpoint.status !== "accepted") fail(`canonical checkpoint is not accepted for ${executionId}`);
  if (typeof checkpoint.runtime_snapshot !== "string" || !checkpoint.runtime_snapshot) fail(`canonical checkpoint snapshot is missing for ${executionId}`);
  const snapshot = path.resolve(caseDir, checkpoint.runtime_snapshot);
  if (!(await exists(snapshot))) fail(`runtime snapshot is missing: ${checkpoint.runtime_snapshot}`);
  checkpoint.snapshot = snapshot;
  return checkpoint;
}

export async function captureCanonicalCheckpoint({ caseId, stage, projectRoot, flowRunId, runtimeHome, subjectSessionId, forkPoint = null, archive, output, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  const loaded = await loadCase(caseId);
  if (!stages.includes(stage)) fail(`invalid stage: ${stage}`);
  if (typeof subjectSessionId !== "string" || !subjectSessionId.trim()) fail("--subject-session is required");
  const archiveRoot = path.resolve(archive); const outputFile = path.resolve(output);
  ...
  const record = {
    schema_id: "dd-eval/canonical-stage-checkpoint@1",
    stage,
    status: "pending_capture",
    runtime_snapshot: archiveRoot,
    subject: { session_id: subjectSessionId, turn_id: forkPoint, fork_point: forkPoint },
    captured_at: now(),
...
export async function acceptCanonicalCheckpoint({ caseId, stage, recordFile }) {
  const loaded = await loadCase(caseId);
  if (!stages.includes(stage)) fail(`invalid stage: ${stage}`);
  const recordFilePath = path.resolve(recordFile); const checkpoint = await readJson(recordFilePath);
  if (checkpoint.schema_id !== "dd-eval/canonical-stage-checkpoint@1" || checkpoint.stage !== stage || checkpoint.status !== "pending_capture") fail("checkpoint record is not an acceptable pending capture");
  if (typeof checkpoint.runtime_snapshot !== "string" || !(await exists(checkpoint.runtime_snapshot))) fail("checkpoint runtime snapshot is missing");
  if (!checkpoint.subject?.session_id) fail("checkpoint Subject session is missing");
  const accepted = { ...checkpoint, status: "accepted", accepted_at: now() };
...
export async function prepare({ caseId, source, output, controllerProfileId, subjectProfileId, judgeProfileId, stageList, segment, e2e = false, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  const context = await validateInput({ caseId, source }); const selected = selectedStages(stageList); const segmentStages = selectedSegment(segment); if (selected.length > 0 && segmentStages.length > 0) fail("use exactly one of --focus or --segment"); if (selected.length === 0 && segmentStages.length === 0 && !e2e) fail("select --focus, --segment, or --e2e");
  ...
  try {
    ...
    for (const execution of executions) {
      ...
      pendingCheckpoints.push({ execution, checkpoint });
    }
    for (const execution of executions) {
    ...
    for (const { execution, checkpoint } of pendingCheckpoints) {
...
export async function sync({ evalRoot, executionId, projectRoot, flowRunId, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  ...
  await writeJson(path.join(context.root, "manifest.json"), context.manifest); await saveState(context); return { ok: true, execution: executionId, status: lifecycle.status, next_action: lifecycle.status === "waiting_for_user" ? "deliver_declared_interaction" : lifecycle.status === "candidate_ready" ? "checkpoint" : "wait_for_subject" };
}
