...
export async function withWriteLock(file, action) {
  // ponytail: one local directory lock; add a durable lease only for cross-host writers.
  const lock = `${file}.lock`; const deadline = Date.now() + 5_000;
  for (;;) {
    try { await mkdir(lock); break; }
    catch (error) { if (error.code !== "EEXIST") throw error; if (Date.now() >= deadline) fail(`timed out waiting for registry lock: ${file}`); await new Promise((resolve) => setTimeout(resolve, 25)); }
  }
...
async function archiveHash(root) {
  ...
  async function visit(directory, prefix = "") {
    ...
    for (const entry of entries.sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      if (info.isSymbolicLink()) fail(`session archive contains a symbolic link: ${rel}`);
      if (info.isDirectory()) await visit(file, rel);
      else if (info.isFile()) digest.update(rel).update("\0").update(await readFile(file)).update("\0");
      else fail(`session archive contains an unsupported entry: ${rel}`);
    }
  }
  await visit(root);
  return digest.digest("hex");
}
async function sessionArchiveReceipt(archive, sessionId, label, harness = "grok-acp") {
  if (typeof archive !== "string" || !archive) fail(`${label} is required`);
  const home = evalHome(); const root = path.resolve(archive);
  const locator = homeRelative(home, root);
  if (harness === "opencode-server") {
    const manifest = await readJson(root);
    if (manifest.schema_id !== "dd-opencode/session-archive@1" || manifest.provider_session_id !== sessionId || typeof manifest.payload !== "string") fail(`${label} does not belong to Session ${sessionId}`);
    return { locator, sha256: await materialHash(root), format: "opencode-export" };
  }
  const manifest = await readJson(path.join(root, "manifest.json"));
  if (manifest.schema_id !== "dd-grok/session-archive@1" || manifest.provider_session_id !== sessionId) fail(`${label} does not belong to Session ${sessionId}`);
  const payload = path.join(root, "session");
  if (!(await lstat(payload)).isDirectory()) fail(`${label} payload is missing`);
  return { locator, sha256: await archiveHash(root), format: "grok-session-directory" };
}
async function verifySessionArchive(receipt, sessionId, label, harness = "grok-acp") {
  record(receipt, label);
  if (typeof receipt.locator !== "string" || !/^[a-f0-9]{64}$/.test(receipt.sha256 ?? "")) fail(`${label} is invalid`);
  const root = fromHome(evalHome(), receipt.locator);
  const actual = await sessionArchiveReceipt(root, sessionId, label, harness);
  if (actual.sha256 !== receipt.sha256) fail(`${label} checksum does not match`);
  return actual;
}
...
function validateCase(definition, caseId) {
  ...
  if (!["authoring", "ready"].includes(definition.status)) fail("case.status must be authoring or ready");
  ...
  if (!["auto", "off", "standard", "deep"].includes(definition.flow.plan_review_mode)) fail("case.flow.plan_review_mode is invalid");
  if (!["auto", "off", "standard", "deep"].includes(definition.flow.code_review_mode ?? "auto")) fail("case.flow.code_review_mode is invalid");
  ...
  if (definition.e2e.stop_boundary !== "code_review_completed") fail("e2e.stop_boundary must be code_review_completed");
...
async function loadProfile(profileId) {
  ...
  if (profile.id !== profileId) fail(`profile id mismatch: ${profileId}`);
  if (!["codex-desktop", "zcode-acp", "grok-acp", "opencode-server", "antigravity-cli"].includes(profile.harness)) fail(`unsupported profile harness: ${profile.harness}`);
  if (typeof profile.model !== "string" || !profile.model || typeof profile.reasoning !== "string" || !profile.reasoning) fail(`profile ${profileId} has no model/reasoning`);
  if (["zcode-acp", "grok-acp", "opencode-server", "antigravity-cli"].includes(profile.harness) && (!profile.provider || !profile.mode)) fail(`${profile.harness} profile ${profileId} requires provider and mode`);
  const runtime_family = profile.harness === "zcode-acp" ? "zcode" : profile.harness === "grok-acp" ? "grok" : profile.harness === "opencode-server" ? "opencode" : profile.harness === "antigravity-cli" ? "antigravity" : "codex";
  const driver = profile.harness === "zcode-acp" ? "dd-zcode" : profile.harness === "grok-acp" ? "dd-grok" : profile.harness === "opencode-server" ? "dd-opencode" : profile.harness === "antigravity-cli" ? "dd-agy" : "codex-native";
  return { runtime_family, driver, ...profile };
}
...
async function loadStarterSessions(loaded, requiredHarnesses = new Set(["codex-desktop"])) {
  const registry = await readJson(path.join(loaded.caseDir, "starter-sessions.json"));
  if (registry.schema_id !== "dd-eval/starter-sessions@3" || registry.case_id !== loaded.definition.id) fail("invalid starter session registry");
  if (requiredHarnesses.has("codex-desktop")) {
  ...
  for (const stage of stages) {
    ...
    if (requiredHarnesses.has("codex-desktop")) {
      ...
      if (entry.parent_session_id !== loaded.checkpoints[stage].subject.checkpoint_session_id) fail(`starter parent is invalid for ${stage}`);
      if (entry.harness !== undefined && entry.harness !== "codex-desktop") fail(`legacy starter ${stage} must use codex-desktop`);
    }
    for (const [harness, alternate] of Object.entries(entry.by_harness ?? {})) {
      if (!["zcode-acp", "grok-acp", "opencode-server", "antigravity-cli"].includes(harness)) fail(`unsupported alternate starter harness: ${harness}`);
      if (!requiredHarnesses.has(harness)) continue;
      record(alternate, `${harness} starter session ${stage}`);
      if (typeof alternate.session_id !== "string" || !alternate.session_id.trim()) fail(`${harness} starter session ${stage} is missing`);
      if (alternate.parent_session_id !== checkpointForHarness(loaded.checkpoints[stage], harness).subject?.checkpoint_session_id) fail(`${harness} starter parent is invalid for ${stage}`);
      const seedMode = alternate.seed_mode ?? "native_fork";
      if (!(["zcode-acp", "antigravity-cli"].includes(harness) && seedMode === "deterministic_replay") && !(harness === "opencode-server" && seedMode === "archive_native_fork") && seedMode !== "native_fork") fail(`${harness} starter ${stage} has unsupported seed mode: ${seedMode}`);
      if (["grok-acp", "opencode-server"].includes(harness)) await verifySessionArchive(alternate.session_archive, alternate.session_id, `${harness} starter archive ${stage}`, harness);
    }
    for (const harness of requiredHarnesses) if (harness !== "codex-desktop") {
      if (!entry.by_harness?.[harness]?.session_id) fail(`starter session ${stage} is missing for ${harness}`);
      if (registry.harness_revisions?.[harness] !== loaded.checkpoints[stage].revision) fail(`starter session registry ${harness} revision does not match canonical checkpoint ${stage}`);
    }
  }
  return registry;
}

export async function validateInput({ caseId, source, requireMode: requiredMode = "authoring", requiredSubjectHarnesses = null }) {
  const mode = requireMode(requiredMode);
  const loaded = await loadCase(caseId); const sourceRoot = path.resolve(source);
  if (mode === "scored" && loaded.definition.status !== "ready") fail("case is not ready for scored execution");
  ...
  const executionProfile = await readJson(path.join(sourceRoot, ".memory-bank", "dd-flow", "project-execution.json"));
  if (executionProfile?.stage_session_mode !== loaded.definition.flow.handoff_mode) fail(`case flow.handoff_mode must match project execution stage_session_mode: ${executionProfile?.stage_session_mode ?? "missing"}`);
  if (executionProfile?.plan_review_mode !== loaded.definition.flow.plan_review_mode) fail(`case flow.plan_review_mode must match project execution plan_review_mode: ${executionProfile?.plan_review_mode ?? "missing"}`);
  if (executionProfile?.code_review_mode !== loaded.definition.flow.code_review_mode) fail(`case flow.code_review_mode must match project execution code_review_mode: ${executionProfile?.code_review_mode ?? "missing"}`);
  const allowedSubjectHarnesses = new Set((await Promise.all(loaded.definition.profiles.subject.map(loadProfile))).map((profile) => profile.harness));
  const subjectHarnesses = requiredSubjectHarnesses === null ? allowedSubjectHarnesses : new Set(requiredSubjectHarnesses);
  for (const harness of subjectHarnesses) if (!allowedSubjectHarnesses.has(harness)) fail(`required Subject harness is not allowed by this case: ${harness}`);
  if (mode === "scored") {
    for (const stage of stages) {
      if (loaded.checkpoints[stage].status !== "accepted") fail(`canonical checkpoint is not accepted for ${stage}`);
      const snapshot = fromHome(evalHome(), loaded.checkpoints[stage].runtime_snapshot?.locator);
      if (!(await exists(snapshot))) fail(`canonical checkpoint runtime snapshot is missing for ${stage}`);
      for (const harness of subjectHarnesses) if (harness !== "codex-desktop") {
        const selected = checkpointForHarness(loaded.checkpoints[stage], harness);
        if (!selected.subject?.checkpoint_session_id) fail(`canonical checkpoint ${stage} has no accepted ${harness} Subject`);
        const alternate = selected.runtime_snapshot?.locator;
        if (!alternate || !(await exists(fromHome(evalHome(), alternate)))) fail(`canonical checkpoint ${stage} runtime snapshot is missing for ${harness}`);
        if (["grok-acp", "opencode-server"].includes(harness)) await verifySessionArchive(selected.subject.session_archive, selected.subject.checkpoint_session_id, `canonical checkpoint ${stage} ${harness} archive`, harness);
      }
    }
  }
  const starters = mode === "scored" ? await loadStarterSessions(loaded, subjectHarnesses) : null;
  ...
  return { ...loaded, starters, judgeBaseline, sourceRoot, checkpoint, checkpointFile, sourceTree: await run("git", ["-C", sourceRoot, "rev-parse", `${checkpoint.source.commit}^{tree}`]) };
}
...
async function allocateEvalOutput(caseId, mode) {
  ...
  try {
    ...
    const roots = [path.join(home, "attempts", "active"), path.join(home, "attempts", "archive")];
    const existing = (await Promise.all(roots.map(async (root) => await exists(root) ? await readdir(root) : []))).flat();
    const highest = existing.reduce((maximum, name) => Math.max(maximum, Number(name.match(/^EVAL-(\d+)--/)?.[1] ?? 0)), 0);
    const number = Math.max(Number.isInteger(current.next) && current.next > 0 ? current.next : 1, highest + 1);
...
function subjectPrime(projectRoot) {
  return [
    `Работай в проекте \`${projectRoot}\`.`,
    `Пройди прайминг строго по абсолютному пути \`${path.join(projectRoot, ".memory-bank", "dd-flow", "prime.md")}\`.`,
    "Не подменяй этот проект другим checkout и не ищи одноимённый файл относительно внешнего cwd.",
    "После прайминга остановись и сообщи о готовности.",
  ].join("\n\n");
}
export function subjectContinuation({ stage, projectRoot, workspaceRoot = projectRoot, ddFlowHome, flowRunId, intakeFile = null, packet }) {
  ...
  const boundary = `\n\nПосле успешного \`stage finish\` для ${stage} остановись. Не запускай следующую стадию: Controller сначала синхронизирует и фиксирует результат, а затем сам передаст допустимое продолжение.`;
...
async function restoreCheckpoint({ checkpoint, projectRoot, runtimeHome, engine, expectedEngineVersion }) {
  ...
  if (identity.package_version !== expectedEngineVersion) fail(`canonical checkpoint ${checkpoint.stage} binds ${identity.package_version}, expected input checkpoint ${expectedEngineVersion}`);
  ...
  if (manifest.package_version !== expectedEngineVersion) fail(`canonical engine manifest version does not match input checkpoint for ${checkpoint.stage}`);
  return restored;
}

export function checkpointForHarness(checkpoint, harness = "codex-desktop") {
  const primaryHarness = checkpoint.harness ?? "codex-desktop";
  const primary = harness === primaryHarness;
  const runtimeSnapshot = primary ? checkpoint.runtime_snapshot : checkpoint.harness_evidence?.[harness]?.runtime_snapshot;
  const subject = primary ? checkpoint.subject : checkpoint.subject_by_harness?.[harness];
  if (typeof runtimeSnapshot?.locator !== "string" || !subject?.checkpoint_session_id) fail(`canonical checkpoint ${checkpoint.stage} is missing ${harness} evidence`);
  if (["grok-acp", "opencode-server"].includes(harness) && (typeof subject.session_archive?.locator !== "string" || !/^[a-f0-9]{64}$/.test(subject.session_archive?.sha256 ?? ""))) fail(`canonical checkpoint ${checkpoint.stage} is missing ${harness} session archive`);
  return { ...checkpoint, selected_harness: harness, runtime_snapshot: runtimeSnapshot, subject };
}

async function acceptedCheckpoint(caseDir, file, executionId, harness = "codex-desktop") {
  const source = await requireJson(caseDir, file, `canonical checkpoint ${executionId}`);
  if (source.schema_id !== "dd-eval/canonical-stage-checkpoint@2" || source.stage !== executionId || source.status !== "accepted") fail(`canonical checkpoint is not accepted for ${executionId}`);
  const checkpoint = checkpointForHarness(source, harness);
  const snapshot = fromHome(evalHome(), checkpoint.runtime_snapshot.locator);
  if (!(await exists(snapshot))) fail(`runtime snapshot is missing: ${checkpoint.runtime_snapshot.locator}`);
  if (["grok-acp", "opencode-server"].includes(harness)) await verifySessionArchive(checkpoint.subject.session_archive, checkpoint.subject.checkpoint_session_id, `canonical checkpoint ${executionId} ${harness} archive`, harness);
  return { ...checkpoint, snapshot };
}

export async function captureCanonicalCheckpoint({ caseId, stage, revision = "REV-001", projectRoot, flowRunId, runtimeHome, canonicalSubjectSessionId, checkpointSubjectSessionId, agentId = null, harness = "codex-desktop", sessionArchive = null, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  // Capture creates the target checkpoint itself.  It must therefore validate
  // the case declaration without trying to read the not-yet-created target.
  const caseDir = path.join(repoRoot, "cases", caseId);
  const definition = await readJson(path.join(caseDir, "case.json"));
  validateCase(definition, caseId);
  if (!stages.includes(stage)) fail(`invalid stage: ${stage}`);
  if (!["codex-desktop", "zcode-acp", "grok-acp", "opencode-server", "antigravity-cli"].includes(harness)) fail("unsupported --harness");
  ...
  const archiveReceipt = ["grok-acp", "opencode-server"].includes(harness) ? await sessionArchiveReceipt(sessionArchive, checkpointSubjectSessionId, "--session-archive", harness) : null;
  const home = evalHome(); const checkpointRoot = path.join(home, "canonical", definition.id, revision, "checkpoints", harness === "codex-desktop" ? `${stage}-entry` : `${stage}-entry--${harness}`);
  ...
  const record = {
    ...
    subject: { canonical_session_id: canonicalSubjectSessionId, checkpoint_session_id: checkpointSubjectSessionId, agent_id: agentId, ...(archiveReceipt ? { session_archive: archiveReceipt } : {}) },
    captured_at: now(),
...
export async function acceptCanonicalCheckpoint({ caseId, stage, recordFile, reviewFile }) {
  ...
  const acceptanceReview = { path: path.relative(caseDir, review).split(path.sep).join("/"), sha256: await materialHash(review), accepted_at: acceptedAt };
  const accepted = { ...checkpoint, status: "accepted", accepted_at: acceptedAt, acceptance_review: acceptanceReview };
  const destination = inside(caseDir, relative(definition.canonical_checkpoints[stage], `canonical checkpoint ${stage}`));
  if (!await exists(destination) || (checkpoint.harness ?? "codex-desktop") === "codex-desktop") await writeJson(destination, accepted);
  else {
    const existing = await readJson(destination);
    if (existing.status !== "accepted" || existing.revision !== checkpoint.revision) fail("non-Codex checkpoint must extend the accepted canonical revision");
    await writeJson(destination, {
      ...existing,
      subject_by_harness: { ...(existing.subject_by_harness ?? {}), [checkpoint.harness]: { ...checkpoint.subject, accepted_at: acceptedAt } },
      harness_evidence: { ...(existing.harness_evidence ?? {}), [checkpoint.harness]: { runtime_snapshot: checkpoint.runtime_snapshot, captured_at: checkpoint.captured_at, acceptance_review: acceptanceReview } }
    });
  }
  return { checkpoint: destination, stage, status: "accepted" };
}

export async function setStarterSession({ caseId, stage, sessionId, parentSessionId, harness = "codex-desktop", sessionArchive = null, seedMode = "native_fork" }) {
  ...
  if (!["codex-desktop", "zcode-acp", "grok-acp", "opencode-server", "antigravity-cli"].includes(harness)) fail("unsupported --harness");
  if (seedMode !== "native_fork" && !(["zcode-acp", "antigravity-cli"].includes(harness) && seedMode === "deterministic_replay") && !(harness === "opencode-server" && seedMode === "archive_native_fork")) fail("unsupported --seed-mode for harness");
  const protectedSessionId = checkpointForHarness(checkpoint, harness).subject?.checkpoint_session_id;
  if (!protectedSessionId) fail(`canonical checkpoint ${stage} has no accepted ${harness} Subject`);
  if (parentSessionId !== protectedSessionId) fail("--parent-session-id does not match the protected checkpoint Session");
  const archiveReceipt = ["grok-acp", "opencode-server"].includes(harness) ? await sessionArchiveReceipt(sessionArchive, sessionId, "--session-archive", harness) : null;
  const revisions = new Set(stages.map((item) => loaded.checkpoints[item].revision));
  if (harness === "codex-desktop" && revisions.size !== 1) fail("canonical checkpoints do not share one revision");
  const destination = path.join(loaded.caseDir, "starter-sessions.json");
  const expectedRevision = [...revisions][0];
  await withWriteLock(destination, async () => {
    const existing = await exists(destination) ? await readJson(destination) : null;
    const registry = !existing || existing.schema_id !== "dd-eval/starter-sessions@3" || existing.case_id !== loaded.definition.id || (harness === "codex-desktop" && existing.revision !== expectedRevision)
      ? { schema_id: "dd-eval/starter-sessions@3", case_id: loaded.definition.id, revision: expectedRevision, sessions: {} }
      : existing;
    registry.sessions = record(registry.sessions, "starter session registry sessions");
    if (harness === "codex-desktop") registry.sessions[stage] = { ...registry.sessions[stage], session_id: sessionId, parent_session_id: parentSessionId, harness };
    else {
      const entry = registry.sessions[stage] ?? {};
      registry.sessions[stage] = entry;
      entry.by_harness = { ...(entry.by_harness ?? {}), [harness]: { session_id: sessionId, parent_session_id: parentSessionId, harness, ...(seedMode !== "native_fork" ? { seed_mode: seedMode } : {}), ...(archiveReceipt ? { session_archive: archiveReceipt } : {}) } };
      registry.harness_revisions = { ...(registry.harness_revisions ?? {}), [harness]: checkpoint.revision };
    }
...
export function subjectSeedMode({ harness, kind, starter = null }) {
  if (kind === "e2e") return "clean_session";
  if (harness === "opencode-server" && starter?.seed_mode === "archive_native_fork") return "archive_native_fork";
  return ["zcode-acp", "antigravity-cli"].includes(harness) && starter?.seed_mode === "deterministic_replay" ? "deterministic_replay" : "shared_stage_starter";
}
...
export async function prepare({ caseId, source, output = null, controllerProfileId, subjectProfileId, judgeProfileId, stageList, segment, e2e = false, scenario = null, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  let context = await validateInput({ caseId, source, requireMode: "authoring" }); const selected = selectedStages(stageList); const segmentStages = selectedSegment(segment); if (selected.length > 0 && segmentStages.length > 0) fail("use exactly one of --focus or --segment"); if (selected.length === 0 && segmentStages.length === 0 && !e2e) fail("select --focus, --segment, or --e2e");
  ...
  context = await validateInput({ caseId, source, requireMode: "scored", requiredSubjectHarnesses: [profiles.subject.harness] });
  const checkpointByStage = new Map();
  for (const kind of [...executionKinds, ...(e2e ? ["e2e"] : [])]) {
    const stage = kind === "e2e" ? "specify" : kind.split("+")[0];
    if (!checkpointByStage.has(stage)) checkpointByStage.set(stage, await acceptedCheckpoint(context.caseDir, context.definition.canonical_checkpoints[stage], stage, profiles.subject.harness));
  }
  ...
  const mode = e2e && (selected.length > 0 || segmentStages.length > 0) ? "full" : e2e ? "e2e" : segment ? "segment" : "focus";
  const profileLabel = `${profiles.subject.model.replace(/^gpt-5\.6-/, "")}-${profiles.subject.reasoning}`;
  const outputRoot = output ? path.resolve(output) : await allocateEvalOutput(caseId, `${mode}--${profileLabel}`);
  const attemptsRoot = path.join(evalHome(), "attempts", "active");
  if (!path.resolve(outputRoot).startsWith(`${attemptsRoot}${path.sep}`)) fail("eval output must be inside DD_EVAL_HOME/attempts/active; canonical data is read-only input");
  ...
  try {
    ...
    for (const execution of executions) {
      ...
      if (execution.kind === "e2e") {
        const primePath = path.join(packetRoot, "prime.md");
        promptReceipts[execution.id].prime = await writePacket(primePath, subjectPrime(projectRoot), "generated:absolute-subject-prime");
      }
      const checkpointStage = execution.kind === "e2e" ? "specify" : firstStage;
      const checkpoint = checkpointByStage.get(checkpointStage);
      const starterEntry = context.starters.sessions[checkpointStage];
      const selectedStarter = profiles.subject.harness === "codex-desktop" ? starterEntry : starterEntry.by_harness?.[profiles.subject.harness];
      if (execution.kind !== "e2e" && !selectedStarter?.session_id) fail(`no ${profiles.subject.harness} starter Session for ${checkpointStage}`);
      execution.subject_starter_session_id = execution.kind === "e2e" ? null : selectedStarter.session_id;
      execution.subject_starter_harness = execution.kind === "e2e" ? null : profiles.subject.harness;
      execution.subject_starter_archive = execution.kind === "e2e" ? null : selectedStarter.session_archive ?? null;
      execution.subject_seed_mode = subjectSeedMode({ harness: profiles.subject.harness, kind: execution.kind, starter: selectedStarter });
    ...
    for (const { execution, checkpoint, definition, checkpointStage } of pendingCheckpoints) {
      ...
      const stageDefinition = execution.kind === "e2e" ? definition : context.definition.stages[checkpointStage];
      if (execution.kind === "e2e") promptReceipts[execution.id].prime = await writePacket(promptReceipts[execution.id].prime.path, subjectPrime(execution.project_root), "generated:absolute-subject-prime");
      const staticPacket = await readFile(await requireFile(context.caseDir, stageDefinition.subject_packet, "stage packet"), "utf8");
      ...
      promptReceipts[execution.id].subject = await writePacket(promptReceipts[execution.id].subject.path, subjectContinuation({ stage: checkpointStage, projectRoot: execution.project_root, workspaceRoot: execution.workspace_root, ddFlowHome: runtimeHome, flowRunId: execution.flow_run_id, intakeFile, packet: staticPacket }), `${stageDefinition.subject_packet}+generated-runtime-continuation`);
    }
    ...
    const launch = await Promise.all(executions.map(async (execution) => {
      ...
      const messages = execution.subject_seed_mode === "clean_session"
        ? [{ kind: "prime", ...promptReceipts[execution.id].prime, markdown: await readFile(promptReceipts[execution.id].prime.path, "utf8") }, { kind: "flow_request", ...continuation, markdown: await readFile(continuation.path, "utf8") }]
        : [{ kind: "stage_request", ...continuation, markdown: await readFile(continuation.path, "utf8") }];
      return {
        id: execution.id,
        next_action: ["clean_session", "deterministic_replay"].includes(execution.subject_seed_mode) ? "create_subject" : "fork_subject",
        ...
        requested_profile: { id: profileIds.subject, harness: profiles.subject.harness, runtime_family: profiles.subject.runtime_family, driver: profiles.subject.driver, provider: profiles.subject.provider ?? null, model: profiles.subject.model, reasoning: profiles.subject.reasoning, mode: profiles.subject.mode ?? null },
        ...
        subject_starter_archive: execution.subject_starter_archive,
        subject_seed_mode: execution.subject_seed_mode,
        messages,
...
async function openEval(evalRoot) {
  const root = path.resolve(evalRoot); const home = evalHome(); const allowed = [path.join(home, "attempts", "active"), path.join(home, "attempts", "archive")].some((base) => root.startsWith(`${base}${path.sep}`));
  if (!allowed) fail("eval root must be a prepared attempt under DD_EVAL_HOME/attempts; canonical checkpoints are read-only inputs");
  const manifest = await readJson(path.join(root, "manifest.json")); const state = await readJson(path.join(root, "state.json")); const sessions = await readJson(path.join(root, "sessions.json"));
...
function requireOpaqueSessionId(value, flag) {
  if (typeof value !== "string" || !value.trim() || ["undefined", "null"].includes(value.trim().toLowerCase())) fail(`${flag} must be a real provider Session ID`);
  return value.trim();
}

export async function addSession({ evalRoot, executionId, role, sessionId, parentSessionId, agentId = null, harness, adapterSessionId = null, daemonId = null, observedProfile = null }) {
  sessionId = requireOpaqueSessionId(sessionId, "--session-id");
  if (parentSessionId !== undefined && parentSessionId !== null) parentSessionId = requireOpaqueSessionId(parentSessionId, "--parent-session-id");
  const context = await openEval(evalRoot); const execution = findExecution(context.manifest, executionId); if (!/^(controller|subject_base|subject|judge)$/.test(role)) fail(`invalid role: ${role}`);
  const expectedProfile = role === "subject" ? context.manifest.profiles.subject : role === "judge" ? context.manifest.profiles.judge : role === "controller" ? context.manifest.profiles.controller : null;
  harness = harness ?? expectedProfile?.harness ?? "codex-desktop";
  if (!["codex-desktop", "zcode-acp", "grok-acp", "opencode-server", "antigravity-cli"].includes(harness)) fail("unsupported --harness");
  if (expectedProfile && harness !== expectedProfile.harness) fail(`Session harness ${harness} does not match expected ${expectedProfile.harness}`);
  if (role === "subject") {
    if (execution.subject_seed_mode === "clean_session") {
      if (parentSessionId) fail("clean E2E Subject Session must not have a parent Session");
    } else if (execution.subject_seed_mode === "deterministic_replay") {
      if (parentSessionId) fail("deterministic replay Subject Session must not claim a native parent Session");
      if (sessionId === execution.subject_starter_session_id) fail("deterministic replay Subject Session must be fresh, not the protected starter");
    } else if (execution.subject_seed_mode === "archive_native_fork") {
      if (parentSessionId) fail("archive-restored OpenCode Subject Session must not claim a native parent Session");
      if (sessionId === execution.subject_starter_session_id) fail("OpenCode Subject Session must be a fresh native fork of the imported starter");
    } else {
  ...
  if (context.sessions.sessions.some((item) => item.harness === harness && item.provider_session_id === sessionId && item.execution_id === executionId && item.role === role)) return { ok: true, idempotent: true };
  const runtimeFamily = harness === "zcode-acp" ? "zcode" : harness === "grok-acp" ? "grok" : harness === "opencode-server" ? "opencode" : harness === "antigravity-cli" ? "antigravity" : "codex";
  context.sessions.sessions.push({ execution_id: executionId, role, harness, runtime_family: runtimeFamily, session_id: sessionId, provider_session_id: sessionId, adapter_session_id: adapterSessionId, daemon_id: daemonId, parent_session_id: parentSessionId ?? null, agent_id: agentId, observed_profile: observedProfile, expected_profile: expectedProfile ? { id: expectedProfile.id, harness: expectedProfile.harness, provider: expectedProfile.provider ?? null, model: expectedProfile.model, reasoning: expectedProfile.reasoning, mode: expectedProfile.mode ?? null } : null, recorded_at: now() }); await saveState(context); return { ok: true, idempotent: false };
}
...
async function observedProfile(session) {
  if (typeof session?.transcript_path !== "string" || !(await exists(session.transcript_path))) return {
  ...
  const lines = (await readFile(session.transcript_path, "utf8")).trim().split("\n");
  for (const line of lines.reverse()) {
    try {
      const event = JSON.parse(line);
      if (event.type !== "turn_context") continue;
      const payload = event.payload ?? {};
      const model = payload.model ?? session.model ?? null;
      const reasoning = payload.effort ?? payload.collaboration_mode?.settings?.reasoning_effort ?? null;
      if (model || reasoning) return { model, reasoning, source: "turn_context" };
    } catch { /* malformed provider line cannot establish profile identity */ }
  }
  return { provider: session.provider ?? null, model: session.model ?? null, reasoning: session.reasoning ?? null, mode: session.mode ?? null, source: "run_sessions" };
}

async function subjectProfileObservation(context, execution, sessionList) {
  const expected = context.manifest.profiles.subject;
  const recorded = context.sessions.sessions.filter((item) => item.execution_id === execution.id && item.role === "subject");
  const productive = sessionList.sessions ?? [];
  const productiveIds = new Set(productive.map((item) => item.provider_session_id ?? item.session_id));
  const observations = await Promise.all(productive.map(async (provider) => {
    const providerId = provider.provider_session_id ?? provider.session_id;
    const receipt = recorded.find((item) => item.provider_session_id === providerId || item.session_id === providerId);
    const actual = receipt?.observed_profile ?? await observedProfile(provider);
    const matches = profileMatches(expected, actual, provider.harness ?? receipt?.harness ?? "codex-desktop");
    return { session_id: provider.session_id, provider_session_id: providerId, harness: provider.harness ?? receipt?.harness ?? "codex-desktop", agent_id: provider.agent_id ?? null, worker_id: provider.worker_id ?? null, status: matches ? "matched" : "mismatch", expected, actual };
  }));
  for (const recordedSession of recorded) if (!productiveIds.has(recordedSession.provider_session_id ?? recordedSession.session_id)) observations.push({ session_id: recordedSession.session_id, provider_session_id: recordedSession.provider_session_id ?? recordedSession.session_id, harness: recordedSession.harness, agent_id: recordedSession.agent_id ?? null, worker_id: null, status: "missing", expected, actual: recordedSession.observed_profile ?? { model: null, reasoning: null, source: "missing" } });
  return { status: observations.length > 0 && observations.every((item) => item.status === "matched") && recorded.every((item) => productiveIds.has(item.provider_session_id ?? item.session_id)) ? "matched" : "invalid", expected, observations };
}

export function profileMatches(expected, actual, harness) {
  return harness === expected.harness
    && actual.model === expected.model
    && actual.reasoning === expected.reasoning
    && (!expected.provider || actual.provider === expected.provider)
    && (!expected.mode || actual.mode === expected.mode);
}

export function syncLifecycleStatus({ profileStatus, stageStatus, runStatus }) {
  if (profileStatus === "invalid") return "invalid_infrastructure_flow";
  if (stageStatus === "paused" || ["paused", "waiting_for_user"].includes(runStatus)) return "waiting_for_user";
  if (stageStatus === "done") return "candidate_ready";
  return "running";
}

function terminalStage(definition, execution) {
  if (execution.kind === "e2e") return definition.e2e.stop_boundary.replace(/_completed$/, "").replaceAll("_", "-");
...
export async function sync({ evalRoot, executionId, projectRoot, flowRunId, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  ...
  const loaded = await loadCase(context.manifest.definition.case_id); const status = await ddFlow(engine, ["run", "status", runId, "--project-root", root], options); const sessionList = await ddFlow(engine, ["stat", "run", "sessions", "ls", "--run", runId, "--project-root", root], options); const stage = terminalStage(loaded.definition, execution); const usageScope = execution.kind === "e2e" ? [] : ["--stage", execution.kind.split("+").join(",")]; const usage = await ddFlow(engine, ["stat", "usage", "--run", runId, "--project-root", root, ...usageScope], options);
  ...
  const stageState = status.index?.stage_runs?.find((item) => item.stage === stage)?.status ?? null; const pausedStage = status.index?.stage_runs?.find((item) => item.status === "paused") ?? null; const interactionScript = await requireJson(loaded.caseDir, executionDefinition(loaded.definition, execution.kind).interactions, "interaction script"); const pauseOrdinal = Number(String(pausedStage?.pause?.id ?? "").match(/^HITL-(\d+)$/)?.[1] ?? 0); const declared = pauseOrdinal > 0 && interactionScript.pauses?.some((pause) => pause.stage === pausedStage.stage && pause.after_pause === pauseOrdinal); const interaction = pausedStage?.pause ? { stage: pausedStage.stage, pause_id: pausedStage.pause.id, question_path: pausedStage.pause.question_path, declared: Boolean(declared) } : null; const lifecycle = context.state.executions[executionId]; const profileObservation = await subjectProfileObservation(context, execution, sessionList); lifecycle.flow = { status, sessions: sessionList, usage, interaction, profile_observation: profileObservation };
  const completedStage = status.index?.stage_runs?.filter((item) => item.status === "done").at(-1)?.stage ?? null;
  lifecycle.status = syncLifecycleStatus({ profileStatus: profileObservation.status, stageStatus: stageState, runStatus: status.run?.status });
  if (["e2e", "segment"].includes(execution.kind === "e2e" ? "e2e" : execution.kind.includes("+") ? "segment" : "focus") && completedStage && !lifecycle.boundaries?.[completedStage] && lifecycle.status === "running") lifecycle.status = "stage_ready";
  context.state.status = lifecycle.status === "waiting_for_user" ? "running" : context.state.status === "prepared" ? "running" : lifecycle.status === "invalid_infrastructure_flow" ? "invalid_infrastructure_flow" : context.state.status;
  await writeJson(path.join(context.root, "manifest.json"), context.manifest); await saveState(context); return { ok: true, execution: executionId, status: lifecycle.status, ...(interaction ? { interaction } : {}), ...(completedStage ? { completed_stage: completedStage } : {}), next_action: lifecycle.status === "waiting_for_user" ? declared ? "deliver_declared_interaction" : "deliver_observed_interaction" : lifecycle.status === "candidate_ready" ? "checkpoint" : lifecycle.status === "stage_ready" ? "checkpoint_stage" : "wait_for_subject" };
}

export async function stageCheckpoint({ evalRoot, executionId, stage }) {
  ...
  if (!["e2e", "segment"].includes(execution.kind === "e2e" ? "e2e" : execution.kind.includes("+") ? "segment" : "focus")) fail("stage checkpoint is only valid for E2E or a multi-stage segment");
  const target = id(stage, "stage"); const stageRun = lifecycle.flow?.status?.index?.stage_runs?.find((item) => item.stage === target);
  if (stageRun?.status !== "done") fail(`stage is not done: ${target}`); if (lifecycle.boundaries?.[target]) fail(`stage boundary already captured: ${target}`);
  const source = inside(execution.run_home, relative(stageRun.dir, "stage run directory")); const destination = path.join(context.root, "boundaries", executionId, `attempt-${String(lifecycle.attempt).padStart(2, "0")}`, target);
  await cp(source, path.join(destination, "run"), { recursive: true });
  const artifacts = [];
  for (const rel of await filesBelow(destination)) { const file = inside(destination, rel); artifacts.push({ path: rel.split(path.sep).join("/"), sha256: await materialHash(file), bytes: (await stat(file)).size }); }
  const receipt = { schema_id: "dd-eval/stage-boundary@1", execution_id: executionId, attempt: lifecycle.attempt, stage: target, captured_at: now(), source_stage: stageRun.dir, artifacts };
  await writeJson(path.join(destination, "receipt.json"), receipt); lifecycle.boundaries ??= {}; lifecycle.boundaries[target] = path.relative(context.root, path.join(destination, "receipt.json"));
  const loaded = await loadCase(context.manifest.definition.case_id);
  lifecycle.status = target === terminalStage(loaded.definition, execution) ? "candidate_ready" : "running"; await saveState(context);
  return { stage: target, receipt: lifecycle.boundaries[target], artifacts: artifacts.length };
}

export async function continuation({ evalRoot, executionId, fromStage, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  const context = await openEval(evalRoot); const execution = findExecution(context.manifest, executionId);
  if (!["e2e", "segment"].includes(execution.kind === "e2e" ? "e2e" : execution.kind.includes("+") ? "segment" : "focus")) fail("successor continuation is only valid for E2E or a multi-stage segment");
  const current = id(fromStage, "from stage"); const currentIndex = stages.indexOf(current); if (currentIndex < 0 || currentIndex === stages.length - 1) fail(`stage has no successor: ${current}`);
  const target = stages[currentIndex + 1]; const lifecycle = context.state.executions[executionId];
  if (!lifecycle.boundaries?.[current]) fail(`capture ${current} boundary before creating a successor continuation`);
  const root = execution.project_root; const home = runtimeHome(execution); const status = await ddFlow(engine, ["run", "status", execution.flow_run_id, "--project-root", root], { env: { DD_FLOW_HOME: home } });
  const workspaceRoot = status.run?.workspace_root ?? root; const loaded = await loadCase(context.manifest.definition.case_id);
  const packet = await readFile(await requireFile(loaded.caseDir, loaded.definition.stages[target].subject_packet, "successor stage packet"), "utf8");
  const prompt = subjectContinuation({ stage: target, projectRoot: root, workspaceRoot, ddFlowHome: home, flowRunId: execution.flow_run_id, packet });
  const output = path.join(context.root, "executions", executionId, `attempt-${String(lifecycle.attempt).padStart(2, "0")}`, "prompts", `${target}.md`);
  await writePacket(output, prompt, `${loaded.definition.stages[target].subject_packet}+generated-successor-continuation`);
  execution.continuations ??= []; execution.continuations.push({ from_stage: current, stage: target, path: output, sha256: await materialHash(output), created_at: now() });
  await writeJson(path.join(context.root, "manifest.json"), context.manifest); return { stage: target, path: output, sha256: await materialHash(output), prompt };
}

async function sourceFiles(execution, descriptor) {
  record(descriptor, "candidate file");
  if (!["project", "workspace", "run"].includes(descriptor.origin)) fail("candidate file origin must be project, workspace or run");
  const root = descriptor.origin === "project" ? execution.project_root : descriptor.origin === "workspace" ? execution.workspace_root : execution.run_home;
  if (!root) fail(`candidate file origin is unavailable: ${descriptor.origin}`);
  if (descriptor.origin === "workspace" && descriptor.path === "{code_changed_files}") {
    const report = await readJson(path.join(execution.run_home, "05-code", "stage-report.json"));
    const paths = [...new Set(report?.semantic?.changed_files ?? [])];
    if (paths.length === 0) fail("CODE report has no changed_files");
    return paths.map((candidatePath) => ({ source: inside(root, relative(candidatePath, "CODE changed file")), destination: path.join("workspace", candidatePath) }));
  }
  let candidatePath = descriptor.path;
  if (candidatePath.includes("{protocol_id}")) { const report = await readJson(path.join(execution.run_home, "03-plan", "stage-report.json")); const ids = [...new Set(report?.semantic?.acceptance ?? report?.artifacts?.protocol_ids ?? report?.protocol_ids ?? [])]; if (ids.length !== 1) fail(`candidate protocol_id is ambiguous: ${ids.join(", ") || "none"}`); candidatePath = candidatePath.replaceAll("{protocol_id}", ids[0]); }
  return [{ source: inside(root, relative(candidatePath, "candidate file path")), destination: path.join(descriptor.origin, candidatePath) }];
}
...
export async function checkpoint({ evalRoot, executionId }) {
  ...
  const artifacts = [];
  await cp(execution.run_home, path.join(destination, "run"), { recursive: true });
  if (execution.kind === "e2e" || execution.kind.split("+").some((item) => ["code", "code-review"].includes(item))) {
    const changed = [...new Set([
      ...(await run("git", ["diff", "--name-only", "HEAD"], { cwd: execution.workspace_root })).split("\n"),
      ...(await run("git", ["ls-files", "--others", "--exclude-standard"], { cwd: execution.workspace_root })).split("\n"),
    ].map((item) => item.trim()).filter(Boolean))];
    for (const changedPath of changed) {
      const source = inside(execution.workspace_root, relative(changedPath, "changed workspace file"));
      if (!(await exists(source)) || !(await stat(source)).isFile()) continue;
      const target = inside(destination, path.join("workspace", changedPath)); await mkdir(path.dirname(target), { recursive: true }); await cp(source, target);
    }
    const evidence = path.join(destination, "git"); await mkdir(evidence, { recursive: true });
    await writeFile(path.join(evidence, "status.txt"), `${await run("git", ["status", "--short"], { cwd: execution.workspace_root })}\n`);
    await writeFile(path.join(evidence, "diff.patch"), `${await run("git", ["diff", "--binary", "HEAD"], { cwd: execution.workspace_root })}\n`);
  }
  if (execution.kind !== "e2e") for (const descriptor of definition.candidate_files) for (const file of await sourceFiles(execution, descriptor)) { if (!(await exists(file.source))) fail(`candidate artifact is missing: ${file.destination}`); const target = inside(destination, file.destination); await mkdir(path.dirname(target), { recursive: true }); await cp(file.source, target); }
...
/** Rebuild evidence after a harness-only capture fix without rerunning Subject work. */
export async function rematerializeCandidate({ evalRoot, executionId }) {
  const context = await openEval(evalRoot); const lifecycle = context.state.executions[executionId];
  if (!lifecycle?.candidate) fail("an existing candidate is required for rematerialization");
  const previous = lifecycle.candidate; lifecycle.attempt += 1; lifecycle.candidate = null; lifecycle.status = "candidate_ready";
  lifecycle.incidents ??= []; lifecycle.incidents.push({ at: now(), kind: "candidate_rematerialized", previous, reason: "harness evidence capture correction; Subject execution unchanged" });
  await saveState(context); const result = await checkpoint({ evalRoot, executionId }); return { ...result, previous_candidate: previous, subject_rerun: false };
}
...
function judgmentNumber(lifecycle) { return (lifecycle.judgments?.length ?? 0) + (lifecycle.rejected_judgments?.length ?? 0) + 1; }
...
export function judgeResultInstructions(resultPath) {
  return [
    ...
    "Use the exact schema and fill-in template below; do not invent different field names. `scope` must be the exact lowercase scope named in the packet (for example `specify`). Each finding `id` must be lowercase kebab-case (for example `finding-001`). Score every listed outcome and flow criterion from 0 through 4; use `score: null, not_applicable: true` only with a rationale. Findings must state evidence and practical impact. Do not return weights or calculated totals.",
    "After writing and checking the JSON, stop. A Controller will accept that exact file deterministically.",
  ].join("\n");
}

export function judgeResultTemplate(scope, assessment) {
  const criterion = (item) => ({ id: item.id, score: 0, not_applicable: false, rationale: "Explain the score from candidate evidence.", evidence: ["candidate evidence path"] });
  return {
    schema_id: "dd-eval/judge-result@2", scope, run_validity: "valid",
    outcome: assessment.outcome.map(criterion), flow: assessment.flow.map(criterion),
    findings: [{ id: "finding-001", severity: "material", summary: "Concise finding, or remove this array entry when there is no finding.", evidence: ["candidate evidence path"], impact: "Practical consequence." }],
    golden: { covered: [], missed: [], alternatives: [], novel: [] },
    conclusion: "Concise evidence-based conclusion."
  };
}
...
export async function judgePrepare({ evalRoot, executionId, rejudge = false }) {
  ...
  requireScoredUsage(lifecycle.flow, executionId);
  ...
  const packet = [
    ...
    "", "## Candidate evidence boundary", `Read any file recursively under \`${path.dirname(path.join(context.root, lifecycle.candidate))}\`. This directory is the complete read-only evidence boundary. Do not read candidate evidence outside it.`,
    ...
    "", judgeResultInstructions(resultPath), "### Schema", "```json", resultContract.trim(), "```", "### Fill-in template", "```json", JSON.stringify(judgeResultTemplate(scope, assessment), null, 2), "```"
  ].join("\n");
  ...
  await writeJson(contextPath, { scope, judgment, result_path: resultPath, candidate_evidence_root: path.dirname(path.join(context.root, lifecycle.candidate)), assessment_sha256: context.manifest.definition.assessment_sha256, methodology_sha256: context.manifest.definition.methodology_sha256, candidate: lifecycle.candidate });
...
export async function judgeReject({ evalRoot, executionId, reason }) {
  if (typeof reason !== "string" || !reason.trim()) fail("--reason is required");
  const context = await openEval(evalRoot); const execution = findExecution(context.manifest, executionId); const lifecycle = context.state.executions[executionId];
  if (lifecycle.status !== "judging" || !Number.isInteger(lifecycle.pending_judgment)) fail("Judge is not prepared");
  lifecycle.rejected_judgments ??= [];
  lifecycle.rejected_judgments.push({ judgment: lifecycle.pending_judgment, packet: lifecycle.judge_packet ?? null, context: lifecycle.judge_context ?? null, reason: reason.trim(), rejected_at: now() });
  lifecycle.pending_judgment = null; lifecycle.status = "candidate_ready"; context.state.status = "running";
  await saveState(context); return { ok: true, execution: execution.id, status: lifecycle.status, rejected_judgment: lifecycle.rejected_judgments.at(-1).judgment };
}
...
export function efficiency(flow, scope) {
  const usage = flow?.usage; const groups = Array.isArray(usage?.groups) ? usage.groups : []; const tokens = usage?.totals ? usage.totals : {};
  if (!usage?.totals) for (const group of groups) for (const [key, value] of Object.entries(group.tokens ?? {})) tokens[key] = tokens[key] === undefined ? value : tokens[key] === null || value === null ? null : tokens[key] + value;
  const sessions = Array.isArray(usage?.sessions) ? new Set(usage.sessions.map((item) => item.session_id)).size : Array.isArray(flow?.sessions?.sessions) ? flow.sessions.sessions.length : null;
  const allStages = Array.isArray(flow?.status?.index?.stage_runs) ? flow.status.index.stage_runs : [];
  const selectedStages = scope.split("+");
  const stages = scope === "e2e" ? allStages : allStages.filter((stage) => selectedStages.includes(stage.stage));
  const started = stages.map((stage) => Date.parse(stage.started_at)).filter(Number.isFinite); const completed = stages.map((stage) => Date.parse(stage.completed_at)).filter(Number.isFinite);
  const active_stage_ms = stages.reduce((sum, stage) => { const begin = Date.parse(stage.started_at); const end = Date.parse(stage.completed_at); return Number.isFinite(begin) && Number.isFinite(end) ? sum + Math.max(0, end - begin) : sum; }, 0);
  const elapsed_ms = started.length && completed.length ? Math.max(...completed) - Math.min(...started) : null;
  return { status: usage ? "available" : "unavailable", scope: usage?.scope ?? { kind: scope === "e2e" ? "run" : "stage", stage: scope }, elapsed_ms, active_stage_ms, tokens: usage ? tokens : null, tool_calls: usage?.tool_calls ?? null, sessions, source: usage ? { observed_at: usage.observed_at ?? null, coverage: usage.coverage ?? null } : null };
}

export function scoredUsageIssues(flow) {
  const usage = flow?.usage; const issues = [];
  if (!usage) return ["usage is unavailable"];
  if (usage.status !== "final") issues.push(`usage status is ${usage.status ?? "unknown"}`);
  const unsettled = usage.session_reconciliation?.unsettled ?? [];
  if (unsettled.length) issues.push(`${unsettled.length} Session(s) are unsettled`);
  if (usage.tool_calls?.status !== "measured") issues.push(`tool-call coverage is ${usage.tool_calls?.status ?? "unavailable"}`);
  return issues;
}

function requireScoredUsage(flow, executionId) { const issues = scoredUsageIssues(flow); if (issues.length) fail(`execution ${executionId} is not ready for scoring: ${issues.join("; ")}`); }
...
export async function finalize({ evalRoot }) {
  ...
  for (const execution of context.manifest.executions) {
    ...
    requireScoredUsage(lifecycle.flow, execution.id);
    ...
    let complete = Boolean(execution.project_root && execution.run_home && await exists(execution.project_root) && await exists(execution.run_home) && lifecycle.candidate);
    if (complete) {
      const candidateFile = path.join(context.root, lifecycle.candidate); const candidateRoot = path.dirname(candidateFile);
      const candidate = await readJson(candidateFile);
      complete = candidate.artifacts.length > 0 && (await Promise.all(candidate.artifacts.map(async (artifact) => {
        const file = inside(candidateRoot, relative(artifact.path, "candidate artifact"));
        return await exists(file) && await materialHash(file) === artifact.sha256;
      }))).every(Boolean);
    }
    executions.push({ id: execution.id, scope, ...scored, findings: evaluation.findings, golden: evaluation.golden, conclusion: evaluation.conclusion, evaluation: lifecycle.evaluation, evaluations: lifecycle.judgments ?? [lifecycle.evaluation], candidate: lifecycle.candidate, efficiency: efficiency(lifecycle.flow, execution.kind), evidence_completeness: complete ? "complete" : "limited" });
  }
