...
const operatorMaterialNames = [
  "initial_prompt",
  "clarification_packet",
  "reference_specification",
  "reference_plan",
  "review_prompt",
  "controller_initial_prompt",
  "controller_clarification_prompt"
];

function relativeMaterialPath(value, name) {
  if (typeof value !== "string" || value.length === 0) fail(`invalid operator material ${name}`);
  const absolute = path.resolve(repoRoot, value);
  const relative = path.relative(repoRoot, absolute);
  if (relative === "" || relative.startsWith(`..${path.sep}`) || relative === ".." || path.isAbsolute(relative)) {
    fail(`operator material ${name} must stay inside dd-eval`);
  }
  return relative;
}

export function resolveOperatorMaterials(caseDir, trackDefinition, checkpoint) {
  const relativeCase = path.relative(repoRoot, caseDir);
  const defaults = {};
  for (const name of operatorMaterialNames) {
    const value = trackDefinition[name];
    if (value === undefined) continue;
    if (typeof value !== "string" || value.length === 0) fail(`invalid track operator material ${name}`);
    defaults[name] = path.join(relativeCase, value);
  }
  if (!defaults.initial_prompt) fail("track initial_prompt is required");
  const overrides = checkpoint.operator_material_overrides;
  if (overrides !== undefined && (!overrides || typeof overrides !== "object" || Array.isArray(overrides))) {
    fail("checkpoint operator_material_overrides must be an object");
  }
  for (const [name, value] of Object.entries(overrides || {})) {
    if (!operatorMaterialNames.includes(name) || typeof value !== "string") {
      fail(`invalid checkpoint operator material override: ${name}`);
    }
    defaults[name] = value;
  }
  return Object.fromEntries(Object.entries(defaults).map(([name, value]) => [
    name,
    value === null ? null : relativeMaterialPath(value, name)
  ]));
}
...
export async function prepare({ caseId, checkpointId, profileId, track = "planning", source, output }) {
  ...
  const operatorMaterials = resolveOperatorMaterials(context.caseDir, trackDefinition, context.checkpoint);
  const operatorMaterialSha256 = Object.fromEntries(
    await Promise.all(Object.entries(operatorMaterials).map(async ([name, relativePath]) => [
      name,
      relativePath
  ...
  const controllerPrompt = `${outputRoot}.controller-initial.md`;
  ...
  try {
    ...
    const intakePath = path.posix.join(".tasks", "dd-flow", "intake", caseId, "initial-request.md");
    const intakeSource = path.join(repoRoot, operatorMaterials.initial_prompt);
    const intakeBytes = await readFile(intakeSource);
    await mkdir(path.join(stage, path.dirname(intakePath)), { recursive: true });
    await writeFile(path.join(stage, intakePath), intakeBytes);
    await rename(stage, outputRoot);
    const controllerSource = operatorMaterials.controller_initial_prompt
      ? await readFile(path.join(repoRoot, operatorMaterials.controller_initial_prompt), "utf8")
      : null;
    const renderedController = controllerSource
      ? controllerSource
        .replaceAll("{{RUN_REPOSITORY}}", outputRoot)
        .replaceAll("{{INTAKE_FILE}}", path.join(outputRoot, intakePath))
      : null;
    if (renderedController !== null) await writeFile(controllerPrompt, renderedController);
    await writeFile(
      runManifest,
      `${JSON.stringify({
        schema_version: 1,
        ...
        operator_materials: operatorMaterials,
        operator_material_sha256: operatorMaterialSha256,
        intake: {
          path: intakePath,
          sha256: createHash("sha256").update(intakeBytes).digest("hex"),
          source_material: operatorMaterials.initial_prompt
        },
        ...(renderedController !== null ? {
          controller_prompt: {
            path: controllerPrompt,
            sha256: createHash("sha256").update(renderedController).digest("hex"),
            source_material: operatorMaterials.controller_initial_prompt
          }
        } : {})
      }, null, 2)}\n`
    );
    return { output: outputRoot, runManifest, inputCommit, inputTree, ...(renderedController !== null ? { controllerPrompt } : {}) };
  } finally {
    await rm(stage, { recursive: true, force: true });
    await rm(archive, { force: true });
  }
  ...
  const result = {
    ...
    flow_observability: {
      ...
      unattributed_flow_seconds:
        typeof flowTimeline?.summary?.elapsed_ms !== "number"
          || stageElapsedMs === null
          || flowTimeline.summary.elapsed_ms < stageElapsedMs
          ? null
