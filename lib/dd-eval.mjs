...
export async function loadCase(caseId) {
  ...
  for (const stage of stages) {
    ...
    if (entry.fixture) await requireJson(caseDir, entry.fixture, `stage ${stage} fixture`);
  }
...
async function importFixture({ fixture, projectRoot, runtimeHome, engine }) { const output = await run(engine, ["run", "fixture", "import", "--fixture", fixture, "--project-root", projectRoot, "--json"], { env: { DD_FLOW_HOME: runtimeHome } }); try { return JSON.parse(output); } catch { fail(`dd-flow fixture import did not return JSON: ${output}`); } }
...
export async function prepare({ caseId, source, output, controllerProfileId, subjectProfileId, judgeProfileId, stageList, e2e = false, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  ...
  const runId = `eval-${randomUUID()}`; const executions = [...selected, ...(e2e ? ["e2e"] : [])].map((kind) => ({ id: kind, kind, attempt: 1, status: "prepared", flow_run_id: null, run_home: null, dd_flow_home: null, project_root: null, sessions: [] }));
  const temporary = `${outputRoot}.tmp-${process.pid}`; const pendingFixtures = []; let published = false; await rm(temporary, { recursive: true, force: true });
  try {
    ...
    for (const execution of executions) {
      ...
      if (execution.kind !== "specify" && execution.kind !== "e2e") {
        const fixture = await requireFile(context.caseDir, definition.fixture, `fixture ${execution.kind}`); await acceptedFixture(fixture, execution.kind); pendingFixtures.push({ execution, fixture });
      }
    }
    for (const execution of executions) {
      ...
      if (execution.kind === "e2e") execution.dd_flow_home = path.join(outputRoot, "dd-flow-home");
    }
    ...
    for (const { execution, fixture } of pendingFixtures) {
      const runtimeHome = path.join(path.dirname(execution.project_root), "dd-flow-home"); const imported = await importFixture({ fixture, projectRoot: execution.project_root, runtimeHome, engine }); execution.flow_run_id = imported.run_id ?? null; execution.run_home = imported.run_home ?? imported.runtime_workspace ?? runtimeHome; execution.dd_flow_home = runtimeHome;
