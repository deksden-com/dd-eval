...
async function loadProfile(profileId) {
  ...
  if (!["codex-desktop", "zcode-acp"].includes(profile.harness)) fail(`unsupported profile harness: ${profile.harness}`);
  ...
  if (profile.harness === "zcode-acp" && (!profile.provider || !profile.mode)) fail(`ZCode profile ${profileId} requires provider and mode`);
  return { runtime_family: profile.harness === "zcode-acp" ? "zcode" : "codex", driver: profile.harness === "zcode-acp" ? "dd-zcode" : "codex-native", ...profile };
}
...
async function loadStarterSessions(loaded, requiredHarnesses = new Set(["codex-desktop"])) {
  ...
  for (const stage of stages) {
    ...
    for (const [harness, alternate] of Object.entries(entry.by_harness ?? {})) {
      if (harness !== "zcode-acp") fail(`unsupported alternate starter harness: ${harness}`);
...
export async function captureCanonicalCheckpoint({ caseId, stage, revision = "REV-001", projectRoot, flowRunId, runtimeHome, canonicalSubjectSessionId, checkpointSubjectSessionId, agentId = null, harness = "codex-desktop", engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  ...
  if (!["codex-desktop", "zcode-acp"].includes(harness)) fail("--harness must be codex-desktop or zcode-acp");
...
export async function addSession({ evalRoot, executionId, role, sessionId, parentSessionId, agentId = null, harness, adapterSessionId = null, daemonId = null, observedProfile = null }) {
  ...
  context.sessions.sessions.push({ execution_id: executionId, role, harness, runtime_family: harness === "zcode-acp" ? "zcode" : "codex", session_id: sessionId, provider_session_id: sessionId, adapter_session_id: adapterSessionId, daemon_id: daemonId, parent_session_id: parentSessionId ?? null, agent_id: agentId, observed_profile: observedProfile, expected_profile: expectedProfile ? { id: expectedProfile.id, harness: expectedProfile.harness, provider: expectedProfile.provider ?? null, model: expectedProfile.model, reasoning: expectedProfile.reasoning, mode: expectedProfile.mode ?? null } : null, recorded_at: now() }); await saveState(context); return { ok: true, idempotent: false };
}
