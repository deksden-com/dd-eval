  ...
  try {
    ...
    for (const execution of executions) {
      ...
      if (execution.kind !== "specify" && execution.kind !== "e2e") {
        const fixture = await requireFile(context.caseDir, definition.fixture, `fixture ${execution.kind}`); await acceptedFixture(fixture, execution.kind); const runtimeHome = path.join(root, "dd-flow-home"); const imported = await importFixture({ fixture, projectRoot, runtimeHome, engine }); execution.flow_run_id = imported.run_id ?? null; execution.run_home = imported.run_home ?? imported.runtime_workspace ?? runtimeHome;
