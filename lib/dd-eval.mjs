...
export async function sync({ evalRoot, executionId, projectRoot, flowRunId, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  ...
  lifecycle.status = syncLifecycleStatus({ profileStatus: profileObservation.status, stageStatus: stageState, runStatus: status.run?.status }); context.state.status = lifecycle.status === "waiting_for_user" ? "running" : context.state.status === "prepared" ? "running" : lifecycle.status === "invalid_infrastructure_flow" ? "invalid_infrastructure_flow" : context.state.status;
...
export async function continuation({ evalRoot, executionId, fromStage, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  ...
  if (!lifecycle.flow?.status?.index?.stage_runs?.some((item) => item.stage === current && item.status === "done")) fail(`cannot continue before ${current} is done`);
