...
function validateCase(definition, caseId) {
  ...
  for (const stage of stages) {
    ...
    for (const key of ["subject_packet", "judge_packet", "rubric", "expectations", "interactions"]) relative(entry[key], `stage ${stage} ${key}`);
  ...
  for (const key of ["subject_packet", "judge_packet", "rubric", "expectations", "interactions"]) relative(definition.e2e[key], `e2e ${key}`);
...
export async function loadCase(caseId) {
  ...
  for (const stage of stages) {
    ...
    for (const key of ["rubric", "expectations", "interactions"]) await requireJson(caseDir, entry[key], `stage ${stage} ${key}`);
  ...
  for (const key of ["rubric", "expectations", "interactions"]) await requireJson(caseDir, definition.e2e[key], `e2e ${key}`);
...
export async function prepare({ caseId, source, output = null, controllerProfileId, subjectProfileId, judgeProfileId, stageList, segment, e2e = false, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
...
export async function addSession({ evalRoot, executionId, role, sessionId, parentSessionId, agentId = null }) {
  const context = await openEval(evalRoot); findExecution(context.manifest, executionId); if (!/^(controller|subject_base|subject|judge)$/.test(role)) fail(`invalid role: ${role}`); if (typeof sessionId !== "string" || !sessionId.trim()) fail("--session-id is required");
...
export async function judgePrepare({ evalRoot, executionId }) {
  ...
  const expectationsFile = await requireFile(loaded.caseDir, definition.expectations, "expectations"); const expectations = await readJson(expectationsFile); if (expectations.status !== "accepted") fail(`expectations are not accepted for ${executionId}`);
  const packet = ["# Evaluation packet", "", await readFile(await requireFile(loaded.caseDir, definition.judge_packet, "Judge packet"), "utf8"), "", "## Candidate receipt", "```json", JSON.stringify(candidate, null, 2), "```", "", "## Rubric", await readFile(await requireFile(loaded.caseDir, definition.rubric, "rubric"), "utf8"), "", "## Case expectations", JSON.stringify(expectations, null, 2)].join("\n");
