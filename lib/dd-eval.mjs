...
export async function judgePrepare({ evalRoot, executionId }) {
  ...
  const packet = ["# Evaluation packet", "", await readFile(await requireFile(loaded.caseDir, definition.judge_packet, "Judge packet"), "utf8"), "", "## Candidate receipt", "```json", JSON.stringify(candidate, null, 2), "```", "", "## Rubric", await readFile(await requireFile(loaded.caseDir, definition.rubric, "rubric"), "utf8"), "", "## Case expectations", JSON.stringify(references.expectations, null, 2), "", "## Accepted oracle", JSON.stringify(references.oracle, null, 2)].join("\n");
