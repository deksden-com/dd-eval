...
export async function prepare({ caseId, source, output, controllerProfileId, subjectProfileId, judgeProfileId, stageList, e2e = false, engine = process.env.DD_FLOW_BIN || "dd-flow" }) {
  ...
  try {
    ...
    for (const execution of executions) { execution.project_root = finalPath(execution.project_root, temporary, outputRoot); execution.run_home = finalPath(execution.run_home, temporary, outputRoot); }
    ...
    for (const { execution, fixture } of pendingFixtures) {
      const runtimeHome = path.join(path.dirname(execution.project_root), "dd-flow-home"); const imported = await importFixture({ fixture, projectRoot: execution.project_root, runtimeHome, engine }); execution.flow_run_id = imported.run_id ?? null; execution.run_home = imported.run_home ?? imported.runtime_workspace ?? runtimeHome;
