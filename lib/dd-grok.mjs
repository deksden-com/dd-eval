...
async function cancelTree(bridge, sessionId) {
  ...
  for (let attempt = 0; attempt < 50; attempt += 1) {
    ...
    if (!(after.subagents ?? []).length) {
      const root = await sessionInfo(bridge, sessionId);
      const status = root?.status ?? root?.state ?? null;
      return { before, after, cancellations, root_status: status, settled: ["completed", "failed", "cancelled", "stopped"].includes(String(status).toLowerCase()) };
    }
  }
