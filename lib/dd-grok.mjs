...
export async function promptSessionWithBridge(bridge, options) {
  ...
  const cursor = bridge.toolCursor(); const textCursor = bridge.assistantTextCursor(sessionId); const turn = await bridge.request("session/prompt", { sessionId, prompt: [{ type: "text", text: prompt }] }, options.timeoutMs ?? 1_800_000); await bridge.flush();
