...
function bridgeOptions(options) {
  ...
  return {
    ...
    env: { ...(options.grokHome ? { GROK_HOME: absolute(options.grokHome, "--grok-home") } : {}), ...(options.authPath ? { GROK_AUTH_PATH: absolute(options.authPath, "--auth-path") } : {}) },
...
export async function createSessionWithBridge(bridge, options, initialized) {
  ...
  const local = { ...options, initialized, rootProviderSessionId: providerSessionId }; await forwardUsage(local, providerSessionId, await sessionUsage(bridge, providerSessionId), bridge.toolSummary());
  const turn = await bridge.request("session/prompt", { sessionId: providerSessionId, prompt: [{ type: "text", text: prompt }] }, options.timeoutMs ?? 1_800_000);
  await bridge.flush(); const evidence = await inspect(bridge, providerSessionId, local); evidence.tool_calls = bridge.toolSummary(); await forwardUsage(local, providerSessionId, evidence.usage, evidence.tool_calls);
...
export async function promptSessionWithBridge(bridge, options) {
  ...
  const local = { ...options, rootProviderSessionId: options.rootProviderSessionId ?? sessionId }; await forwardUsage(local, sessionId, await sessionUsage(bridge, sessionId), bridge.toolSummary());
  const turn = await bridge.request("session/prompt", { sessionId, prompt: [{ type: "text", text: prompt }] }, options.timeoutMs ?? 1_800_000); await bridge.flush();
  const evidence = await inspect(bridge, sessionId, local); evidence.tool_calls = bridge.toolSummary(); await forwardUsage(local, sessionId, evidence.usage, evidence.tool_calls);
...
export async function inspectSessionWithBridge(bridge, options) { requireJournal(options); const sessionId = await identity(bridge, options); return receipt(options, { harness: "grok-acp", provider_session_id: sessionId, adapter_session_id: sessionId, ...await inspect(bridge, sessionId, options) }); }
...
export async function forkSessionWithBridge(bridge, options) {
  ...
  const result = unwrap(await bridge.request(extension("x.ai/session/fork"), { sourceSessionId: sessionId, sourceCwd: absolute(options.cwd, "--cwd"), ...target }));
