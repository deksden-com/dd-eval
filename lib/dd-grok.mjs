...
function mergeToolCalls(before, delta) { const by_tool = { ...(before?.by_tool ?? {}) }; for (const [name, count] of Object.entries(delta.by_tool ?? {})) by_tool[name] = (by_tool[name] ?? 0) + count; return { total: (before?.total ?? 0) + (delta.total ?? 0), failures: (before?.failures ?? 0) + (delta.failures ?? 0), by_tool }; }
...
function recordToolCalls(options, bridge, sessionId, cursor) { const result = options.toolUsage ? mergeToolCalls(sessionToolCalls(options, bridge, sessionId), bridge.toolSummarySince(cursor)) : bridge.toolSummary(); options.toolUsage?.set(sessionId, result); return result; }
