...
const GROK_HARNESS_CONTRACT = "dd-grok-harness@1";
...
export function observedProfile(initialized, requested) {
  ...
  const model = state.currentModelId ?? null;
  const available = (state.availableModels ?? []).find((item) => item?.modelId === model);
  return { provider: null, model, reasoning: available?._meta?.reasoningEffort ?? null, mode: null };
}
...
async function identity(bridge, options) {
  ...
  bridge.registerModelSession?.(sessionId, sessionId, profile(options), options.rootProviderSessionId && options.rootProviderSessionId !== sessionId ? options.rootProviderSessionId : null);
  return sessionId;
}

async function cancelTree(bridge, sessionId) {
  const before = await subagents(bridge, sessionId).catch(error => ({ observation_error: { code: error.code, message: error.message } })); const cancellations = [];
  for (const child of before.subagents ?? []) {
    const childId = child.sessionId ?? child.subagentId;
    if (childId) {
      try { cancellations.push(await bridge.request(extension("x.ai/subagent/cancel"), { sessionId, subagentId: childId })); }
      catch (error) { cancellations.push({ session_id: childId, cancelled: false, error: { code: error.code, message: error.message } }); }
    }
  }
  bridge.notify("session/cancel", { sessionId });
  for (let attempt = 0; attempt < 50; attempt += 1) {
    await new Promise((resolve) => setTimeout(resolve, 100));
    const after = await subagents(bridge, sessionId).catch(error => ({ observation_error: { code: error.code, message: error.message } }));
    if (after.observation_error) return { before, after, cancellations, settled: false };
    if (!(after.subagents ?? []).length) {
      const root = await sessionInfo(bridge, sessionId).catch(error => ({ observation_error: { code: error.code, message: error.message } }));
      const status = root?.status ?? root?.state ?? root?.data?.status ?? null;
      return { before, after, cancellations, root_status: status, settled: ["completed", "failed", "cancelled", "stopped", "idle"].includes(String(status).toLowerCase()) };
    }
  }
  const root = await sessionInfo(bridge, sessionId).catch(() => null);
  return { before, after: await subagents(bridge, sessionId).catch(() => ({ subagents: [] })), cancellations, root_status: root?.status ?? root?.state ?? null, settled: false };
}
...
export async function doctor(options = {}) {
  ...
  return { compatible: true, observed_runtime: { grok: version, acp: 1, dd_harness_contract: GROK_HARNESS_CONTRACT } };
}
...
export async function createSessionWithBridge(bridge, options, initialized) {
  requireJournal(options); const cwd = absolute(options.cwd, "--cwd"); const requested = requiredProfile(options);
  ...
  await bridge.flush(); const evidence = await inspect(bridge, providerSessionId, local); evidence.tool_calls = sessionToolCalls(local, bridge, providerSessionId); await forwardUsage(local, providerSessionId, evidence.usage, evidence.tool_calls);
  return receipt(options, { harness: "grok-acp", runtime_family: "grok", provider_session_id: providerSessionId, adapter_session_id: providerSessionId, cwd, initialized, profile: profileReceipt, evidence });
}
...
export async function promptSessionWithBridge(bridge, options) {
  ...
  const cursor = bridge.toolCursor(); const textCursor = bridge.assistantTextCursor(sessionId); const turn = await bridge.request("session/prompt", { sessionId, prompt: [{ type: "text", text: prompt }] }, null, { idleSessionId: sessionId, idleTimeoutMs: options.timeoutMs ?? 600_000 }); await bridge.flush();
  ...
  return receipt(options, { harness: "grok-acp", provider_session_id: sessionId, adapter_session_id: sessionId, assistant_text: bridge.assistantTextSince(sessionId, textCursor), turn, profile: profileReceipt, evidence });
}
