...
**P8 — контракт неопределённости SPECIFY и надёжность HITL fixture.**
Разделить безопасное обратимое предположение и решение, меняющее
наблюдаемое поведение/права/внешний API. Последнее, если его нет в запросе
или авторитетном источнике, должно вести к `stage pause` либо явному
узкому исключению из scope, а не скрытому придуманному default. Пересмотреть
порядок resolution gate в общем flow-pack SPECIFY template; проверить
`task-priority` как конкретный пример, не зашивать названия уровней в engine.
Для scored проверки обязательного HITL сделать неразрешённость важного решения
явной в исходном пользовательском задании; canonical ответ оставить скрытым.
Так тест будет проверять работающий pause/resume, а не случайную склонность
модели задавать вопросы. Добавить prompt/fixture regression: неполный
приоритетный контракт → вопрос, ясное мелкое обратимое решение → default без
лишнего HITL, уже заданное решение → без повтора. До этого не считать AGY
beta.104 квалифицированным по E2E и не править старый EVAL.
