...
3. Выполнять registry readback для metadata и tarball до появления опубликованного артефакта или явной отмены workflow. Повторять временные ошибки без общего deadline; auth/conflict завершать сразу. Ограничивать отдельный сетевой вызов, чтобы зависший запрос не блокировал отмену, но не считать медленную propagation ошибкой релиза. Использовать Node timer вместо внешнего `sleep`.
