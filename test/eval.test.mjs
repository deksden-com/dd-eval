...
test("case pins its input checkpoint and exact engine without Session starter state", async () => {
  ...
  assert.equal(loaded.inputCheckpoint.value.id, "cp-091-task-priority-shared-runtime-flow-4-1-0-engine-0-9-0-beta-42");
  ...
  assert.equal(loaded.inputCheckpoint.value.flow_pack.engine.version, "0.9.0-beta.42");
  assert.equal(loaded.inputCheckpoint.value.flow_pack.engine.commit, "fb9736bf2af837404a43027631113232f0134b58");
  assert.equal(loaded.inputCheckpoint.value.flow_pack.engine.artifact_sha256, "7d0ba0ffaf22a3ce6bd65ce8e8c4f28d9cfce82802fdfc054489bc9757626a31");
