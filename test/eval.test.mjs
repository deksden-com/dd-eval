...
test("case pins its input checkpoint and exact engine without Session starter state", async () => {
  ...
  assert.equal(loaded.inputCheckpoint.value.id, "cp-062-task-priority-flow-4-0-1-engine-0-9-0-beta-7-merge-gate");
  assert.equal(loaded.inputCheckpoint.value.source.commit, "86db34668add2cd2f9a7c59adfced5e7ae57c3b8");
  assert.equal(loaded.inputCheckpoint.value.flow_pack.commit, "02f5bf905e030158bc96d629e63b47d6c5283470");
  assert.equal(loaded.inputCheckpoint.value.flow_pack.engine.version, "0.9.0-beta.7");
