...
test("case pins its input checkpoint and exact engine without Session starter state", async () => {
  ...
  assert.equal(loaded.inputCheckpoint.value.id, "cp-066-task-priority-project-flow-pack-4-0-2-engine-0-9-0-beta-11");
  ...
  assert.equal(loaded.inputCheckpoint.value.flow_pack.engine.version, "0.9.0-beta.11");
