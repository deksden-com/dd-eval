...
`, { mode: 0o755 });
  ...
  try {
    ...
    assert.equal(workspaceHooks["dd-flow"].PreToolUse[0].matcher, "^run_command$");
    assert.equal(workspaceHooks["dd-flow"].PostToolUse, undefined);
    assert.ok(workspaceHooks["dd-flow"].Stop);
    assert.equal(workspaceHooks.PreToolUse, undefined);
