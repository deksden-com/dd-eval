...
const args=process.argv.slice(2); if(args.includes('--version')){console.log('1.1.22');process.exit(0)} if(args.includes('models')){console.log('gemini-3.1-pro-high available');process.exit(0)}
...
`, { mode: 0o755 });
  try {
    ...
    const created = await callDaemon(state, "session.create", { prompt: "specify" });
    assert.equal(created.provider_session_id, "agy-root"); assert.equal(created.result.status, "SUCCESS"); assert.equal(created.usage.total_tokens, 5);
