  ...
  try {
    ...
    await writeFile(server, `
      ...
      readline.createInterface({ input: process.stdin }).on("line", (line) => {
        ...
        else if (method === "zcode/session/subagents") result = { running: [], completed: [] };
        else if (method === "zcode/session/usage") result = { inputTokens: 12, outputTokens: 3 };
        else if (method === "session/prompt") { send({ jsonrpc: "2.0", method: "session/update", params: { sessionId: params.sessionId, update: { sessionUpdate: "agent_message_chunk", content: { type: "text", text: "ok" } } } }); result = { stopReason: "end_turn" }; }
    ...
    const notifications = [];
    const prompted = await promptSession({ ...common, sessionId: "native-root", prompt: "work", onNotification: (event) => notifications.push(event) });
    assert.equal(prompted.turn.stopReason, "end_turn");
    assert.equal(notifications.length, 1);
    const forked = await forkSession({ ...common, sessionId: "native-root", target: { kind: "latestCheckpoint" } });
